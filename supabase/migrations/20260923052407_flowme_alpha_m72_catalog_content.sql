-- M7-2 DEV ONLY: wkmzcxpnojobxrgebapw. No user rows, source catalog, public state or grants changed.
-- Extend the signed creator validator with a content-only locator. Existing M3/M4 writers and transaction/Undo semantics remain unchanged.
...
begin
  ...
  if input_command->>'kind' is distinct from 'creator'
    ...
    or input_command#>>'{intent,type}' <> all(array['working','library-action','native-operation','source-stage',
      'source-transition','source-upgrade','history-restore','raw-handoff','native-handoff','native-lineage','raw-update','catalog-content-import'])
