...
const outputRoot = path.resolve(
  ...
  process.env.FLOWME_P35_OFFLINE_PREVIEW_DIR
    ?? 'docs/content-audit/2026-07-27-p35-owner-feedback-independent-review-handoff/offline-preview',
);
...
await runCase('My Flow mobile workspace states', async () => {
  ...
  await executionShell.getByRole('button', { name: /열기/u }).click();
  ...
  await workspace.getByTestId('my-flow-workspace-tab-plan').click();
  await capture(page, errors, {
  ...
  await capture(page, errors, {
    ...
    label: '내 Flow 기록',
    reviewQuestions: ['기록 탭의 정체성이 명확한가', 'history·회고·재사용이 섞이는가'],
  });

  await workspace.getByTestId('my-flow-workspace-tab-execute').click();
