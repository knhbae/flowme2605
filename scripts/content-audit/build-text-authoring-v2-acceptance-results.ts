...
const RESULT_PATH = resolve(
  RESULT_DIRECTORY,
  'simulation-matrix-v2-results.json',
);
const UI_EVIDENCE_PATH = resolve(
  REPO_ROOT,
  'docs/content-audit/2026-07-31-flowme-text-authoring-grammar-simulation',
  'ui-simulation-evidence.json',
);
...
function semanticSnapshot(document: TextAuthoringDocument): JsonRecord {
  const stepTitleById = new Map(
    document.parseResult.canonical.steps.map((step) => [
      step.stepId,
      step.title,
    ]),
  );
  ...
  const anchor =
    document.parseResult.canonical.fields.find(
...
function g01(): AcceptanceCaseOutput {
  ...
  return {
    ...
    evidence: [
      ...
      evidence(
        '설명은 바로 위 Item의 detail이다',
        actual.detail,
        '설명입니다.',
      ),
      evidence('속성 bullet 해석 이슈가 없다', actual.issueTypes, []),
      evidence('Todo 제목에 속성명이 섞이지 않는다', actual.todoTitles, [
        '항목',
      ]),
...
function g03(): AcceptanceCaseOutput {
  ...
  const actual = {
    ...
    links: linkValues(item),
    todoTitles: [
      ...new Set(result.artifacts.todo.rows.map((row) => row.title)),
    ],
    issueTypes: issueTypes(document),
  };
  ...
  return {
    ...
    evidence: [
      evidence(
        '공식 속성 전체를 넣어도 Item 수가 불어나지 않는다',
        actual.itemCount,
        1,
      ),
      ...
      evidence('Markdown 링크를 보존한다', actual.links, [
        { label: '공식 자료', url: 'https://example.com/official' },
      ]),
      evidence('속성명이 결과 제목이 되지 않는다', actual.todoTitles, [
        '전체 속성 항목',
      ]),
...
function g04(): AcceptanceCaseOutput {
  ...
  return {
    ...
    evidence: [
      ...
      evidence(
        '앞서 쓴 시간도 뒤의 날짜 schedule에 재적용된다',
        item.schedule?.time,
        '09:30',
      ),
      evidence(
        '앞서 쓴 반복도 뒤의 날짜 schedule에 재적용된다',
        item.schedule?.repeat,
        '매주',
      ),
      evidence('설명을 보존한다', actual.detail, '마지막에 쓴 설명'),
      evidence('writer는 공식 순서로 정규화한다', writerPropertyLabels, [
        '설명',
        '날짜',
        '시간',
        '반복',
...
function unknownPropertyCase(
  ...
  const item = document.parseResult.canonical.items[0];
  const preservedDetail = unknownLine.trim().replace(/^-\s*/u, '');
  const actual = {
    itemCount: document.parseResult.canonical.items.length,
  ...
  return {
    ...
    evidence: [
      ...
      evidence(
        '알려지지 않은 속성을 원문 표기 그대로 설명에 보존한다',
        actual.detail,
        preservedDetail,
      ),
      includesEvidence(
        '문제 행의 원문을 source row로 보존한다',
        actual.sourceRows,
        unknownLine,
      ),
...
function g06(): AcceptanceCaseOutput {
  ...
  const actual = {
    ...
    detail: item?.detail ?? null,
    subchecks: (item?.subchecks ?? []).map((subcheck) => ({
  ...
  return {
    ...
    evidence: [
      ...
      evidence(
        '하위 체크박스를 Todo의 checklist로 보존한다',
        actual.subchecks,
        [{ title: '하위 작업', sourceChecked: false }],
      ),
      includesEvidence(
        '중첩 원문을 source row로 보존한다',
        actual.sourceRows,
        nestedLine,
      ),
    ],
...
function g08(): AcceptanceCaseOutput {
  ...
  const actual = {
    ...
    date:
      document.parseResult.canonical.items[0]?.schedule?.kind === 'absolute'
...
function g09(): AcceptanceCaseOutput {
  ...
  return {
    ...
    evidence: [
      evidence(
        '표식 없는 문장은 canonical Item을 만들지 않는다',
        actual.itemCount,
        0,
      ),
      evidence(
        '원문 텍스트를 byte-equivalent로 보존한다',
        actual.rawText,
        rawText,
      ),
      includesEvidence(
        '원문 문장을 source row로 보존한다',
        actual.sourceRows,
        rawText,
      ),
      evidence('명시적 승인 전 ambiguous_role로 남긴다', actual.issueTypes, [
        'ambiguous_role',
      ]),
    ],
    apis: ['createTextAuthoringDocument'],
...
function g10(): AcceptanceCaseOutput {
  ...
  const actual = {
    ...
    nestingLevels: reparsed.parseResult.canonical.items.map(
      (item) => item.nestingLevel,
    ),
    hasDuplicateProperty,
    visibleV2PropertyCount: markdown
      .split(/\r?\n/u)
  ...
  return {
    ...
    evidence: [
      evidence(
        '실제 parser 재입력에서 Step, Item, field, link, schedule이 같다',
        actual.semanticEquivalent,
        true,
      ),
      ...
      evidence(
        '지원 Markdown receipt의 변경 수가 0이다',
        receipt.changedCount,
        0,
      ),
      evidence(
        '지원 Markdown receipt의 미해결 수가 0이다',
...
function d01(): AcceptanceCaseOutput {
  ...
  return {
    actual,
    evidence: [
      evidence('canonical은 작성 순서를 유지한다', sourceOrder, [
        '늦은 항목',
        '빠른 항목',
      ]),
      evidence('Todo는 작성 순서를 유지한다', todoOrder, sourceOrder),
      evidence('Calendar만 날짜 오름차순이다', calendarOrder, [
        '빠른 항목',
        '늦은 항목',
      ]),
      evidence(
        'projection은 원문을 자동 변경하지 않는다',
        actual.sourceMutationCount,
        0,
      ),
      evidence('원문이 byte-equivalent다', actual.rawSourceUnchanged, true),
      evidence(
        '원문 정렬 적용 가능 상태를 계산할 수 있다',
        actual.sourceAlignmentAvailable,
        true,
      ),
    ],
    apis: ['createTextAuthoringDocument', 'buildAuthoringArtifactProjection'],
...
function d02(): AcceptanceCaseOutput {
  ...
  const beforeIds = document.parseResult.canonical.items.map(
    (item) => item.itemId,
  );
  const beforeLineage = itemLineage(document);
  const orderedItemIds = projection(document).artifacts.calendar.rows.map(
    (row) => row.itemId,
  );
  const aligned = applyAuthoringOperation(
    document,
    {
  ...
  const actual = {
    ...
    sourceLineagePreserved: isDeepStrictEqual(
      itemLineage(aligned),
      beforeLineage,
    ),
  ...
  return {
    ...
    evidence: [
      evidence(
        '명시적 작업 후 같은 Step 안 Item 순서가 날짜순이다',
        actual.alignedOrder,
        ['빠른 항목', '늦은 항목'],
      ),
      evidence(
        '소유 속성이 Item 블록과 함께 이동한다',
        actual.propertyMovedWithItem,
        true,
      ),
      evidence('stable Item ID를 보존한다', actual.stableItemIds, true),
      evidence(
        'source lineage를 보존한다',
        actual.sourceLineagePreserved,
        true,
      ),
      evidence(
        'revision에 명시적 원문 정렬 작업을 기록한다',
        actual.revisionOperation,
        'align_source_order',
      ),
      evidence('undo가 원문을 복원한다', actual.undoRawRestored, true),
      evidence('undo가 원래 Item 순서를 복원한다', actual.undoOrder, [
        '늦은 항목',
        '빠른 항목',
      ]),
    ],
...
function d03(): AcceptanceCaseOutput {
  ...
  return {
    ...
    evidence: [
      ...
      evidence(
        '이미 정렬된 원문은 새 revision을 만들지 않는다',
        actual.noRewriteNeeded,
        true,
      ),
    ],
    apis: [
...
function d04(): AcceptanceCaseOutput {
  ...
  const actual = {
    ...
    calendarDates: result.artifacts.calendar.rows.map(
      (row) => row.date ?? null,
    ),
  ...
  return {
    ...
    evidence: [
      evidence(
        '체크/할 일은 활성 상태다',
        [actual.todoEligible, actual.todoCount],
        [true, 2],
      ),
      evidence(
        '기준일 없는 상대 날짜는 Calendar를 만들지 않는다',
        [actual.calendarEligible, actual.calendarCount],
        [false, 0],
      ),
      evidence(
...
function d05(): AcceptanceCaseOutput {
  ...
  const actual = {
    ...
    scheduleAnchors: document.parseResult.canonical.items.map((item) =>
    ...
    relativeExpressions: document.parseResult.canonical.items.map((item) =>
      item.schedule?.kind === 'relative' ? item.schedule.raw : null,
    ),
  };
  return {
    actual,
    evidence: [
      evidence(
        '원문의 ISO 기준일을 flow field로 보존한다',
        actual.anchorField,
        '2026-08-10',
      ),
      evidence('상대 schedule에 계산 기준을 연결한다', actual.scheduleAnchors, [
        '2026-08-10',
        '2026-08-10',
      ]),
      evidence('Calendar가 활성화된다', actual.calendarEligible, true),
      evidence('D-3부터 D-Day까지 날짜순으로 계산한다', actual.calendarDates, [
        '2026-08-07',
        '2026-08-10',
      ]),
      evidence(
        '원문 상대 표현을 canonical schedule에 보존한다',
        actual.relativeExpressions,
        ['D-3', 'D-Day'],
      ),
    ],
    apis: ['createTextAuthoringDocument', 'buildAuthoringArtifactProjection'],
...
function d06(): AcceptanceCaseOutput {
  ...
  const actual = {
    ...
    persistedRaw,
    anchorLineIndex: persistedRaw
      .split(/\r?\n/u)
      .indexOf('- 기준일: 2026-08-10'),
    calendarDates: firstProjection.artifacts.calendar.rows.map(
      (row) => row.date,
    ),
    refreshedCalendarDates: refreshedProjection.artifacts.calendar.rows.map(
      (row) => row.date,
    ),
  };
  return {
    ...
    evidence: [
      evidence(
        'UI-only anchor option은 Calendar를 활성화하지 않는다',
        actual.hiddenOnlyCalendarCount,
        0,
      ),
      evidence(
        '선택한 기준일은 원문 제목 바로 뒤 canonical 줄로 지속된다',
        actual.anchorLineIndex,
        1,
      ),
      evidence('원문 반영 직후 Calendar가 계산된다', actual.calendarDates, [
        '2026-08-07',
      ]),
      evidence(
        '같은 원문을 새로 파싱해도 결과가 같다',
        actual.refreshedCalendarDates,
        actual.calendarDates,
      ),
    ],
    apis: [
      'createTextAuthoringDocument',
...
function d07(): AcceptanceCaseOutput {
  ...
  const result = projection(document);
  const orderedItemIds = result.artifacts.calendar.rows.map(
    (row) => row.itemId,
  );
  const aligned = applyAuthoringOperation(
    document,
  ...
  return {
    ...
    evidence: [
      ...
      evidence('모든 Item은 Todo에 작성 순서로 남는다', actual.todoOrder, [
        '절대 날짜 항목',
        '날짜 없는 항목',
        '상대 날짜 항목',
      ]),
...
function d08(): AcceptanceCaseOutput {
  ...
  const actual = {
    issueTypes: issueTypes(document),
    schedules: document.parseResult.canonical.items.map((item) =>
      scheduleValue(item),
    ),
    calendarCount: result.artifacts.calendar.count,
  ...
  return {
    ...
    evidence: [
      evidence(
        '모호한 날짜마다 invalid_date 이슈를 남긴다',
        actual.issueTypes,
        ['invalid_date', 'invalid_date'],
      ),
      evidence(
        '연도나 날짜를 추정해 schedule을 만들지 않는다',
        actual.schedules,
        [null, null],
      ),
...
function d09(): AcceptanceCaseOutput {
  ...
  const stepOrders = aligned.parseResult.canonical.steps.map((step) => ({
    ...
    itemTitles: step.itemIds.map(
      (itemId) =>
        aligned.parseResult.canonical.items.find(
  ...
  const actual = {
    ...
    stableItemIds: Object.keys(beforeStepById).every(
      (itemId) => itemId in afterStepById,
    ),
    sourceLineagePreserved: isDeepStrictEqual(
      beforeLineage,
      itemLineage(aligned),
  ...
  return {
    actual,
    evidence: [
      evidence('각 Step 내부에서만 날짜순 정렬한다', stepOrders, [
        { stepTitle: '첫 단계', itemTitles: ['첫 단계 빠름', '첫 단계 늦음'] },
        {
          stepTitle: '둘째 단계',
          itemTitles: ['둘째 단계 빠름', '둘째 단계 늦음'],
        },
      ]),
      evidence(
        'Item이 다른 Step으로 이동하지 않는다',
        actual.stepMembershipPreserved,
        true,
      ),
...
function hasStableArtifactViewContract(
  ...
  return Object.values(result.artifacts).every(
    (view) =>
...
function a01(): AcceptanceCaseOutput {
  ...
  const dated = projection(
    authoringDocument(
      'A01',
      ['# 날짜 있음', '## 실행', '- [ ] 항목', '  - 날짜: 2026-08-03'].join(
        '\n',
      ),
    ),
  );
  ...
  const slotKeys = [undated, dated, table].map((result) =>
    Object.keys(result.artifacts),
  );
  const viewShapes = [undated, dated, table].map(artifactViewShape);
  const stableViewContracts = [undated, dated, table].map(
    hasStableArtifactViewContract,
  );
...
function a02(): AcceptanceCaseOutput {
  ...
  return {
    actual,
    evidence: [
      evidence('일반 목록은 Todo가 활성이다', actual.todo, {
        eligible: true,
        count: 3,
      }),
      evidence('일반 목록은 텍스트/Memo가 활성이다', actual.memo, {
        eligible: true,
        count: 3,
      }),
      evidence(
        '제목뿐인 일반 목록의 Sheet는 비활성이다',
        [actual.sheet.eligible, actual.sheet.count],
        [false, 0],
      ),
      includesEvidence(
        'Sheet 비활성 이유를 제공한다',
        actual.sheet.reasons,
...
function a03(): AcceptanceCaseOutput {
  ...
  const actual = {
    ...
    sourceTableRowCount: document.parseResult.canonical.sourceRows.filter(
  ...
  return {
    ...
    evidence: [
      ...
      evidence(
        '원본 표는 Sheet를 우선 추천한다',
        actual.primaryArtifact,
        'sheet',
      ),
      evidence('원본 열 이름을 유지한다', actual.columns, [
        '활동',
        '담당',
        '자료',
      ]),
...
function a04(): AcceptanceCaseOutput {
  ...
  return {
    ...
    evidence: [
      evidence(
        '반복되는 의미 필드 3개가 있으면 Sheet가 활성이다',
        actual.eligible,
        true,
      ),
      ...
      evidence(
        ...
        ['항목', '날짜', '장소', '완료 기준'].every((label) =>
          actual.columns.includes(label),
        ),
        true,
      ),
      evidence(
        '단순 순서 열은 생성하지 않는다',
        actual.columns.includes('순서'),
        false,
      ),
    ],
    apis: ['createTextAuthoringDocument', 'buildAuthoringArtifactProjection'],
...
function a05(): AcceptanceCaseOutput {
  ...
  return {
    ...
    evidence: [
      evidence(
        '원문 그대로 복사는 byte-equivalent다',
        actual.rawByteEquivalent,
        true,
      ),
      evidence(
        '정리된 TXT는 원문과 다른 변환 결과다',
        actual.plainTextTransformed,
        true,
      ),
      evidence(
        '정리된 Markdown은 원문과 다른 변환 결과다',
        actual.markdownTransformed,
        true,
      ),
      evidence(
        '변환 결과에 정리 메모라는 모호한 이름을 넣지 않는다',
        actual.ambiguousLabelPresent,
        false,
      ),
    ],
    apis: [
...
function a06(): AcceptanceCaseOutput {
  ...
  return {
    ...
    evidence: [
      evidence(
        'Calendar row가 실행 상세 전체를 제공한다',
        actual.calendar,
        expectedDetail,
      ),
      evidence(
        'Todo row가 실행 상세와 링크를 제공한다',
        actual.todo,
        expectedDetail,
      ),
      evidence(
        ...
        tableRows[0]?.some((cell) =>
          String(cell).includes('https://example.com/booking'),
        ),
        true,
      ),
      evidence(
...
function a07(): AcceptanceCaseOutput {
  ...
  const nonSheetTableLeaks = owners.filter(
    ({ artifact, format }) =>
      artifact !== 'sheet' && ['csv', 'tsv', 'xlsx'].includes(format),
  );
  const rawSourceOwners = owners.filter(
    ({ format }) => format === 'raw_source',
  );
  const actual = {
  ...
  return {
    ...
    evidence: [
      evidence(
        'CSV·TSV·XLSX는 Sheet 맥락에만 노출된다',
        nonSheetTableLeaks,
        [],
      ),
      evidence('Sheet 결과 메뉴가 표 내보내기 형식을 제공한다', tableFormats, [
        'csv',
        'tsv',
        'xlsx',
      ]),
      evidence('원문 복사는 Memo/text 맥락에만 있다', rawSourceOwners, [
        { artifact: 'memo', format: 'raw_source' },
      ]),
    ],
...
function a08(): AcceptanceCaseOutput {
  ...
  const actual = {
    ...
    displayedOrdinals: document.parseResult.canonical.items.map(
      (item) => item.order + 1,
    ),
    writerItemLines: markdown
      .split(/\r?\n/u)
      .filter((line) => /^- \[ \] /u.test(line)),
    numberedItemLines: markdown
      .split(/\r?\n/u)
  ...
  return {
    actual,
    evidence: [
      evidence('번호 목록을 Item 3개로 읽는다', actual.titles, [
        '첫 항목',
        '둘째 항목',
        '셋째 항목',
      ]),
      evidence(
        'canonical order로 1, 2, 3을 계산할 수 있다',
        actual.displayedOrdinals,
        [1, 2, 3],
      ),
      evidence(
        'writer는 모든 Item을 canonical 체크박스로 쓴다',
        actual.writerItemLines.length,
        3,
      ),
      evidence(
        'writer는 수동 번호 표식을 남기지 않는다',
        actual.numberedItemLines,
        [],
      ),
    ],
    apis: ['createTextAuthoringDocument', 'exportTextAuthoringMarkdown'],
...
export function buildV2AcceptanceMatrixResults(
  ...
  const expectedIds = [...V2_AUTOMATED_ACCEPTANCE_IDS, ...V2_BROWSER_QA_IDS];
  const matrixIds = matrix.scenarios.map((scenario) => scenario.id);
  if (!isDeepStrictEqual(matrixIds, expectedIds)) {
    throw new Error(
      `simulation matrix IDs differ: ${JSON.stringify(matrixIds)}`,
    );
  }

  const rows = matrix.scenarios.map((scenario) => {
    const automatedResult = automated.get(
      scenario.id as V2AutomatedAcceptanceId,
    );
    if (automatedResult) {
      return {
        id: scenario.id,
    ...
    return {
      ...
      actual: {
        execution: 'not_run',
        reason:
          'viewport, interaction, focus, accessibility, and browser diagnostics require browser evidence',
      },
      pass: null,
      ...
      evidence: [
        {
          requirement:
            '브라우저에서 직접 확인할 때까지 자동 통과로 간주하지 않는다',
          expectedEvidence:
            'viewport screenshots, interaction assertions, focus/accessibility checks, and browser diagnostics',
          status: 'pending_browser_qa',
        },
      ],
      apis: [],
  ...
  return {
    ...
    summary: {
      ...
      pendingBrowserQa: rows.filter(
        (row) => row.status === 'pending_browser_qa',
      ).length,
    },
    rows,
  };
...
function main(): void {
  ...
  console.log(
    JSON.stringify(
      {
        output:
          'docs/content-audit/2026-08-04-flowme-text-authoring-grammar-ux-improvement-results/simulation-matrix-v2-results.json',
        ...summary,
      },
      null,
      2,
