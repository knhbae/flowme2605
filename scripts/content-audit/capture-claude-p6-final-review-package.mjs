...
const packageName = '2026-07-04-claude-design-p6-final-review-package';
...
const baseURL = process.env.FLOWME_EVIDENCE_BASE_URL || `http://127.0.0.1:${process.env.FLOWME_EVIDENCE_PORT || '3220'}`;
...
const p6DisplayTerms = [
  /저장 전 보기/,
  /일정 지도/,
  /저장한 지도/,
  /지도 원문/,
  /진도형 Flow/,
  /\bAJD\s+(?:[가-힣A-Z0-9]|D-)/i,
  /\bMathbang\s+[가-힣A-Z0-9]/i,
];
...
const routeShots = [
  ...
  { route: '/flow-maps/moving-d30', file: '03-flow-map-moving-mobile.png', label: '이사 Flow Map 저장 전' },
  { route: '/flow-maps/middle-school-math-1', file: '04-flow-map-math-mobile.png', label: '중1 수학 Flow Map 저장 전' },
  ...
  { route: '/restart/moving-d30', file: '12-restart-moving-mobile.png', label: '이사 restart prototype' },
  { route: '/my', file: '13-my-empty-mobile.png', label: 'My Flow 빈 상태' },
  { route: '/calendar', file: '14-calendar-empty-mobile.png', label: '캘린더 빈 상태' },
];

const scenarioLabels = {
  s01: '첫 진입과 Flow 찾기 스캔',
  s02: '이사 Flow Map 저장 후 My Flow/Calendar',
  s03: '날짜 없는 중1 수학 저장 후 첫 실행',
  s04: '공유 /f 저장 전/후와 My Flow 전환',
  s05: '특수 workbench와 restart 하단 접근성',
  s06: '여러 Flow 저장 후 반복 사용자 실행 허브',
  s07: '하단 sticky/fixed layer 최하단 확인',
};
...
async function capture(page, records, input) {
  ...
  const evidence = await page.evaluate((payload) => {
    ...
    const matchedInternalTerms = payload.internalTerms
      ...
      .map((term) => term.label);
    const matchedP6Terms = payload.p6Terms
      .filter((term) => textLines.some((line) => new RegExp(term.source, term.flags).test(line)))
      .map((term) => term.label);
    const matchedP6Lines = textLines
      .filter((line) => payload.p6Terms.some((term) => new RegExp(term.source, term.flags).test(line)))
      .slice(0, 20);
    ...
    function count(text) {
      return textLines.filter((line) => line.includes(text)).length;
    }
    ...
    return {
      ...
      rawIsoLines,
      p6Counts: {
        movingFirstTaskTitle: count('이사 방식과 견적 후보 정하기'),
        mathFirstTaskTitle: count('1. 소인수분해'),
        firstTaskLabel: count('먼저 할 일'),
        overdueLabel: count('밀린 할 일'),
        nextTaskLabel: count('다음 할 일'),
        checkItemsLong: count('확인할 항목'),
        checkItemsShort: count('확인 항목'),
      },
      markers: {
        ...
        movingRestartSource: Boolean(document.querySelector('[data-testid="moving-source-section"]')),
      },
      fixedRects: {
    ...
    p6Terms: p6DisplayTerms.map((term) => ({ label: term.toString(), source: term.source, flags: term.flags })),
...
async function captureScenarios(browser, records) {
  ...
  await scenarioFixedLayerBottoms(browser, records);
}
...
async function scenarioHomeToFlows(browser, records) {
  ...
  await settle(page);
  await capture(page, records, { kind: 'scenario', scenarioId: 's01', label: 'Flow 찾기 상단', note: '통합 콘텐츠 목록 상단과 카드 CTA 문구 확인', file: 's01-02-flows-top.png' });
  await page.locator('[data-testid="flow-map-catalog-card"], [data-testid="single-flow-catalog-card"]').first().scrollIntoViewIfNeeded();
  await capture(page, records, { kind: 'scenario', scenarioId: 's01', label: '첫 콘텐츠 카드', note: '카드 정보 위계와 열어보기 CTA 확인', file: 's01-03-first-card.png' });
  await context.close();
}
...
async function scenarioMovingMap(browser, records) {
  ...
  await page.getByTestId('my-flow-post-save-panel').waitFor({ state: 'visible' });
  await page.getByTestId('my-flow-workspace').waitFor({ state: 'visible' });
  await capture(page, records, { kind: 'scenario', scenarioId: 's02', label: '저장 직후 My Flow', note: '첫 할 일 제목과 상태 라벨이 반복되지 않는지 확인', file: 's02-03-post-save-my-flow.png' });
  ...
  await capture(page, records, { kind: 'scenario', scenarioId: 's02', label: '첫 할 일 상세', note: '확인 항목 라벨과 체크/메모/source/export 위계 확인', file: 's02-04-first-task-detail.png' });
...
async function scenarioMathMap(browser, records) {
  ...
  await capture(page, records, { kind: 'scenario', scenarioId: 's03', label: '중1 수학 저장 전', note: 'Mathbang/source slug와 일정 지도 표현 제거 확인', file: 's03-01-math-map-before-save.png' });
  await page.getByTestId('flow-map-save-all-mobile').click();
  await page.waitForURL('**/my?savedMap=middle-school-math-1');
  await page.getByTestId('my-flow-post-save-panel').waitFor({ state: 'visible' });
  await capture(page, records, { kind: 'scenario', scenarioId: 's03', label: '날짜 없는 콘텐츠 저장 후', note: '빈 상태 대신 첫 실행 항목과 제목 반복 제거 확인', file: 's03-02-math-post-save.png' });
  await page.getByTestId('my-flow-post-save-open-first').click();
  await page.getByTestId('my-flow-inline-detail').first().waitFor({ state: 'visible' });
  await capture(page, records, { kind: 'scenario', scenarioId: 's03', label: '날짜 없는 첫 할 일 상세', note: '진도표 상세와 확인 항목 라벨 확인', file: 's03-03-math-first-task.png' });
  await context.close();
}
...
async function scenarioPublicFlow(browser, records) {
  ...
  await capture(page, records, { kind: 'scenario', scenarioId: 's04', label: '공유 전세계약 서류', note: '공유 상세 CTA와 source/detail 위계 확인', file: 's04-05-jeonse-before-save.png' });
  ...
  await capture(page, records, { kind: 'scenario', scenarioId: 's04', label: '공유 이사 D-30', note: '입력과 저장/export 위계, source brand 표시 정리 확인', file: 's04-06-public-moving.png' });
  await context.close();
}
...
async function scenarioWorkbenches(browser, records) {
  ...
  await resetStorage(page, '/restart/moving-d30');
  await capture(page, records, { kind: 'scenario', scenarioId: 's05', label: '이사 restart 첫 화면', note: 'AJD source brand 접두어와 restart 화면 톤 확인', file: 's05-08-restart-top.png' });
  await page.getByTestId('moving-source-section').scrollIntoViewIfNeeded();
  await capture(page, records, { kind: 'scenario', scenarioId: 's05', label: '이사 restart 출처 영역', note: 'source 링크는 유지하되 source brand가 콘텐츠명처럼 튀지 않는지 확인', file: 's05-09-restart-source.png' });
  await page.evaluate(() => window.scrollTo(0, document.documentElement.scrollHeight));
  await capture(page, records, { kind: 'scenario', scenarioId: 's05', label: '이사 restart 하단', note: '마지막 콘텐츠와 하단 접근성 확인', file: 's05-10-restart-bottom.png' });
  await context.close();
}
...
async function scenarioRepeatUser(browser, records) {
  ...
  await page.waitForURL('**/my?savedMap=middle-school-math-1');
  await page.getByTestId('my-flow-post-save-panel').waitFor({ state: 'visible' });
  await capture(page, records, { kind: 'scenario', scenarioId: 's06', label: '여러 콘텐츠 저장 직후', note: '방금 저장한 콘텐츠의 첫 실행 항목과 반복 라벨 확인', file: 's06-01-multiple-post-save.png' });
...
async function scenarioFixedLayerBottoms(browser, records) {
  const context = await newMobileContext(browser);
  const page = await context.newPage();
  for (const item of [
    { route: '/f/vehicle-inspection-prep', file: 's07-01-public-vehicle-bottom.png', label: '공유 자동차검사 최하단' },
    { route: '/f/moving-d30-basic', file: 's07-02-public-moving-bottom.png', label: '공유 이사 D-30 최하단' },
    { route: '/flow-maps/moving-d30', file: 's07-03-flow-map-moving-bottom.png', label: '이사 Flow Map 최하단' },
    { route: '/flow-maps/middle-school-math-1', file: 's07-04-flow-map-math-bottom.png', label: '중1 수학 Flow Map 최하단' },
    { route: '/calendar', file: 's07-05-calendar-bottom-clean.png', label: '캘린더 빈 상태 최하단' },
  ]) {
    await resetStorage(page, item.route);
    await page.evaluate(() => window.scrollTo(0, document.documentElement.scrollHeight));
    await capture(page, records, { kind: 'scenario', scenarioId: 's07', label: item.label, note: 'fixed/sticky 하단 레이어와 마지막 콘텐츠 관계 확인', file: item.file });
  }
  await context.close();
}
...
function validate(records) {
  ...
  for (const item of records) {
    ...
    if (item.matchedP6Terms.length > 0) failures.push(`${item.screenshot}: P6 display terms ${item.matchedP6Lines.join(' | ')}`);
    if (item.rawIsoLines.length > 0 && !item.url.startsWith('/restart/')) failures.push(`${item.screenshot}: raw ISO dates ${item.rawIsoLines.join(' | ')}`);
  }
  for (const file of ['s02-03-post-save-my-flow.png', 's03-02-math-post-save.png', 's06-01-multiple-post-save.png']) {
    const item = records.find((record) => record.screenshot.endsWith(file));
    if (!item?.markers.postSavePanel) failures.push(`${file}: missing post-save marker`);
  }
  const movingPostSave = records.find((item) => item.screenshot.endsWith('s02-03-post-save-my-flow.png'));
  if (movingPostSave?.p6Counts.movingFirstTaskTitle > 1) failures.push('moving post-save repeats first task title more than once');
  const mathPostSave = records.find((item) => item.screenshot.endsWith('s03-02-math-post-save.png'));
  if (mathPostSave?.p6Counts.mathFirstTaskTitle > 1) failures.push('math post-save repeats first task title more than once');
  ...
  const restartSource = records.find((item) => item.screenshot.endsWith('s05-09-restart-source.png'));
  if (!restartSource?.markers.movingRestartSource) failures.push('restart source evidence missing source section marker');
  return failures;
}
...
function renderReadme(payload) {
  return `# Claude Design P6 final review package
...
function renderAudit(payload) {
  const routeRows = payload.records
    .map((item) => `| ${item.kind} | ${item.scenarioId || '-'} | \`${item.url}\` | ${item.label} | [${path.basename(item.screenshot)}](./${item.screenshot}) | ${item.noHorizontalOverflow ? 'OK' : 'overflow'} | ${item.matchedP6Terms.length} | ${item.rawIsoLines.length} | ${item.p6Counts.movingFirstTaskTitle}/${item.p6Counts.mathFirstTaskTitle} |`)
    .join('\n');
...
- P6 표시어(\`저장 전 보기\`, \`일정 지도\`, \`AJD ...\`, \`Mathbang ...\`) hits: ${payload.validation.p6DisplayTermHits}
- Raw ISO visible text hits outside \`/restart\`: ${payload.validation.rawIsoHits}
- \`/restart/moving-d30\` prototype raw ISO residual hits: ${payload.validation.restartRawIsoHits}
- Post-save repeated first-task title failures: ${payload.validation.repeatedFirstTaskTitleFailures}
...
function renderPrompt(payload) {
  return `FlowMe P6-01~P6-08 개선 루프를 다시 검토해줘.
...
- Branch: \`${branchName}\`
- Commit: \`${baselineCommit}\`
- P6 표시어 scan hits: ${payload.validation.p6DisplayTermHits}
- raw ISO visible text hits outside \`/restart\`: ${payload.validation.rawIsoHits}
- \`/restart/moving-d30\` prototype raw ISO residual hits: ${payload.validation.restartRawIsoHits}
- trailing Flow title hits: ${payload.validation.flowSuffixHits}
- horizontal overflow failures: ${payload.validation.horizontalOverflowFailures}
- repeated first-task title failures: ${payload.validation.repeatedFirstTaskTitleFailures}
`;
}
    ...
    .map((item) => `
      <article class="card">
        ...
        <dl>
          ...
          <div><dt>P6 terms</dt><dd>${item.matchedP6Terms.length}</dd></div>
          <div><dt>ISO</dt><dd>${item.rawIsoLines.length}</dd></div>
          <div><dt>First-title count</dt><dd>${item.p6Counts.movingFirstTaskTitle}/${item.p6Counts.mathFirstTaskTitle}</dd></div>
        </dl>
      </article>
    `)
    .join('');
...
<head>
  ...
  <title>FlowMe Claude Design P6 Final Review</title>
  <style>
...
<body>
  <main>
    <header>
      <p><strong>FlowMe Claude Design P6 Final Review</strong></p>
      <h1>P6-01~P6-08 마감 감사 패키지</h1>
      <p>모바일 390px 기준 route top-shot과 실제 저장 시나리오 screenshot을 함께 묶었습니다. Claude Design이 Vercel 없이 GitHub 파일만 보고 P7 backlog를 판단할 수 있게 만드는 패키지입니다.</p>
    ...
    <section class="summary">
      ...
      <div class="pill"><strong>${payload.validation.p6DisplayTermHits}</strong><span>P6 display term hits</span></div>
...
try {
  ...
  const repeatedFirstTaskTitleFailures = failures.filter((failure) => failure.includes('repeats first task title')).length;
  const payload = {
    ...
    validation: {
      ...
      p6DisplayTermHits: records.reduce((sum, item) => sum + item.matchedP6Terms.length, 0),
      rawIsoHits: records.reduce((sum, item) => item.url.startsWith('/restart/') ? sum : sum + item.rawIsoLines.length, 0),
      restartRawIsoHits: records.reduce((sum, item) => item.url.startsWith('/restart/') ? sum + item.rawIsoLines.length : sum, 0),
      repeatedFirstTaskTitleFailures,
    },
    github: {
      readme: `${githubBase}/docs/content-audit/${packageName}/README.md`,
  ...
  if (failures.length > 0) {
    throw new Error(`P6 review package validation failed:\n${failures.join('\n')}`);
  }
