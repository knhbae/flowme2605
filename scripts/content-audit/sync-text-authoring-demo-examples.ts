...
import type { GrammarSimulationScenario } from "../../lib/flow/text-authoring/grammar-simulation";
...
function expectedResultLabel(scenario: GrammarSimulationScenario): string {
  const destination = scenario.naturalDestination;
  if (scenario.group === "exception_handling") {
    if (destination === "review") {
      return `예외 처리 · 원문 수정 필요 ${scenario.expected.issueCount ?? 0}건`;
    }
    const count =
      scenario.expected.artifactCounts?.[destination] ??
      scenario.expected.itemCount ??
      0;
    return destination === "memo"
      ? "예외 처리 · TXT 원문 메모 보존"
      : `예외 처리 · ${RESULT_LABELS[destination]} ${count}개`;
  }
  if (destination === "review") {
  ...
  const count =
    ...
    scenario.expected.itemCount ??
    0;
  return `${RESULT_LABELS[destination]} ${count}개`;
}

const examples = TEXT_AUTHORING_GRAMMAR_SIMULATION_SCENARIOS.map((scenario) => {
  ...
  return {
    ...
    resultLabel:
      scenario.group === "exception_handling"
        ? "예외 처리"
        : RESULT_LABELS[scenario.naturalDestination],
    expectedResultLabel: expectedResultLabel(scenario),
    title: scenario.options?.title ?? scenario.expected.title ?? scenario.title,
    source:
      sourceUrl ??
      sourceTitle ??
