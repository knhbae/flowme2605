...
function comparisonSection(results: GrammarSimulationResult[]): string {
  ...
  for (const result of results) {
    ...
    groups.set(result.comparisonKey, [
      ...(groups.get(result.comparisonKey) ?? []),
      result,
    ]);
  }
  ...
  if (!evidence) {
    return `
      <div class="empty-evidence">
        이 보고서는 문법 의미 시뮬레이션만 소유합니다. 현재 UI 증거는
        <code>2026-08-11-flowme-text-authoring-exception-coverage-v5-results/browser-qa.json</code>에서 확인하세요.
      </div>`;
  }
...
const loadedUiEvidence = existsSync(UI_EVIDENCE_PATH)
  ? (JSON.parse(readFileSync(UI_EVIDENCE_PATH, "utf8")) as UiSimulationEvidence)
  : undefined;
const uiCatalogCounts = loadedUiEvidence?.checks.flatMap((check) => [
  ...[...check.observed.matchAll(/"qaOptions":(\d+)/gu)].map((match) =>
    Number(match[1]),
  ),
  ...[...check.observed.matchAll(/"total":(\d+)/gu)].map((match) =>
    Number(match[1]),
  ),
]);
// A historical evidence file must never be presented as current catalog proof.
const uiEvidence =
  uiCatalogCounts?.includes(results.length) === true
    ? loadedUiEvidence
    : undefined;
...
const findings = [
  ...
  {
    status: "confirmed",
    title: "지원하는 반복은 회차로 펼치고 조건은 표시만 한다",
    detail:
      "매주·매월 반복은 4주 또는 명시 횟수까지 회차로 펼치며, 실행 조건은 회차 수에 영향을 주지 않고 RRULE도 만들지 않는다.",
  },
  {
    status: "confirmed",
    title: "Flow Markdown과 긴 목록은 source row를 축약하지 않는다",
    detail:
      "K-MOOC 14개와 LibriVox 38개는 root Item과 들여쓴 속성으로, 신차 14개와 이사 27개는 기존 Item 순서로 유지했다.",
  },
  {
    status: "confirmed",
    title: "검토가 끝난 예외도 필요한 입력 제한은 유지한다",
    detail:
      "담당자 속성과 표식 없는 설명문은 설명·TXT로 보존한다. 8월 3일·내일·URL-only도 처리 방침은 확정했지만 원문 수정이나 본문 입력 전까지 결과 제한을 유지한다.",
  },
  {
    status: "confirmed",
    title: "일일 종료일과 같은 날 시간순 일정을 보강했다",
    detail:
      "매일 반복은 명시 종료일까지 네 결과에 같은 회차를 만들고, Calendar는 같은 날짜에서 종일 일정을 먼저 둔 뒤 시간을 오름차순으로 표시한다.",
  },
  {
    status: "decision",
...
const reportData = {
  ...
  goal: "기존 FLOW 콘텐츠와 조건 변경을 현재 문법·산출물·UI에 적용해 의미 보존, 예외 처리, 반복 종료일, 같은 날 시간순 경계를 검증한다.",
  claimBoundary:
...
<body>
  ...
  <div class="page summary-grid" aria-label="시뮬레이션 요약">
    ...
    <article class="summary-card"><span>조건·호환·예외</span><strong>${Number(groupCounts.condition_change) + Number(groupCounts.compatibility) + Number(groupCounts.exception_handling) + Number(groupCounts.review_needed)}</strong><small>검토 완료 정책과 입력 제한 분리</small></article>
  ...
  <main class="page">
    ...
    <section id="scenarios">
      <div class="section-heading">
        <div><h2>${results.length}개 시나리오 자세히 보기</h2></div>
      ...
      <div class="controls">
        <div class="filter-row" role="group" aria-label="시나리오 필터">
          ...
          <button class="filter-button" type="button" data-filter="exception_handling" aria-pressed="false">예외 처리 ${groupCounts.exception_handling}</button>
          ${Number(groupCounts.review_needed) > 0 ? `<button class="filter-button" type="button" data-filter="review_needed" aria-pressed="false">검토 필요 ${groupCounts.review_needed}</button>` : ""}
        </div>
    ...
    <section id="method">
      ...
      <div class="artifact-links">
        ${standalone ? `<a class="artifact-link" href="../2026-08-04-flowme-text-authoring-grammar-ux-improvement-results/flowme-text-authoring-v2-test.html">Standalone 작성기 열기</a>` : ""}
...
writeFileSync(REPORT_PATH, reportHtml.replace(/[ \t]+$/gm, ""), "utf8");
...
- Demo dropdown: existing content ${groupCounts.existing_content}, condition changes ${groupCounts.condition_change}, compatibility ${groupCounts.compatibility}, reviewed exceptions ${groupCounts.exception_handling}, review needed ${groupCounts.review_needed}
${uiTotal > 0 ? `- Route/standalone UI result: ${uiPassed}/${uiTotal} passed` : "- Route/standalone UI result: not captured"}
${
  standalone
    ? `- Standalone bytes: ${standalone.bytes}
- Standalone SHA-256: \`${standalone.sha256}\``
  ...
  JSON.stringify(
    {
      ...
      ui: uiEvidence ? `${uiPassed}/${uiTotal}` : "not-attached",
    },
    null,
    2,
  ),
