...
assert.equal(
  readFileSync('tests/e2e/historical-app/app/icon.svg', 'utf8').replace(/\r\n/g, '\n'),
  readFileSync('app/icon.svg', 'utf8').replace(/\r\n/g, '\n'),
  'historical browser icon must preserve the current product icon content',
);
