...
test("TA keeps every supported syntax available on demand, switches top examples, and reflects edits without a parse click", async ({
  ...
  await expect(simpleResultRows).toHaveCount(3);
  await expect(simpleResultRows.nth(0)).toContainText("첫 번째 항목입니다");
  await expect(simpleResultRows.nth(1)).toContainText("두 번째 항목입니다");
  await expect(simpleResultRows.nth(2)).toContainText("날짜 없는 항목입니다");
  ...
  await expect(page.getByTestId("ta-authoring-repeat-boundary")).toContainText(
    "첫 번째 항목입니다. · 매주 월요일",
  );
  await expect(page.getByTestId("ta-authoring-repeat-boundary")).toContainText(
    "ICS RRULE",
...
test("TA top examples expose the real input and converted result for moving, K-MOOC, and Allblanc", async ({
  ...
  await expect(page.getByRole("button", { name: "표/엑셀 14" })).toBeVisible();
  await expect(page.getByTestId("ta-authoring-preflight")).toHaveAttribute(
    "data-count",
    "14",
  );
...
test("TA product selector exposes five representative examples and four fixed result slots", async ({
  ...
  const resultSlots = page.locator('[data-testid^="ta-authoring-result-slot-"]');
...
test("TA QA-only dropdown exposes all 27 validated examples and applies each behavior group", async ({
  page,
}) => {
  ...
  await expect(exampleSelect.locator("[data-example-scenario-id]")).toHaveCount(27);
  ...
  await expect(page.getByRole("button", { name: "표/엑셀 38" })).toBeVisible();
  await expect(page.getByTestId("ta-authoring-preflight")).toHaveAttribute(
    "data-count",
    "38",
  );
...
test("TA mobile QA dropdown reaches every group and opens the 38-row example without overflow", async ({
  page,
}) => {
  ...
  await expect(exampleSelect).toBeVisible();
  await expect(
    page.getByTestId("ta-authoring-example-category-error_boundary"),
  ).toBeAttached();
  await exampleSelect.selectOption("qa:content-librivox-38");
  await expect(exampleSelect).toHaveValue("qa:content-librivox-38");
  await expect(page.getByTestId("ta-authoring-item")).toHaveCount(38);
...
test("TA dropdown can reapply the same validated example after a structure-only edit", async ({
  ...
  await page.getByTestId("ta-authoring-item-review").getByRole("button", { name: "검토 닫기" }).click();
...
test("TA keeps raw text primary and reveals transformed text formats only on demand", async ({
  page,
}) => {
  await openAuthoring(page, 1440, 900);
  const source = page.getByTestId("ta-authoring-source");
  ...
  await expect(
    page.getByText(
      "이 제작 데모에서는 체크와 할 일이 같은 항목 목록을 씁니다.",
    ),
  ).toHaveCount(0);
  ...
  await expect(textBoundary.getByText("원문 그대로", { exact: true })).toBeVisible();
  const otherFormats = textBoundary.getByText("다른 텍스트 형식", {
    exact: true,
  });
  await expect(otherFormats).toBeVisible();
  await expect(page.getByTestId("ta-authoring-copy-raw-text")).toBeEnabled();
  await expect(page.getByTestId("ta-authoring-copy-structured-text")).toBeHidden();
  await otherFormats.click();
  await expect(page.getByTestId("ta-authoring-copy-structured-text")).toBeEnabled();
  await expect(
    page.getByTestId("ta-authoring-copy-structured-markdown"),
  ).toBeEnabled();
  await expect(page.getByTestId("ta-authoring-memo-boundary")).toHaveCount(0);
  await resultHelp.click();
...
test("TA authoring preview exposes calendar details, todo subfields, sheet columns, and links", async ({
  ...
  await expect(firstRow).toContainText("조건입니다.");
  await expect(firstRow.getByRole("link", { name: /참고 자료/u })).toHaveAttribute(
    "href",
    "https://example.com/resource",
  );

  await page.getByTestId("ta-authoring-result-slot-calendar").click();
  firstRow = page.getByTestId("ta-authoring-artifact-row").first();
  await expect(firstRow.locator('[data-authoring-preview-field="date"]')).toContainText(
    "2026-08-03",
  );
  await expect(firstRow.locator('[data-authoring-preview-field="time"]')).toContainText(
    "09:00",
  );
  await expect(firstRow.locator('[data-authoring-preview-field="timezone"]')).toContainText(
    "Asia/Seoul",
  );
  ...
  await expect(sheetPreview.first().locator("thead")).toContainText("진도 정보");
  await expect(sheetPreview.first().locator("thead")).toContainText("출처");
  await expect(sheetPreview.getByRole("link").first()).toHaveAttribute(
    "href",
    "https://www.kmooc.kr/view/course/detail/20097",
  );
  ...
  await expect(
    page
      .getByTestId("flow-artifact-memo-preview")
      .getByRole("link", { name: /참고 자료/u }),
...
test("TA resets input scroll on example switch and briefly confirms a mobile live parse", async ({
  ...
  await expect(
    page.getByTestId("ta-authoring-stage-result"),
...
test("TA keeps initial-example edits recoverable and saves them into one draft", async ({
  ...
  await inspector.getByRole("button", { name: "변경 적용" }).click();
...
test("TA mobile keeps one active stage and restores focus after closing the Item inspector", async ({
  ...
  const applyButton = inspector.getByRole("button", { name: "변경 적용" });
  ...
  await openItemReview(page);
  const reviewDialog = page.getByTestId("ta-authoring-item-review");
  const structureScroller = reviewDialog.locator("[data-authoring-dialog-scroll]");
...
test("TA restores one corrected Item to the original parser mapping and can undo that restore", async ({
  page,
}) => {
  await openAuthoring(page, 1440, 900);
  await parseJeju(page);
  ...
  let inspector = page.getByTestId("ta-authoring-inspector");
  ...
  await page.getByTestId("public-flow-artifact-preview-row-edit").first().click();
  inspector = page.getByTestId("ta-authoring-inspector");
  await inspector.getByRole("button", { name: "원래 해석으로 복구" }).click();
  await expect(page.getByTestId("ta-authoring-item").first()).toContainText(
    "항공권 확인",
  );
...
test("TA mobile holds, converts, undoes, and keeps one ambiguous sentence without losing the source", async ({
  page,
}) => {
  await openAuthoring(page, 390, 844);
  await page.getByTestId("ta-authoring-title").fill("제주 여행 준비");
  ...
  await expect(issueSection).toHaveCount(0);
  await expect(page.getByTestId("ta-authoring-item")).toHaveCount(2);
  await expect(page.getByTestId("ta-authoring-item-review-close")).toBeFocused();
  await expect(page.getByTestId("ta-authoring-source")).toHaveValue(
    AMBIGUOUS_MEMO,
  );
...
test("TA short viewports can reach the bottom of input, structure, and result", async ({
  ...
  for (const viewport of viewports) {
    await test.step(`${viewport.width}x${viewport.height}`, async () => {
      ...
      await openItemReview(page);
      const reviewDialog = page.getByTestId("ta-authoring-item-review");
      const reviewScroller = reviewDialog.locator("[data-authoring-dialog-scroll]");
      await scrollToEnd(reviewScroller);
      await expect.poll(() => shell.evaluate((element) => element.scrollTop)).toBe(0);
      await expect(page.getByTestId("ta-authoring-item").last()).toBeVisible();
      await expect(page.getByTestId("ta-authoring-item-review-close")).toBeInViewport();
      await page.getByTestId("ta-authoring-item-review-close").click();
...
test("TA keeps personal ownership in the receipt and downloads real XLSX and plain text files", async ({
  ...
  await page.getByRole("button", { name: /^표\/엑셀 5$/u }).click();
  await page.getByTestId("ta-authoring-export-open").click();
  const exportDialog = page.getByTestId("ta-authoring-export-dialog");
  ...
  await expect(exportDialog).toContainText("표/엑셀 · 5개 · Excel(XLSX)");
  await exportDialog.getByText("닫기", { exact: true }).click();

  await page.getByRole("button", { name: /^체크\/할 일 5$/u }).click();
  await page.getByTestId("ta-authoring-export-open").click();
  ...
  expect(text).toContain("제목: 제주 여행 준비");
  expect(text).toContain("항목 1: 항공권 확인");
  expect(text).not.toContain("<!-- flowme:");
  expect(text).not.toContain("- [ ]");
  await expect(exportDialog).toContainText("체크/할 일 · 5개 · TXT");
  await expectNoHorizontalOverflow(page);
});
...
test("TA exports one timed ICS event and blocks background save shortcuts while export is open", async ({
  page,
}) => {
  const timedMarkdown = [
    "# 병원 예약",
  ...
  const [download] = await Promise.all([
    page.waitForEvent("download"),
    exportDialog.getByRole("button", { name: "1개 가져가기 확정" }).click(),
  ]);
  ...
  await expect(exportDialog).toContainText("캘린더 · 1개 · 캘린더 파일(ICS)");
});
...
test("TA-05 correction suggestion keeps a distinct local receipt and shows the supported Markdown boundary", async ({
  ...
  await expect(saveDraftButton(page)).toContainText(
    "수정 제안 저장",
  );
  await saveDraftButton(page).click();
  const receipt = page.getByTestId("ta-authoring-receipt");
...
test("TA previews authored check state and confirms source-order alignment before rewriting input", async ({
  ...
  await page.getByTestId("ta-authoring-result-slot-calendar").click();
  await page.getByRole("button", { name: "입력도 이 순서로 맞추기" }).click();
  ...
  await page.getByTestId("ta-authoring-result-slot-memo").click();
  await page.getByText("다른 텍스트 형식", { exact: true }).click();
  await expect(
...
test("TA confirms split boundaries and blocks a conflicting merge without mutation", async ({
  ...
  await openAuthoring(page, 1024, 768);
  await page.getByTestId("ta-authoring-source").fill([
    "# 구조 확인",
    "## 실행",
    "- [ ] 오전 준비 확인",
    "  - 날짜: 2026-08-01",
  ...
  const mergeDialog = page.getByTestId("ta-authoring-merge-confirm");
  await expect(mergeDialog).toContainText("값이 다른 속성은 임의로 고르지 않습니다");
  await mergeDialog.getByTestId("ta-authoring-merge-apply").click();
  await expect(page.getByTestId("ta-authoring-item")).toHaveCount(2);
  await expect(page.getByTestId("ta-authoring-status")).toContainText(
  ...
  await expect(splitDialog.getByTestId("ta-authoring-split-boundary")).toBeVisible();
  await expect(page.getByTestId("ta-authoring-item")).toHaveCount(2);
...
test("TA-05 stages a saved source update, preserves the old source and user value, and applies only the explicit choice", async ({
  page,
}) => {
  const sourceA = [
    "# 항공권 일정",
  ...
  const updateBanner = page.getByTestId("ta-authoring-source-update-banner");
  await expect(updateBanner).toHaveAttribute(
    "data-source-state",
    "conflict_source_vs_user",
  );
  await expect(updateBanner).toContainText("내 수정 1개");
  await updateBanner.getByTestId("ta-authoring-source-update-later").click();
  await expect(updateBanner).toBeVisible();

  await updateBanner.getByTestId("ta-authoring-source-update-open").click();
  const compareDialog = page.getByTestId("ta-authoring-source-compare-dialog");
  await expect(compareDialog).toBeVisible();
  await expect(
    compareDialog.getByTestId("ta-authoring-source-old-value"),
  ).toContainText("2026-08-01");
  await expect(
    compareDialog.getByTestId("ta-authoring-source-new-value"),
  ).toContainText("2026-08-03");
  await expect(
    compareDialog.getByTestId("ta-authoring-source-user-value"),
  ).toContainText("2026-08-05");

  await compareDialog
    .getByTestId("ta-authoring-source-choice-keep-user")
    .click();
  await expect(compareDialog).toContainText("자료 링크가 달라졌습니다");
  await compareDialog.getByTestId("ta-authoring-source-choice-use-new").click();
  await expect(compareDialog).toContainText("주의가 달라졌습니다");
  await compareDialog.getByTestId("ta-authoring-source-choice-use-new").click();
  await compareDialog.getByTestId("ta-authoring-source-update-apply").click();
