...
import {
  programClone, programFailure, programId, programResult, type ProgramData, type ProgramParticipationDraft,
  type ProgramPost, type ProgramReply, type ProgramTransition,
} from '../../../lib/flow/integrated-poc/contract';
import { validateProgramData } from '../../../lib/flow/integrated-poc/program-data';
import {
  createProgramPost, createProgramReply, deleteProgramPost, deleteProgramReply, editProgramPost, editProgramReply,
  listProgramActivity, programPostEditToken, programReplyEditToken, toggleProgramReaction,
} from '../../../lib/flow/integrated-poc/community';
...
function CommunityBody({ data, mutate, navigate, view, selectedPostId, selectedReplyId, onRegisterEditors, presentation, onPresentationChange }: ProgramCommunityProps) {
  ...
  const currentDraft = () => draftRef.current ? { ...draftRef.current,
    title: titleRef.current?.value ?? draftRef.current.title, body: bodyRef.current?.value ?? draftRef.current.body } : null;
  ...
  function save(next: ProgramParticipationDraft) {
    ...
    queue.current = queue.current.catch(() => false).then(async () => {
      try {
        ...
        const outcome = await mutate('작성 중인 글 저장', current => saveProgramParticipationDraft(current, actorId, next, { expected }), { groupId: next.id, history: false });
        if (outcome.ok) savedDraftRef.current = programClone(next);
        if (mounted.current) { setSaveState(outcome.ok ? '이 기기에 저장됨' : '저장하지 못했어요'); if (!outcome.ok) { setError(programErrorMessage(outcome.reason)); setDraftConflict(outcome.reason === 'conflict'); } else { setDraftConflict(false); setError(''); } }
        return outcome.ok;
  ...
  async function submit() {
    ...
    try {
      ...
      const outcome = await guardedMutation(mutate, snapshot.editTargetId ? '글 수정' : '글 공개', current => submitProgramParticipation(current, actorId, snapshot, utcNow(), { expected: snapshot }));
  ...
  async function perform(label: string, build: Parameters<ProgramMutate>[1]) {
    if (busy || lockCount.current > 0 || composing.current) return; setBusy(true); setError('');
    const outcome = await guardedMutation(mutate, label, build); if (!outcome.ok) setError(programErrorMessage(outcome.reason));
    setBusy(false); return outcome;
  }
  async function remove() {
    if (!confirmDelete) return;
    const target = confirmDelete;
    const outcome = await perform('글 삭제', current => target.kind === 'post' ? deleteProgramPost(current, actorId, target.id, utcNow()) : deleteProgramReply(current, actorId, target.id, utcNow()));
    if (outcome?.ok) setConfirmDelete(null);
  }
  async function discard() {
    const target = draftRef.current; if (!target) return; await queue.current;
    const expected = savedDraftRef.current;
    const outcome = await perform('작성 중인 글 버리기', current => discardProgramParticipationDraft(current, actorId, target.id, { expected }));
    if (outcome?.ok) { draftRef.current = null; savedDraftRef.current = null; setDraft(null); setDiscarding(false); }
  }
  async function attach(files: FileList | null) {
    const snapshot = draftRef.current; if (!files || !snapshot || lockCount.current > 0) return;
    ...
    try {
      const selected = await Promise.all(Array.from(files).map(file => new Promise<ProgramParticipationDraft['media'][number]>((resolve, reject) => {
        ...
        const reader = new FileReader(); reader.onerror = () => reject(new Error('사진을 읽지 못했어요. 다시 선택해 주세요.'));
      ...
      if (draftRef.current?.id === snapshot.id) change({ media: [...draftRef.current.media, ...selected] });
    } catch (reason) { setError(reason instanceof Error ? reason.message : '사진을 읽지 못했어요.'); }
    finally { mediaPending.current = false; setReadingMedia(false); }
  }
  const reactions = (targetKind: 'post' | 'reply', targetId: string) => {
    ...
    return <button type="button" aria-pressed={relevant.some(entry => entry.actorId === actorId)} disabled={busy}
      onClick={() => void perform('도움됐어요 반응', current => toggleProgramReaction(current, actorId, targetKind, targetId))}>도움됐어요{relevant.length ? ` ${relevant.length}` : ''}</button>;
  };
  ...
  return <section className={styles.community} aria-label={view === 'activity' ? '내 활동' : '이야기'}>
    <header className={styles.header}><h1>{view === 'activity' ? '내 활동' : '이야기'}</h1><button className={styles.primary} type="button" disabled={busy} onClick={() => void openDraft(newProgramParticipationDraft())}>글 쓰기</button></header>
    {error && <div role="alert" className={styles.error}>{error}{draft && <button type="button" onClick={() => save(draft)}>초안 저장 다시 시도</button>}</div>}
    {drafts.length > 0 && <details className={styles.drafts}><summary>작성 중인 글 {drafts.length}</summary>{drafts.map(entry => <button key={entry.id} type="button" disabled={busy || readingMedia} onClick={() => void openDraft(entry)}>{entry.title || entry.body.slice(0, 40) || `새 ${kinds[entry.kind]}`} · 이어 쓰기</button>)}</details>}
    {draft && <section className={styles.composer} ref={composeRef} aria-label="글 작성"
      ...
      {draftConflict && <div className={styles.notice} role="alert"><h3>다른 탭의 초안과 비교</h3>
        {latestDraft ? <><p><strong>{latestDraft.title || kinds[latestDraft.kind]}</strong> · {kinds[latestDraft.kind]}</p><p className={styles.body}>{latestDraft.body}</p>
          <details><summary>최신 사진·연결·근거 확인</summary><p>주제: {latestDraft.topic || '없음'}</p>
            ...
            {latestDraft.media.map(media => <figure className={styles.attachment} key={media.id}>{/* eslint-disable-next-line @next/next/no-img-element */}<img src={media.dataUrl} alt={media.alt} /><figcaption>{media.alt}</figcaption></figure>)}
      ...
      {!preview ? <>
        ...
        {draft.kind !== 'reply' && <>
          <label className={styles.fileButton}>{readingMedia ? '사진 읽는 중…' : '사진 추가'}<input type="file" accept="image/png,image/jpeg,image/webp,image/gif" multiple disabled={readingMedia} onChange={event => { void attach(event.target.files); event.currentTarget.value = ''; }} /></label>
          {draft.media.map((media, index) => <div className={styles.attachment} key={media.id}>
            {/* Local data URLs only; model validation enforces MIME and size. */}
            {/* eslint-disable-next-line @next/next/no-img-element */}<img src={media.dataUrl} alt={media.alt} />
            <label>사진 설명<input value={media.alt} maxLength={500} onChange={event => change({ media: draft.media.map(entry => entry.id === media.id ? { ...entry, alt: event.target.value } : entry) })} /></label>
        ...
        {draft.media.map(media => <figure key={media.id}>{/* eslint-disable-next-line @next/next/no-img-element */}<img src={media.dataUrl} alt={media.alt} /><figcaption>{media.alt}</figcaption></figure>)}
        ...
        <p className={styles.notice}>이 내용만 이 기기의 공개 목록에 반영됩니다. 실제 서비스에는 게시되지 않습니다.</p>
      ...
      {conflict && <div className={styles.notice}><h3>최신 내용과 비교</h3>
        ...
        {draft.kind !== 'reply' && <details><summary>최신 사진과 근거 확인</summary>{data.public.posts.find(entry => entry.id === draft.editTargetId)?.media.map(media => <figure key={media.id}>{/* eslint-disable-next-line @next/next/no-img-element */}<img src={media.dataUrl} alt={media.alt} /><figcaption>{media.alt}</figcaption></figure>)}
    ...
    {selectedPostId ? post ? <article className={styles.detail}>
      ...
      <h2>{post.title}</h2>{post.deleted ? <p>작성자가 삭제한 글입니다. 기존 답글은 아래에서 볼 수 있어요.</p> : <><p className={styles.body}>{post.body}</p>
        {post.media.map(media => <figure key={media.id}>{/* eslint-disable-next-line @next/next/no-img-element */}<img src={media.dataUrl} alt={media.alt} /><figcaption>{media.alt}{media.synthetic ? ' · 합성 이미지' : ''}</figcaption></figure>)}
        <div className={styles.actions}>{reactions('post', post.id)}<button type="button" onClick={() => startReply(post)}>답글 쓰기</button>
          {post.authorId === actorId && <details className={styles.menu}><summary>글 관리</summary><button type="button" onClick={() => startPostEdit(post)}>수정</button><button type="button" onClick={() => setConfirmDelete({ kind: 'post', id: post.id })}>삭제</button></details>}</div></>}
      ...
      <section aria-label="답글"><h3>답글</h3>{selectedReplyId && !selectedReply && <p role="status">요청한 답글을 찾을 수 없어요. 이 글의 답글 목록을 표시합니다.</p>}{data.public.replies.filter(reply => reply.postId === post.id).length === 0 && <p className={styles.empty}>아직 답글이 없어요.</p>}
        {data.public.replies.filter(reply => reply.postId === post.id).map(reply => <article className={styles.reply} id={`reply-${reply.id}`} key={reply.id} tabIndex={-1} aria-current={selectedReplyId === reply.id ? 'location' : undefined}>
          ...
          <div className={styles.actions}>{!reply.deleted && !post.deleted && <>{reactions('reply', reply.id)}<button type="button" onClick={() => startReply(post, reply)}>답글</button></>}
            {!reply.deleted && reply.authorId === actorId && <details className={styles.menu}><summary>답글 관리</summary>{!post.deleted && <button type="button" onClick={() => startReply(post, undefined, reply)}>수정</button>}<button type="button" onClick={() => setConfirmDelete({ kind: 'reply', id: reply.id })}>삭제</button></details>}</div>
        </article>)}
      </section>
