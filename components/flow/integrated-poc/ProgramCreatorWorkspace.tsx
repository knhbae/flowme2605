...
import { applyProgramCreatorAction, creatorWorkingFromRecord, fingerprintPersonalWorkspacePocAuthoringSource as fingerprint,
...
export function ProgramCreatorWorkspace(props: ProgramCreatorWorkspaceProps) {
  ...
  function compareSaved(entry:ProgramCreatorSavedRevision) {
    ...
    if(entry.kind==='text-authoring-v1') {try{const read=readNativeCreatorHistory(window.localStorage);if(read.kind!=='ready')throw Error('read');raw=read.raw;}catch{report(false,'기존 저장 이력을 읽지 못했습니다. 보관된 원문과 구조 자료는 아래에서 확인할 수 있습니다.');return;}}
    const result=previewProgramCreatorSavedRestore(dataRef.current,actorId,bufferRef.current!.draftId,entry.id,raw);
  ...
  async function restoreSaved() {
    ...
    try {
      ...
      const result=await mutate('제작 저장본 복구',current=>{
        let raw:string|null=null;
        if(review.entry.kind==='text-authoring-v1'){const read=readNativeCreatorHistory(window.localStorage);if(read.kind!=='ready')return {ok:false,data:current,reason:'conflict'};raw=read.raw;}
        const changed=restoreProgramCreatorSavedRevision(current,{actorId,requestId:request.id,preview:review},raw,now);
      ...
      if(nextWorking){baseline.current=nextWorking;bufferRef.current=nextWorking;setBuffer(nextWorking);structureHistory.current=null;orderHistory.current=null;setOrderPreview(null);setComparison(null);setUpdateReview(null);setNativeItem(null);setMount(n=>n+1);}
  ...
  async function flushDraft(): Promise<boolean> {
    ...
    const task = (async () => {
      try {
        const result = await mutate('제작 중 입력 복구 저장', current => setProgramCreatorWorking(current, { actorId: capturedActor, expectedWorking: expected, working: wanted }, new Date().toISOString()), { history: false });
  ...
  const port = useMemo<ProgramEditorFlush>(() => ({
    ...
    hasPendingInput: () => composing.current || !!editor.current?.readSnapshot()?.composing || !!nativeContextPort.current?.hasPendingInput?.() || !!sourceUpdatePort.current?.hasPendingInput?.() || !!nativeHandoffRef.current || !!nativeLineageRef.current || !programSame(nativeBuffer(), baseline.current),
  ...
  function synchronizeCreatorWorking() {
    ...
    setOrderPreview(null);setHistoryPreview(null);setHistoryEntry(null);setComparison(null);setUpdateReview(null);
    setReplacement(null);setChoice(null);setNativeItem(restoreSelection?.itemId??null);setPropertyLine('');setOrderStep('');
    return true;
  }
  ...
  async function switchWorking(next: ProgramCreatorWorking | null, route = choiceRoute.current ?? captureSelectionRoute('user')) {
    ...
    try {
      ...
      const result = await mutate('제작 원문 열기', current => setProgramCreatorWorking(current, { actorId, expectedWorking: baseline.current, working: next }, new Date().toISOString()), { history: false });
  ...
  async function commit(build: (current: ProgramData, now: string) => PersonalWorkspacePocCreatorDraftAction, label: string) {
    ...
    try {
      ...
      const result = await mutate(label, current => { const transition = applyProgramCreatorAction(current, { actorId, requestId, action: build(current, now),expectedStructure,expectedNativeDocument,expectedNativeSelection }, now);
  ...
  async function acceptNativeLineage(){
    ...
    try{
      if(saving.current&&!await saving.current)return;
      const now=request.now??=new Date().toISOString(),result=await mutate('기존 개인 문서와 제작 항목 연결',current=>applyProgramCreatorNativeLineage(current,{actorId,requestId:request.id,preview,mapping:lineageMappingRef.current},now));
  ...
  async function acceptNativeHandoff() {
    ...
    try {
      ...
      const result=await mutate('제작 설정 개인 실행 연결',current=>{const transition=applyProgramNativeCreatorHandoff(current,{actorId,requestId:request.id,preview:review,choices:nativeHandoffChoices},now);if(transition.ok)changed=transition.changed;return transition;});
  ...
  async function applyNativeOperation(request:ProgramCreatorNativeOperationRequest):Promise<{ok:true}|{ok:false;reason:string}> {
    ...
    try{
      ...
      const result=await mutate('제작 구조 변경',data=>{const changed=applyProgramNativeCreatorOperation(data,{actorId,requestId:request.requestId,draftId:current.draftId,expectedWorking,expectedOwner:request.expectedOwner,operation:request.operation},request.now);
        if(changed.ok)capture.working=changed.data.spaces[actorId].creatorWorkspace!.working;return changed;});
  ...
  async function nativeReplace(raw: string, range?: { start: number; end: number }, templateId?: ProgramCreatorWorking['templateId']) {
    if (pending.current || lockCount.current || !bufferRef.current) return;
    ...
    try {
      ...
      if (!result.ok) { report(false, `원문에 적용하지 않았습니다 (${result.reason}). 입력을 유지했습니다.`); return; }
      ...
      bufferRef.current = next; setBuffer(next); setReplacement(null); report(true, '원문에 적용했습니다. 원문 편집기의 되돌리기를 사용할 수 있습니다.');
    } finally { pending.current = false; setBusy(false); }
  }
  ...
  function reviewUpdate() {
    ...
    setUpdateChoices({});setUpdateReview(preview);
  }
  async function acceptUpdate() {
    if(!updateReview||pending.current||lockCount.current)return;
    pending.current=true;setBusy(true);const release=port.lockInput?.()??(()=>{});
    try{
      if(!await flushDraft())return;
      const result=await mutate('제작 업데이트 선택 수용',current=>applyCreatorUpdate(current,{actorId,preview:updateReview,choices:updateChoices}));
      if(!result.ok){report(false,'개인 문서나 원문이 바뀌었거나 저장하지 못했습니다. 비교와 선택은 유지했습니다. 저장 실패라면 다시 시도하고, 내용이 바뀌었다면 최신 내용으로 다시 비교해 주세요.');return;}
      setUpdateReview(null);report(true,'선택한 제작 내용을 반영했습니다. 개인 날짜·진행·메모와 이전 기록은 보존했습니다. 전체 되돌리기로 취소할 수 있습니다.');
  ...
  async function applyReplacement() {
    ...
    if (!replacement.structure) { await nativeReplace(replacement.raw, undefined, replacement.templateId); return; }
    const snapshot = editor.current?.readSnapshot(); if (!snapshot) return;
  ...
  function propertyApply() {
    ...
    const result = planPersonalWorkspacePocAuthoringPropertyEdit({ intent: 'apply', rawText: snapshot.rawText, expectedSourceFingerprint: snapshot.sourceFingerprint,
      itemSourceLine: Number(propertyLine), key: propertyKey, value: propertyValue, beforeSelection: { start: snapshot.selectionStart, end: snapshot.selectionEnd } });
    if (result.status === 'applied') void nativeReplace(result.nextRawText);
    else report(result.status === 'no-op', result.status === 'blocked' ? `속성을 적용하지 않았습니다 (${result.reason}). 원문을 확인해 주세요.` : '바뀐 내용이 없습니다.');
  }
  async function handoff(confirmed = false) {
    ...
    try {
      ...
      const result = await mutate('제작물 개인 문서 인계', current => handoffProgramCreatorDraft(current, { actorId, requestId, draftId: record.draftId, expectedRecordRevision: record.recordRevision, today }, now));
  ...
  return <section className={styles.workspace} aria-label="제작 작업 공간" aria-busy={busy}
    ...
    {buffer && <>
      <div hidden={tab !== 'input'}>
        ...
        <details><summary>작성 틀·예시 선택</summary><p>직접 선택하고 확인한 내용만 원문에 넣습니다. 구조 템플릿 예시는 빈 제작 원문에서 시작합니다.</p><div className={styles.templates}>{PERSONAL_WORKSPACE_POC_AUTHORING_TEMPLATES.map(template => <div key={template.templateId}><strong>{template.label}</strong><button disabled={blocked} onClick={() => setReplacement({ raw: template.scaffold, label: `${template.label} 빈 틀`, templateId: template.templateId })}>빈 틀 확인</button><button disabled={blocked} onClick={() => showStructure(template.templateId)}>예시 확인</button></div>)}</div></details>
        ...
        {replacement && <section className={styles.notice} aria-label="제작 원문 적용 확인"><h2>{replacement.label}</h2><pre>{replacement.raw}</pre>
          {replacement.structure ? <><p>컴파일러가 확인한 Item {replacement.structure.itemCount}개를 빈 제작 원문에 한 번 만듭니다. 날짜와 내용은 예시이며 적용 뒤 직접 수정할 수 있습니다.</p><details><summary>구조 템플릿 검증 정보</summary><p>StructureDraft {replacement.structure.contractVersion} · 카탈로그 {replacement.structure.catalogVersion}. 원문이 바뀌면 적용하지 않습니다.</p></details></> : <p>적용하면 현재 원문 전체를 바꿉니다. 지금 원문은 적용 전까지 유지됩니다.</p>}
          <button disabled={blocked} onClick={() => void applyReplacement()}>{replacement.structure ? '확인한 구조 예시로 시작' : '확인한 내용으로 원문 바꾸기'}</button><button onClick={() => setReplacement(null)}>취소</button></section>}
        ...
        {preview?.materialized&&<details><summary>항목 속성 편집</summary><div className={styles.properties}><label>대상 항목<select value={propertyLine} onChange={event => setPropertyLine(event.target.value)}><option value="">항목 선택</option>{preview.materialized.parseResult.items.map(item => <option key={item.sourceLine} value={item.sourceLine}>{item.title} · {item.sourceLine}행</option>)}</select></label><label>속성<select value={propertyKey} onChange={event => setPropertyKey(event.target.value)}>{PERSONAL_WORKSPACE_POC_AUTHORING_PROPERTY_CATALOG.map(property => <option key={property.key} value={property.key}>{property.label}</option>)}</select></label><label>값<input value={propertyValue} onChange={event => setPropertyValue(event.target.value)} /></label><button disabled={blocked || !propertyLine} onClick={propertyApply}>원문 속성에 적용</button></div></details>}
      ...
      {updateReview&&<section aria-label="제작 업데이트 선택 비교" className={styles.comparison} onKeyDown={event=>{if(event.key==='Escape'&&!busy){event.preventDefault();setUpdateReview(null);}}}>
        ...
        {updateReview.rows.map(row=>{const choice=updateChoices[row.id]??{source:'keep',date:'keep',time:'keep',children:'keep'};const change=(field:keyof CreatorUpdateChoice,value:'keep'|'incoming')=>setUpdateChoices(v=>({...v,[row.id]:{...choice,[field]:value}}));return <fieldset key={row.id} disabled={busy||locked}><legend>{row.title} · {row.sourceKind}</legend>
        ...
        <button disabled={busy||locked} onClick={()=>void acceptUpdate()}>선택한 변경만 한 번 적용</button><button disabled={busy} onClick={()=>setUpdateReview(null)}>모두 유지하고 비교 닫기</button><button disabled={busy||locked} onClick={reviewUpdate}>최신 내용으로 다시 비교</button>
      </section>}
    </>}
  </section>;
}
