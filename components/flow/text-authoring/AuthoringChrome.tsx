            ...
            <li key={item.key} className="min-w-0">
              ...
              >
                <span className="block text-[11px] font-bold tracking-[0.02em] sm:text-sm">
                  <span className="mr-1 text-[var(--flowme-positive-strong)]">0{item.number}</span>{' '}
                  {item.label}
                </span>
              </button>
            </li>
...
export function AuthoringExampleSwitcher({
  ...
  const selectedValue = activeExampleId
    ? `product:${activeExampleId}`
    : showQaCatalog && activeScenarioId
      ? `qa:${activeScenarioId}`
      : '';
        ...
        <label className="relative min-w-0 flex-1 sm:max-w-sm">
          ...
          <select
            ...
            onChange={(event) => {
              ...
              const selectedExample = mode === 'product'
                ? productExamples.find((example) => example.id === id)
                : mode === 'qa' && showQaCatalog
                  ? validatedExamples.find((example) => example.scenarioId === id)
                  : undefined;
              if (selectedExample) onSelect(selectedExample);
            }}
          >
            <option value="">대표 예시 선택</option>
            <optgroup label={`대표 예시 · ${productExamples.length}개`}>
            ...
            {showQaCatalog
              ? TEXT_AUTHORING_EXAMPLE_GROUPS.map((group) => (
                  <optgroup
                    key={group.id}
                  ...
                  >
                    {validatedExamples
                      ...
                      .map((example) => (
                        ...
                        >
                          {example.label} · {example.expectedResultLabel ?? example.resultLabel}
                        </option>
                      ))}
                  </optgroup>
                ))
          ...
          <span data-testid="ta-authoring-example-count" className="sr-only">
            {showQaCatalog
              ? `QA 전체 ${validatedExamples.length}개`
              : `대표 ${productExamples.length}개`}
          </span>
        </label>
        {showQaCatalog ? (
          <span className="shrink-0 rounded bg-[var(--flowme-warning-soft)] px-2 py-1 text-[10px] font-bold text-[var(--flowme-warning-strong)]">
            내부 QA
          </span>
        ) : null}
      </div>
