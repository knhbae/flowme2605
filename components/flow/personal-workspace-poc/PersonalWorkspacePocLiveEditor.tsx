...
import {
  ...
  type PersonalWorkspacePocLiveEditorLineRole,
} from '@/lib/flow/personal-workspace-poc-editor-presentation';

// Preserve the existing component API while both runtimes share one pure projection.
export { buildPersonalWorkspacePocLiveEditorPresentation };
export type {
  PersonalWorkspacePocLiveEditorLineKind,
  PersonalWorkspacePocLiveEditorLineRole,
  PersonalWorkspacePocLiveEditorLineGuide,
  PersonalWorkspacePocLiveEditorPresentationLine,
} from '@/lib/flow/personal-workspace-poc-editor-presentation';
...
export type PersonalWorkspacePocLiveEditorProps = Readonly<{
  ...
  label?: string;
  disabled?: boolean;
  /** Keep this native host selectable while refusing source mutations. */
  readOnly?: boolean;
  rows?: number;
...
>(function PersonalWorkspacePocLiveEditor({
  ...
  const applyNativeReplacement = useCallback(async (
    ...
    // Read the current DOM flag as well: a caller may retain a handle that was
    // issued while this same textarea was still writable.
    if (!textarea || !current || disabled || readOnly || textarea.readOnly) {
  ...
  }, [disabled, readOnly, readSnapshot, syncSelectionAndScroll]);
