...
module.exports = (function () {
  ...
  function editText(state, docId, text, options) {
    if (!validate(state) || !str(text, MAX_RAW, false)) return state;
    if (options !== undefined && (!options || typeof options !== 'object' || Array.isArray(options) ||
        Object.keys(options).some(function (key) { return key !== 'progressDate'; }) ||
        (Object.prototype.hasOwnProperty.call(options, 'progressDate') && (options.progressDate === null || !validDate(options.progressDate))))) return state;
    var doc = getDocument(state, docId);
    if (!doc) return state;
    var normalized = text.replace(/\r\n?/g, '\n');
    if (normalized === raw(doc)) return state;
    var split = normalized ? normalized.split('\n') : [];
    if (split.length > MAX_LINES) return state;
    ...
    if (lostBinding && lines.some(function (line) { return !previousIds.has(line.id) && /^ *- /.test(line.text); })) return state;
    ...
    if (edited.issues.some(function (issue) { return issue.blocking; })) return state;
    for (var binding of next.bindings.filter(function (entry) { return entry.docId === docId && entry.kind === 'task'; })) {
      ...
      if (!projected || !previous) return state;
      ...
      if (projected.title !== previous.title || projectedDone !== previous.done) {
        ...
        if (existing && (existing.title !== projected.title || existing.done !== projectedDone)) return state;
    ...
    if (next.bindings.some(function (binding) { return binding.kind === 'task' && !afterTasks.has(binding.taskId); })) return state;
    for (var entry of desired) {
      ...
      if (!current || (current.docId === docId && (current.title !== before.title || current.done !== before.done) &&
          (current.title !== requested.title || current.done !== requested.done))) return state;
    ...
    return next ? finalize(state, pinStructuralDates(state, next, docId, true)) : state;
  }
  ...
  return { MAX_DEPTH: MAX_DEPTH, raw: raw, parseDocument: parseDocument, tasks: tasks, editText: editText,
