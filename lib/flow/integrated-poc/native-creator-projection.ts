/** Program-owned projection v2. Derived from the pinned D2 projector.
...
} from "./native-creator-vendor/text-authoring/types";
import {
  adaptTextAuthoringDocumentToFlowBundle,
  type AuthoringAdapterLossManifest,
} from "./native-creator-vendor/text-authoring/flow-bundle-adapter";
...
function itemIssues(
  ...
  // Resolve only the missing-start diagnostic in this read context; retain all
...
function itemValidations(
  ...
  if (!item) return [];
  const issues = itemIssues(document, item, anchor);
...
function recurrenceStartDate(item: UnknownRecord, anchor?: string): string | undefined {
  if (!isRecord(item.schedule)) return undefined;
  return resolveAuthoringScheduleDate(
    (item.schedule.kind === "relative" && anchor
...
function projectRecurrenceRows(
  ...
  const projected = rows.flatMap((row) => {
    ...
    if (recurrenceIsInvalid(document, item, options.anchor)) {
    ...
    const startDate = recurrenceStartDate(item, options.anchor) as string;
...
function makeArtifactRow(
  ...
  const validations = itemValidations(document, item, anchor);
  return {
...
export function buildProgramAuthoringArtifactProjection(
  document: TextAuthoringDocument,
  options: BuildAuthoringArtifactProjectionOptions = {},
): AuthoringArtifactProjection {
  if (options.anchor !== undefined && !isValidAuthoringDate(options.anchor)) {
  ...
  const invalidRecurrenceItemIds = new Set(
    items.flatMap((item, index) => {
      const itemId = stringValue(item.itemId) ?? `authoring-item-${index + 1}`;
      return recurrenceIsInvalid(document, item, options.anchor) ? [itemId] : [];
    }),
  );
  ...
  for (const artifact of Object.keys(
    ...
    let rows = shapeRows[artifact].map((row) =>
      makeArtifactRow(document, row, itemById.get(row.sourceItemId), stepById, options.anchor),
    );
    if (
