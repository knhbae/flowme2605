...
import {createNativeOwnerReplayCache,NATIVE_OWNER_CACHE_MAX_ENTRY_BYTES,NATIVE_OWNER_CACHE_MAX_TOTAL_BYTES} from './native-owner-replay-cache';
import {createTextAuthoringDocument} from './native-creator-vendor/text-authoring/parser';
import {createNativeCreatorDocumentOwner,validateNativeCreatorDocumentOwner,readNativeCreatorDocument} from './native-creator-document';
test('NOR01 exact content, mutable input and returned result isolation',()=>{let calls=0;const cache=createNativeOwnerReplayCache((v:any)=>{calls++;return v.valid?{nested:{text:v.text}}:null});const input={valid:true,text:'A'};const first=cache.read(input)!;first.nested.text='tampered';assert.equal(cache.read({...input})!.nested.text,'A');assert.equal(calls,1);input.valid=false;assert.equal(cache.read(input),null);assert.equal(cache.read(input),null);assert.equal(calls,3);assert.equal(cache.stats().hits,1);});
test('NOR02 descriptor-safe checks precede a hit and never invoke getters',()=>{let invoked=0;const cache=createNativeOwnerReplayCache(()=>({ok:true}));assert(cache.read({a:1}));const getter=Object.defineProperty({},'a',{enumerable:true,get(){invoked++;return 1}});assert.equal(cache.read(getter),null);assert.equal(invoked,0);for(const value of [new Date(),[,,],{a:undefined},{a:NaN},Object.assign({a:1},{[Symbol('x')]:1})])assert.equal(cache.read(value),null);const cycle:any={};cycle.self=cycle;assert.equal(cache.read(cycle),null);});
...
test('NOR06 an applied source owner and its replay result reuse validation within the bounded budget',()=>{
 ...
 const input={valid:true,text:'s'.repeat(168_126)};
 const first=cache.read(input)!;first.text='caller mutation';
 assert.equal(cache.read(structuredClone(input))!.text.length,128_068);assert(cache.read(input));
