...
export const sourceFitAudits: SourceFitAudit[] = [
  ...
  defineAudit({
    ...
    sourceTitle: '초기이유식 식단표 공유 (개인 경험)',
    sourceUrl: 'https://kimstar1021.tistory.com/63',
    sourcePrecision: 'exact',
    sourceUsefulness: '원문은 한 보호자가 사용한 날짜 입력형 식단표와 레시피를 정확히 보여 주므로 개인 경험 자료로는 유용하다. 다만 고정 시작 시점, 3일 간격, 재료 순서와 알레르기 대응을 현재 일반 실행 일정으로 권할 근거는 아니다.',
    idealReconstruction: '현재 공식 지침과 아이별 전문가 판단을 먼저 확인하고, 보호자가 선택한 메뉴와 날짜만 기록하는 개인 식단 달력으로 다시 설계한다. 원문 레시피는 선택 참고 자료로만 남긴다.',
    naturalArtifacts: [
      {
        kind: 'monthly_calendar',
        artifactTitle: '보호자가 확인한 이유식 메뉴 달력',
        simulatedInputs: ['확인한 시작일=2026-08-01', '메뉴=보호자가 고른 메뉴', '보류 재료=달걀', '관찰 메모=전문가에게 확인'],
        expectedOutput: ['확인한 날짜와 메뉴', '사용자가 정한 새 재료', '개별 중단·상담 메모'],
        currentFlowMatch: '현재 Flow는 원문의 11개 메뉴를 3일 간격으로 자동 배치해 개인 경험을 일반 일정처럼 보이게 한다.',
        currentUxSupport: '달력 UI는 재사용할 수 있지만 메뉴와 날짜는 사용자가 확인한 값이어야 한다.',
        gap: '질병관리청 최신 안내는 식품 순서가 엄격히 정해져 있지 않고 아이별 상황에 맞춰야 한다고 설명하므로 고정 33일 실행표를 바로 저장하게 해서는 안 된다.',
      },
      {
        kind: 'spreadsheet',
        artifactTitle: '보호자 식사 관찰 메모',
        simulatedInputs: ['날짜=2026-08-01', '먹인 음식=쌀미음', '보호자 메모=소량 섭취', '확인할 내용=진료 시 질문'],
        expectedOutput: ['사실 기록', '개인 메모', '전문가에게 확인할 내용'],
        currentFlowMatch: '메모 형태는 유지할 수 있지만 증상을 판정하거나 재시도 시점을 안내해서는 안 된다.',
        currentUxSupport: '실행 지시가 아니라 상담에 가져갈 사실 기록으로만 제한해야 한다.',
        gap: '원문의 개인 알레르기 대응 문장을 사용자 행동 규칙으로 재사용하지 않는 경계가 필요하다.',
      },
    ],
    userJourney: [
      '개인 경험 식단표임을 확인한다.',
      '현재 공식 안내와 아이 상태에 맞는 시작 시기를 확인한다.',
      '확인한 메뉴와 날짜만 개인 달력에 기록한다.',
      '변화가 있으면 사실을 메모하고 전문가에게 확인한다.',
    ],
    currentGap: '개인 경험을 33일 고정 실행표로 공개 저장할 수 있어, 현재 공식 안내의 개별 판단 원칙보다 원문 순서가 강하게 작동한다.',
    contentAction: '원문과 기존 저장 기록은 보존하되 신규 공개 실행을 보류한다. 공식 근거를 중심으로 사용자 선택형 기록 모델을 별도 검토한 뒤 재공개 여부를 판단한다.',
    uxAction: '공개 카탈로그와 저장 경로에서 제외하고, 기존 저장본은 My Flow에서 실행 보류 상태와 원문 링크만 제공한다.',
    scores: {
      actionDensity: 11,
      temporalStructure: 12,
  ...
  defineAudit({
    ...
    checkedAt: '2026-07-12',
    sourceTitle: '정부24 동물등록 신청·변경신고 구형 민원 안내',
    sourceUrl: 'https://www.gov.kr/mw/AA020InfoCappView.do?CappBizCD=15410000003&HighCtgCD=A09006',
    sourcePrecision: 'exact',
    sourceUsefulness:
      '민원 페이지는 접속되지만 등록인식표를 등록 방식처럼 남겨 현재 국가동물보호정보시스템의 내장형·외장형 안내와 맞지 않는다.',
    idealReconstruction:
      '현재 국가동물보호정보시스템을 정본으로 삼아 2개월 이상 반려견, 내장형·외장형, 대행기관 방문과 등록번호 보관을 다룬다.',
    naturalArtifacts: [
      {
        kind: 'checklist',
        ...
        currentFlowMatch: '등록 준비 구조는 맞지만 인식표를 등록 방식으로 비교하는 항목은 현재 안내와 맞지 않는다.',
        currentUxSupport: '오래된 방식 안내를 실행하지 않도록 신규 공개 경로를 닫고 최신 공식 Flow로 연결해야 한다.',
        gap: '같은 과업을 최신 공식 페이지 기반 Flow가 이미 더 정확하게 제공한다.',
      },
    ],
    userJourney: [
      '입양 후 등록 대상 여부를 확인한다.',
    ...
    currentGap: '접속 가능한 구형 민원 페이지와 현재 등록 방식 안내가 달라 신규 실행 근거로 쓸 수 없다.',
    contentAction: '원문과 기존 기록은 보존하되 최신 국가동물보호정보시스템 기반 Flow를 대표로 사용한다.',
    uxAction: '구형 public route는 닫고 기존 저장본에는 최신 Flow 이동 경로를 제공한다.',
    decision: 'catalog_preview_only',
    scores: {
      actionDensity: 14,
      temporalStructure: 6,
  ...
  defineAudit({
    ...
    sourcePrecision: 'exact',
    sourceUsefulness:
      '2026 공식 안내가 전원, 리모컨, 실외기 통풍, 필터, 18도 냉방 시험 가동을 순서대로 제공해 계절 전 자가점검에 맞는다.',
    idealReconstruction:
      '계절 시작 전에 전원, 리모컨, 실외기 통풍, 필터, 냉방 시험 가동의 다섯 가지 자가점검을 한 번 수행한다.',
    naturalArtifacts: [
      {
        kind: 'checklist',
        artifactTitle: '여름 전 에어컨 자가점검표',
        simulatedInputs: ['제품=스탠드형', '실외기실=있음', '필터=극세필터'],
        expectedOutput: ['전원/차단기 확인', '리모컨 확인', '실외기 통풍 확보', '필터 청소', '18도 냉방 시험 가동'],
        currentFlowMatch: '현재 Flow 항목이 2026 공식 자가점검 순서와 맞는다.',
        currentUxSupport: '날짜와 회차를 만들지 않고 다섯 항목을 바로 확인할 수 있다.',
        gap: '모델별 필터 분리 방법은 원문과 사용설명서를 계속 확인해야 한다.',
      },
    ],
    userJourney: [
      '2026 공식 사전점검 안내를 보고 모델별 확인 범위를 파악한다.',
      '전원, 리모컨, 실외기, 필터, 냉방 상태를 순서대로 체크한다.',
      '필터는 공식 안내의 2주 확인 주기를 참고해 다음 점검 시점을 정한다.',
      '이상이 있으면 별도의 전문 세척 또는 서비스 경로를 확인한다.',
    ],
    currentGap: '모델별 필터 분리 방법은 Flow 안에 복제하지 않고 원문과 사용설명서를 확인해야 한다.',
    contentAction: '2026 공식 원문의 다섯 가지 자가점검만 유지하고 전문 세척 예약 과업은 별도 Flow로 둔다.',
    uxAction: '날짜와 회차가 없는 즉시 실행 체크리스트로 보여준다.',
    decision: 'keep_representative',
    scores: {
      actionDensity: 13,
      temporalStructure: 5,
  ...
  defineAudit({
    ...
    idealReconstruction: '고정 주기와 날짜를 만들지 않고 필터 LED가 깜빡일 때 공식 순서대로 전원 차단, 분리, 마른 이물 제거, 재조립과 리셋을 확인한다.',
    naturalArtifacts: [
      {
        kind: 'checklist',
        ...
        currentUxSupport: '날짜와 회차 없이 상태 신호부터 리셋까지 한 체크리스트로 실행하고 완료 상태를 저장할 수 있다.',
    ...
    naturalArtifacts: [
      {
        ...
        expectedOutput: ['D-7 검사 기간과 예약 확인', 'D-3 예약 정보/등화/타이어 점검', 'D-Day 검사소 방문', 'D+1 결과표 보관과 정비 메모'],
  ...
  defineAudit({
    ...
    sourcePrecision: 'exact',
    sourceUsefulness: '2025-10-15 출간된 2026년 1급 교재가 필기와 실기, 최신기출, 실기 유형, 온라인 채점 서비스를 제시해 2026 시험용 D-30 역산 학습표와 오답 기록으로 바꾸기 적합하다.',
    ...
    currentGap: '현재 Flow는 D-30 학습 순서를 제공한다. 사용자 화면에서 1급·2026 적용 범위와 2027년 출제기준 재확인 경계를 명확히 유지해야 한다.',
