...
test('content lab exposes natural artifact audit coverage for real-source flows', () => {
  ...
  assert.equal(summary.naturalArtifactRealSourceAuditedCount, 28);
  assert.equal(summary.naturalArtifactRealSourceRemainingCount, summary.realSourceFlowCount - 28);
