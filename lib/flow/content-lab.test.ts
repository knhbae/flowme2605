...
test('content lab exposes source-fit audit summary for representative cleanup', () => {
  ...
  assert.equal(summary.sourceFitAuditedCount, 95);
  assert.ok(summary.sourceFitAverageScore >= 70);
  assert.equal(summary.sourceFitDecisionCounts.keep_representative, 21);
  assert.equal(summary.sourceFitDecisionCounts.reshape_before_featured, 64);
  assert.equal(summary.sourceFitDecisionCounts.catalog_preview_only, 10);
});
...
test('content lab exposes full content inventory coverage', () => {
  ...
  assert.equal(summary.manualSourceFitAuditedCount, 95);
  ...
  assert.equal(summary.sourceNeedsReviewInventoryCount, 37);
...
test('content lab exposes natural artifact audit coverage for real-source flows', () => {
  ...
  assert.equal(summary.naturalArtifactRealSourceAuditedCount, 55);
...
test('content lab exposes lifecycle buckets for keep, fix, preview, and removal review', () => {
  ...
  assert.equal(summary.lifecycleBucketCounts.keep, 21);
  ...
  // Six manually refreshed routes moved from fix to keep across the current-source passes.
  assert.equal(summary.lifecycleBucketCounts.fix, 110 + summary.curatedSourceAppSeedInventoryCount);
...
test('content lab tracks the 2026-06-01 official batch as the next source review queue', () => {
  ...
  assert.equal(summary.sourceReviewPriorityTotalCount, 37);
  assert.equal(countSum, 37);
  assert.equal(summary.sourceReviewPriorityCounts.audit_now, 23);
  ...
  assert.equal(summary.sourceReviewPriorityItems.length, 37);
});
