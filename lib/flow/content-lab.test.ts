...
test('content lab exposes source-fit audit summary for representative cleanup', () => {
  ...
  assert.equal(summary.sourceFitDecisionCounts.reshape_before_featured, 67);
  assert.equal(summary.sourceFitDecisionCounts.catalog_preview_only, 21);
...
test('content lab exposes lifecycle buckets for keep, fix, preview, and removal review', () => {
  ...
  assert.equal(summary.lifecycleBucketCounts.keep, 44);
  ...
  assert.equal(summary.lifecycleBucketCounts.fix, 105);
