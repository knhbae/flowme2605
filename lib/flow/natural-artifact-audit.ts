...
export const realSourceNaturalArtifactAudits: RealSourceNaturalArtifactAudit[] = [
  ...
  },
  {
    ...
    checkedAt: '2026-05-22',
    sourceTitle: 'ThankyouBUBU YouTube 홈트 채널',
    sourceUrl: 'https://www.youtube.com/@ThankyouBUBU',
    sourceEvidence: [
      'seed metadata uses the ThankyouBUBU channel as a broad home-workout source rather than one exact starter video.',
      'search/channel snippets show the channel is centered on home workout videos, but a starter Flow still needs an exact video or playlist source before representative promotion.',
    ],
    ...
    naturalArtifacts: [
      {
        ...
        currentFlowMatch: '현재 Flow는 홈트 시작이라는 방향은 맞지만 채널 전체를 근거로 삼아 사용자가 어떤 영상으로 시작해야 하는지 좁혀주지 않는다.',
        currentUxSupport: 'decision comparison UI는 있지만 broad channel Flow에는 영상 후보 선택표가 자동 생성되지 않는다.',
        gap: '채널 기반 홈트 Flow는 대표 영상 후보를 먼저 고르는 선택표가 필요하다.',
      },
      {
        kind: 'routine_calendar',
        ...
        currentFlowMatch: 'routine calendar로 전환될 여지는 있지만 초보 적응 단계와 컨디션 기록이 구체화되지 않았다.',
        currentUxSupport: '반복 캘린더는 가능하지만 채널에서 고른 영상 후보와 루틴 캘린더가 연결되지 않는다.',
        gap: 'starter Flow는 영상 선택 결과를 바로 반복 캘린더로 바꿔야 한다.',
      },
    ],
    currentContentGap: '채널 전체 출처만으로는 초보자 starter Flow의 항목 순서와 난이도를 충분히 검증할 수 없다.',
    currentUxGap: '영상 후보 비교, 난이도 선택, 반복 캘린더가 하나의 생성 흐름으로 이어지지 않는다.',
    nextContentAction: 'ThankyouBUBU starter Flow는 exact starter video or playlist source로 교체하고 후보 선택표를 작성한다.',
    nextUxAction: 'channel Flow에 video candidate comparison to routine calendar 전환을 추가한다.',
    decision: 'keep_catalog_review',
  },
  {
    ...
    sourceUrl: 'https://www.youtube.com/@ThankyouBUBU',
    sourceEvidence: [
      'seed metadata uses the ThankyouBUBU channel as a broad source for a 20-minute recurring workout routine.',
      'channel/search snippets support the general home-workout category, but the 20-minute routine needs an exact video, playlist, or program source.',
    ],
    ...
    naturalArtifacts: [
      {
        ...
        currentFlowMatch: '반복 루틴이라는 방향은 맞지만 exact 20분 영상의 동작 구성과 휴식 기준을 검증하지 못한다.',
        currentUxSupport: 'routine recurrence는 보이지만 쉬는 날과 주간 완료 요약이 충분히 강조되지 않는다.',
        gap: '20분 루틴은 월간 캘린더에 운동일/휴식일/회차수를 한눈에 보여줘야 한다.',
      },
      {
        kind: 'memo',
    ...
    currentContentGap: '20분 루틴의 동작 구성과 안전 조정 기준이 exact source로 고정되어 있지 않다.',
    currentUxGap: '월간 반복 캘린더, 휴식일, 주간 완료 요약, 공통 조정 메모가 한 화면에서 연결되지 않는다.',
    nextContentAction: '20분 루틴 Flow를 exact 20-minute video or playlist source로 교체하고 주간 반복 기준을 명시한다.',
