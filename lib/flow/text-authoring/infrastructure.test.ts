...
function documentFixture(
  ...
  const sourceRows = Array.from({ length: count }, (_, index) => ({
    ...
    documentId,
    rowType: options.primaryArtifact === 'sheet'
      ? 'table_row' as const
      : 'check' as const,
    rawText: `- [ ] 항목 ${index + 1}`,
    sourceRange: {
  ...
  return {
    ...
    primaryInputKind: options.primaryArtifact === 'sheet' ? 'table' : 'markdown',
    sourceTitle: '테스트 원문',
...
function revision(
  ...
  next.revisionHistory = [...next.revisionHistory, structuredClone(next.revision)];
...
test('draft storage uses only the authoring namespace and round-trips a document', () => {
  ...
  const storage = createMemoryTextAuthoringStorage({ [legacyKey]: legacyValue });
...
test('draft save and recovery preserve held issue decisions as outstanding', () => {
...
test('draft list supports ownership search plus archive and restore', () => {
  ...
  const creator = documentFixture({ count: 5, title: '제주 여행 메모', ownership: 'creator' });
  const personal = documentFixture({ count: 10, title: '차량 점검', ownership: 'personal' });
  repository.save(creator);
  repository.save(personal);

  assert.deepEqual(repository.search('제주').map((entry) => entry.title), ['제주 여행 메모']);
  assert.equal(repository.list({ ownership: 'personal' }).length, 1);
  repository.archive(creator.documentId);
  assert.equal(repository.load(creator.documentId)?.document.lifecycleStatus, 'archived');
  assert.equal(repository.list().some((entry) => entry.draftId === creator.documentId), false);
  assert.equal(repository.list({ includeArchived: true, status: 'archived' }).length, 1);
  repository.restore(creator.documentId);
  assert.equal(repository.load(creator.documentId)?.status, 'draft');
  assert.equal(repository.load(creator.documentId)?.document.lifecycleStatus, 'draft');
});
...
test('duplicate gets separate draft, document, and revision IDs without source mutation', () => {
  ...
  const sourceSnapshot = structuredClone(document.parseResult.canonical.sourceRows);
  ...
  assert.deepEqual(
    duplicated.document.parseResult.canonical.sourceRows.map((row) => row.rawText),
...
test('explicit saves keep version history and can restore an earlier revision', () => {
  ...
  const restored = repository.restoreVersion(first.documentId, history[0].versionId);
...
test('a quota write failure is typed and restores the previous saved value', () => {
  ...
  assert.equal(repository.load(first.documentId)?.document.title, '보존할 저장본');
});
...
test('sheet projections preserve K-MOOC-like 14 and LibriVox-like 38 rows', () => {
  const kmooc = buildAuthoringArtifactProjection(
    documentFixture({ count: 14, title: 'K-MOOC 14주', primaryArtifact: 'sheet' }),
  );
  const librivox = buildAuthoringArtifactProjection(
    documentFixture({ count: 38, title: 'LibriVox 38장', primaryArtifact: 'sheet' }),
  );
...
test('FlowBundle adapter preserves every canonical item ID, order, and source', () => {
  ...
  assert.equal(result.projectionOptions.itemOverrides?.[
...
test('supported Markdown exports and reparses every item with explicit loss fields', () => {
  ...
  assert.match(markdown, /^<!-- flowme:dialect=flowme-supported-markdown-v2 -->/u);
  assert.equal(reparsed.items.length, 5);
  assert.equal(reparsed.items[0].itemId, document.parseResult.canonical.items[0].itemId);
...
test('Markdown round-trip reports nonzero Item nesting when canonical export flattens it', () => {
  const document = createTextAuthoringDocument(
    [
      '# 이전 들여쓰기 입력',
      '## 준비',
  ...
  assert.ok(receipt.lossFields.includes('item.nestingLevel'));
});
...
test('canonical Markdown stays equivalent when the actual input parser reads it again', () => {
  ...
  assert.deepEqual(
    ...
    [
      { label: '원문', url: 'https://example.com/source' },
