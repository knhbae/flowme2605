...
test('v2 unknown property bullets and nested checkboxes remain issues, not Items', () => {
  const raw = [
    '# 경계 확인',
    '## 준비',
    '- [ ] 부모 항목',
  ...
  assert.deepEqual(
    ...
    ['unknown_property', 'unsupported_nested_item'],
  );
  assert.deepEqual(
    document.parseResult.canonical.sourceRows
      .filter((row) => row.rowType === 'unsupported')
      .map((row) => row.rawText),
    ['  - 담당자: 홍길동', '  - [ ] 하위 작업'],
  );
  assert.equal(document.rawText, raw);
