...
test("sync_item_to_working_text rewrites one canonical Item atomically and preserves stable surrounding lineage", () => {
  ...
  assert.deepEqual(
    synced.parseResult.canonical.steps.map((step) => step.stepId),
    initialStepIds,
  );
  assert.equal(synced.parseResult.canonical.items[0].itemId, target.itemId);
  assert.equal(synced.parseResult.canonical.items[1].itemId, unaffected.itemId);
  assert.deepEqual(
    synced.parseResult.canonical.items[1].sourceRowIds,
    unaffectedLineage,
  );
  ...
  assert.deepEqual(
    synced.parseResult.canonical.items[0].subchecks?.map(
      (subcheck) => subcheck.subcheckId,
    ),
    initialSubcheckIds,
  );
  ...
  const reparsed = createTextAuthoringDocument(synced.rawText, {
...
test("sync_item_to_working_text fails closed for ambiguous or unsupported source shapes", () => {
  ...
  cases.forEach(({ label, raw }) => {
    ...
    assert.equal(result, initial, label);
    assert.equal(result.rawText, raw, label);
  });
  ...
  assert.equal(corruptResult, corrupted);
