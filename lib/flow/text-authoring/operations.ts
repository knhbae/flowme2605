...
function normalizedWorkingTextPatch(
  ...
  return {
    ...
    completion: oneLineWorkingText(patch.completion),
    date: oneLineWorkingText(patch.date),
    relativeDate: oneLineWorkingText(patch.relativeDate),
    time: oneLineWorkingText(patch.time),
    timezone: oneLineWorkingText(patch.timezone),
    place: oneLineWorkingText(patch.place),
    duration: oneLineWorkingText(patch.duration),
    repeat: oneLineWorkingText(patch.repeat),
...
function remapRowIds(
  values: string[],
  rowIdMap: ReadonlyMap<string, string>,
): string[] {
  return values.map((value) => rowIdMap.get(value) ?? value);
}

function remapWorkingTextStableIds(
  source: TextAuthoringDocument,
  candidate: TextAuthoringDocument,
  targetItemIndex: number,
  oldStartLine: number,
  oldReplacedLineCount: number,
  newBlockLineCount: number,
): boolean {
  ...
  const keyedTargetRows = (
  ...
  ): Map<string, (typeof oldTargetRows)[number]> | undefined => {
    const keyed = new Map<string, (typeof oldTargetRows)[number]>();
    ...
    for (const row of rows) {
      ...
      if (/^- \[[ xX]\]\s+.+$/u.test(row.rawText)) {
        ...
        const property = canonicalPropertyLine(row.rawText);
        if (!property) return undefined;
        const count = (labelCounts.get(property.label) ?? 0) + 1;
        labelCounts.set(property.label, count);
        key = `property:${property.label}:${count}`;
      }
      if (keyed.has(key)) return undefined;
      keyed.set(key, row);
    }
    return keyed;
  };
  const oldTargetByKey = keyedTargetRows(oldTargetRows);
  const nextTargetByKey = keyedTargetRows(nextTargetRows);
  if (!oldTargetByKey || !nextTargetByKey) return false;

  const rowIdMap = new Map<string, string>();
  nextTargetByKey.forEach((nextRow, key) => {
  ...
  oldParse.canonical.sourceRows
    ...
    .forEach((row) => {
      ...
      values.push(row);
      oldRowsByLine.set(row.sourceRange.startLine, values);
    });
  for (const nextRow of nextParse.canonical.sourceRows) {
    if (rowIdMap.has(nextRow.sourceRowId)) continue;
  ...
  const remapEntity = (value: string) => entityIdMap.get(value) ?? value;

  nextParse.canonical.sourceRows.forEach((row) => {
    row.sourceRowId = rowIdMap.get(row.sourceRowId) ?? row.sourceRowId;
  });
  ...
  canonical.steps.forEach((step, index) => {
    ...
    step.itemIds = step.itemIds.map(remapEntity);
    step.sourceRowIds = remapRowIds(step.sourceRowIds, rowIdMap);
    const oldStep = oldSteps[index];
    step.order = oldStep.order;
  });
  canonical.items.forEach((item, index) => {
    item.itemId = remapEntity(item.itemId);
    item.stepId = remapEntity(item.stepId);
    item.sourceRowIds = remapRowIds(item.sourceRowIds, rowIdMap);
    item.properties.forEach((property) => {
      property.sourceRowIds = remapRowIds(property.sourceRowIds, rowIdMap);
    });
    ...
    item.role = oldItem.role;
    item.included = oldItem.included;
    item.nestingLevel = oldItem.nestingLevel;
    item.order = oldItem.order;
  });
  canonical.fields.forEach((field) => {
    field.owner.id = remapEntity(field.owner.id);
    field.sourceRowIds = remapRowIds(field.sourceRowIds, rowIdMap);
  });
  const memoIdMap = new Map<string, string>();
  canonical.memos.forEach((memo) => {
    const previousMemoId = memo.memoId;
  ...
  candidate.reviewGates?.forEach((gate) => {
    gate.sourceRowIds = remapRowIds(gate.sourceRowIds, rowIdMap);
  });

  const tombstoneSnapshotId = source.sourceState?.active.snapshotId;
  if (removedTargetRows.length > 0 && !tombstoneSnapshotId) return false;
...
/**
 * Re-parses the left editor as the current working source without pretending
 * that the user imported a new external source. The first captured source
 * snapshot remains available for comparison/copy, while the working parse and
 * every projection move to the editor value in one undoable revision.
 */
function syncWorkingTextFromInput(
  document: TextAuthoringDocument,
  ...
  const candidate = createTextAuthoringDocument(operation.rawText, {
    ...
    ...(sourceUrl ? { sourceUrl } : {}),
    reviewRequirements: cloneAuthoringValue(operation.reviewRequirements ?? []),
    now: document.createdAt,
  });

  // Preserve stable lineage for the common case where one Item block is edited
  // in place. Larger structural rewrites still use the fresh, internally
  // consistent parser identity from the candidate document.
  const oldLines = splitSourceLinesWithEndings(document.rawText).map(
    (line) => line.content,
  );
  const nextLines = splitSourceLinesWithEndings(operation.rawText).map(
    (line) => line.content,
  );
  let sharedPrefix = 0;
  while (
  ...
  const affectedItems = oldItems.filter((item) => {
    ...
    return oldChangedLineCount === 0
      ...
      : changedStartLine <= itemEndLine && changedEndLine >= itemStartLine;
  });
  if (affectedItems.length === 1) {
    const targetItemIndex = oldItems.findIndex(
      (item) => item.itemId === affectedItems[0].itemId,
    );
  ...
  const activeSnapshot = cloneAuthoringValue(
    document.sourceState?.active ?? candidate.sourceState?.active,
  );
  if (!activeSnapshot) return false;

  document.title = candidate.title;
  document.rawText = candidate.rawText;
  document.inputKinds = cloneAuthoringValue(candidate.inputKinds);
  document.primaryInputKind = candidate.primaryInputKind;
  document.parseResult = cloneAuthoringValue(candidate.parseResult);
  if (candidate.sourceTitle) document.sourceTitle = candidate.sourceTitle;
  ...
  document.reviewGates = (candidate.reviewGates ?? []).map((gate) => ({
    ...cloneAuthoringValue(gate),
    sourceSnapshotId: activeSnapshot.snapshotId,
  }));
  document.lifecycleStatus = candidate.lifecycleStatus;
  return true;
}

/**
 * Rewrites only a root canonical Markdown checkbox Item. Unsupported source
 * shapes return false before mutating the cloned document, so the public
 * operation returns the original document unchanged and never partially syncs.
 */
function syncItemToWorkingText(
  document: TextAuthoringDocument,
  ...
  const rootRow = document.parseResult.canonical.sourceRows.find(
    (row) => row.sourceRowId === itemMapping[0].sourceLineage[0],
  );
  const rootMatch = rootRow
  ...
  const oldLines = splitSourceLinesWithEndings(document.rawText);
  const rootLineIndex = rootRow.sourceRange.startLine - 1;
  if (oldLines[rootLineIndex]?.content !== rootRow.rawText) return false;
  let boundary = rootLineIndex + 1;
  while (boundary < oldLines.length) {
    const value = oldLines[boundary].content;
    if (/^(?:#{1,6}\s+|- \[[ xX]\]\s+)/u.test(value)) break;
    boundary += 1;
  }
  ...
  for (let index = rootLineIndex + 1; index < contentEnd; index += 1) {
    ...
    const subcheck = canonicalSubcheckLine(rawText);
    if (subcheck) {
      existingSubchecks.push({ ...subcheck, rawText, line: index + 1 });
      continue;
    }
    const property = canonicalPropertyLine(rawText);
    if (!property) return false;
    existingProperties.push({ ...property, rawText, line: index + 1 });
  }
  const editableCounts = new Map<string, number>();
  existingProperties.forEach((property) => {
    if (!WORKING_TEXT_EDITABLE_LABELS.has(property.label)) return;
    editableCounts.set(
      property.label,
      (editableCounts.get(property.label) ?? 0) + 1,
    );
  });
  ...
  if (
    ...
    !remapWorkingTextStableIds(
      ...
      targetItemIndex,
      rootLineIndex + 1,
      oldReplacedLineCount,
      nextBlockLines.length,
    )
  ) {
    return false;
  }

  document.rawText = nextRawText;
  document.inputKinds = candidate.inputKinds;
  document.primaryInputKind = candidate.primaryInputKind;
  document.parseResult = candidate.parseResult;
  return true;
}
 ...
 * `align_source_order`, `sync_item_to_working_text`, and
 * `sync_working_text_from_input` operations. Those operations update the
 * working text while retaining the separately captured source snapshot, and
 * are restored by the normal undo path.
 */
