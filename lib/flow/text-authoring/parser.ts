...
import {
  isCanonicalAuthoringItemPropertyLabel,
...
function addItem(
  ...
  const sources: AuthoringLink[] = state.options.sourceUrl
    ? [{
        label: state.options.sourceTitle ?? '원문',
        url: state.options.sourceUrl,
        type: 'official',
        sourceRowIds: [unit.row.sourceRowId],
      }]
    : [];
...
function linkFromProperty(
  ...
  const url = extractUrls(property.value)[0];
...
function applyScheduleProperty(
  ...
  if (property.key === 'date') {
    ...
    if (schedule?.kind === 'absolute') {
      ...
      addInvalidDateIssue(state, unit);
    }
    return;
  }
...
function applyProperty(
  ...
  if (property.targetKind === 'detail') {
    ...
    const link = linkFromProperty(property, unit.row.sourceRowId);
    if (link) item.resources.push(link);
    ...
    const link = linkFromProperty(property, unit.row.sourceRowId);
    if (link) item.sources.push(link);
...
function tableActionColumn(headers: string[], cells: string[]): number {
  const actionIndex = headers.findIndex((header) => (
    /(활동|할\s*일|action|task|item|실행)/iu.test(header)
  ));
  if (actionIndex >= 0 && cells[actionIndex]) return actionIndex;
    ...
    if (indentedBullet && indentationWidth(indentedBullet[1]) >= 2) {
      ...
      if (/^\[[ xX]\]\s+/u.test(bulletValue)) {
        ...
        if (
          property
          && property.key !== 'anchor'
          && isCanonicalAuthoringItemPropertyLabel(property.label)
        ) {
          if (state.currentItem) {
            applyProperty(
              state,
              line,
              property,
              Math.floor(indentationWidth(indentedBullet[1]) / 2),
            );
          } else {
            addUnsupportedLine(
              state,
