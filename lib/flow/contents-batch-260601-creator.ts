...
- 오늘 만들 레시피 영상 한 편 고르기
  ...
  how: 재생목록에서 조회수 상위 영상(초간단 김치찌개, 쫄면, 만능양파햄 샌드위치 등)을 보고, 냉장고 재료·오늘 시간·조리 난이도에 맞는 영상 1편을 선택합니다.
...
## 1. 이번 주 레시피 고르기
- 이번 주 메뉴 3~5개 후보 정하기
  why: 오늘의집 5만원 식단 가이드처럼 메뉴를 먼저 정해야 중복 재료를 합칠 수 있어 식재료 낭비와 장보기 비용을 줄일 수 있습니다.
...
- 카테고리별로 목록 정렬하기(채소·단백질·냉동·양념)
  how: 마트 동선에 맞게 분류하면 장보는 시간이 줄어듭니다. 일요일 저녁 또는 월요일 오전에 점검+장보기를 묶으면 루틴이 정착됩니다.
...
- 이번 주 식단 시트 최종 확인하기
  ...
  // ──────────────────────────── 정리 / 수납 ────────────────────────────
  {
    ...
    flow: {
      ...
      description: '오늘의집 "깔끔한 옷장을 유지하기 위한 9가지 정리 방법" 가이드를 바탕으로 옷장을 비우고, 분류하고, 되돌려 넣는 하루 정리를 실행합니다.',
      category: '정리/수납',
      structure_type: 'checklist',
      anchor_type: 'none',
      status: 'published',
      risk_level: 'low',
      primary_destination: 'memo',
      source_title: '오늘의집 – 깔끔한 옷장을 유지하기 위한 9가지 정리 방법',
      source_url: 'https://ohou.se/advices/4269',
      conversion_note: '오늘의집 옷장 정리 9가지 가이드(ohou.se/advices/4269)를 옷장 비우기→분류→정리→기부/처분 하루 실행 체크리스트로 전환했습니다.',
...
- 옷장 전체 꺼내 바닥에 펼치기
  ...
  link: 오늘의집 옷장 정리 가이드 | https://ohou.se/advices/4269 | reference
- 1년 이상 안 입은 옷 분류하기
  why: 자주 안 입는 옷이 정리된 옷장을 다시 막는 주된 원인입니다. 늘어나거나 망가진 옷은 1순위로 비웁니다.
  how: 지난 1년을 떠올리며 입었던 옷과 안 입었던 옷으로 두 묶음으로 나눕니다.
  done: 기부/처분 후보를 별도로 분리했다.

## 2. 분류하기
- 계절·종류별로 묶어 쌓기
  how: 겨울/여름, 상의/하의/아우터/원피스로 묶고, 색상과 길이에 맞춰 정리하면 한눈에 파악됩니다.
  done: 카테고리별 묶음이 정해졌다.
- 자주 입는 옷 위치 우선순위 정하기
  why: 꺼내기 쉬운 위치에 자주 입는 옷이 있어야 정리된 상태가 유지됩니다. 새 옷 1개 들어오면 안 입는 옷 1개를 반드시 비우는 규칙을 정합니다.
...
- 냉장고 유통기한 지난 식품 버리기
  ...
  how: 냉장·냉동칸을 순서대로 꺼내 유통기한을 확인하고 폐기합니다. 이후 비닐 대신 밀폐용기로 교체합니다.
  done: 유통기한 지난 식품을 모두 버렸다.
  link: 오늘의집 냉장고 정리 가이드 | https://ohou.se/advices/8631 | reference
- 남은 식재료 종류별로 정리하기
  how: 가벼운 것은 위로, 무거운 것은 아래에 수납합니다. 자주 먹는 반찬은 손 닿기 쉬운 앞칸에 배치합니다.
...
- 정리 전후 사진 찍어 기록하기
  ...
  // ──────────────────────────── 재테크 / 재무 ────────────────────────────
  {
    ...
    flow: {
      ...
      description: '슈카월드 유튜브 채널의 가계부·비용 재생목록을 참고해 지출 카테고리를 잡고 첫 달 가계부 기록 루틴을 만듭니다.',
      category: '재테크/재무',
      structure_type: 'routine',
      anchor_type: 'start_date',
      status: 'published',
      ...
      primary_destination: 'sheet',
      source_title: '슈카월드 – 가계부(월간) 재생목록',
      source_url: 'https://www.youtube.com/playlist?list=PLWUTlSrfRMlHLT38s6jQu5svoYSxUwfgT',
      conversion_note: '슈카월드 가계부 재생목록을 개인 지출 기록 루틴으로 전환했습니다. 투자 조언이 아닌 기록 출발점입니다.',
...
- 내 지출 유형에 맞는 카테고리 4~6개 정하기
  why: 카테고리가 너무 많으면 기록이 번거롭고, 너무 적으면 원인 파악이 어렵습니다. 슈카월드는 가계부를 "두 번 쓰는" 방식을 권장합니다.
  how: 고정지출(월세·보험·구독), 변동지출(식비·교통·쇼핑), 저축·투자를 기본으로 삼아 내 상황에 맞게 조정합니다. 50-30-20 법칙(고정비 50%, 유연지출 30%, 재정목표 20%)을 참고합니다.
  done: 카테고리 목록을 시트 첫 줄에 적었다.
  link: 슈카월드 가계부 재생목록 | https://www.youtube.com/playlist?list=PLWUTlSrfRMlHLT38s6jQu5svoYSxUwfgT | creator
...
- 다음 달 조정 사항 1개만 정하기
  ...
  },
  {
    ...
    flow: {
      ...
      risk_level: 'financial_sensitive',
      primary_destination: 'sheet',
      source_title: '토스피드 – 통장쪼개기, 많이 쪼갤수록 돈을 빨리 모을 수 있을까?',
      source_url: 'https://toss.im/tossfeed/article/howtomanage-salary',
      conversion_note: '토스피드 통장 쪼개기 가이드(선 이체, 후 소비 원칙)를 월급날 루틴 체크리스트로 전환했습니다.',
...
- 월급 입금 확인하고 실수령액 기록하기
  ...
  link: 토스피드 통장 쪼개기 가이드 | https://toss.im/tossfeed/article/howtomanage-salary | reference
...
- 저축·비상금 계좌로 목표 금액 이체하기
  ...
  how: 예금, 적금, ISA, 연금저축 등 목표 금액을 자동이체 또는 즉시 이체합니다. 비상금은 월급의 5~10%를 별도 통장으로 보냅니다.
  done: 저축 금액을 이체했다.

## 3. 생활비 분리
- 이번 달 생활비 한도를 별도 계좌나 봉투로 분리하기
  why: 생활비와 전체 잔액이 섞이면 실제로 쓸 수 있는 금액이 얼마인지 파악하기 어렵습니다. 통장 쪼개기는 급여통장·생활비통장·투자통장·비상금통장 4개가 기본입니다.
...
- 주간 읽은 페이지 합산하고 다음 주 목표 정하기
  how: 이번 주 페이지 합계를 보고 부담 없이 달성 가능한 다음 주 목표를 정합니다. 브런치 30일 챌린지 참가자들은 "매일 한다는 관성"이 핵심이었다고 평가했습니다.
...
- 한 달 완료 후 다음 책 후보 2권 미리 정하기
  ...
  },
  {
    ...
    flow: {
      ...
      description: '브런치스토리 "@libraryman – 책 읽는 습관을 만드는 방법"을 참고해 한 권을 시작부터 완독까지 끝내는 실행 계획을 만듭니다.',
      category: '자기계발/독서',
      structure_type: 'checklist',
      anchor_type: 'none',
      status: 'published',
      risk_level: 'low',
      primary_destination: 'memo',
      source_title: '브런치스토리 @libraryman – 책 읽는 습관을 만드는 방법',
      source_url: 'https://brunch.co.kr/@libraryman/164',
      conversion_note: '브런치 완독 습관 가이드(brunch.co.kr/@libraryman/164)를 책 한 권 준비·실행·완료 체크리스트로 전환했습니다.',
      tags: ['독서', '완독', '자기계발', 'reference'],
    },
    text: `## 1. 시작 준비
- 읽을 책과 목표 완독일 정하기
  why: 완독일을 정해야 하루 분량을 역산할 수 있습니다. 브런치 가이드는 "책 읽을 시간을 먼저 확보하는 것"을 출발점으로 강조합니다.
  how: 책 총 페이지를 목표 일수로 나눠 하루 최소 페이지를 계산합니다. 예: 하루 100페이지 목표가 집중력을 끌어올린다는 팁도 있습니다.
  done: 책 제목과 완독 목표일, 하루 분량을 메모에 적었다.
  link: 브런치스토리 책 읽는 습관 만들기 | https://brunch.co.kr/@libraryman/164 | reference
- 읽기 전 목차와 서문 훑기
...
- 다음 주 유지 또는 조정할 루틴 1가지 정하기
  ...
  // ──────────────────────────── 여행 ────────────────────────────
  {
    ...
    flow: {
      ...
      risk_level: 'low',
      primary_destination: 'memo',
      source_title: '브런치스토리 @fruitful0522 – 국내여행 체크리스트',
      source_url: 'https://brunch.co.kr/@fruitful0522/24',
      conversion_note: '브런치 국내여행 체크리스트(brunch.co.kr/@fruitful0522/24)를 D-7부터 출발 전까지 타임라인으로 전환했습니다.',
...
- 숙소·교통(기차·버스·항공) 예약 내역 재확인하기 D-7
  ...
  link: 브런치 국내여행 체크리스트 | https://brunch.co.kr/@fruitful0522/24 | reference
...
- 캐리어 짐 배치 확인(무거운 것 아래, 자주 꺼낼 것 위)
  how: 무겁고 변형이 적은 물건은 캐리어 아래쪽에, 충전기·우산 등 자주 꺼내는 것은 위쪽에 배치합니다. 10일 이상 장기 여행은 압축가방 활용을 권장합니다.
...
- 집 출발 전 잠금·가스·전기 확인 목록 정하기
  ...
  // ──────────────────────────── 홈카페 / 취미 ────────────────────────────
  {
    ...
    flow: {
      ...
      risk_level: 'low',
      primary_destination: 'calendar',
      source_title: '홈카페스페이스 – 초보자를 위한 실용 가이드: 홈카페 입문서',
      source_url: 'https://homecafe.space/hands-on-home-cafe-guide-for-beginners/',
      conversion_note: '홈카페 입문 가이드(기본 장비·메뉴·루틴)를 일상 루틴으로 전환했습니다.',
...
- 오늘 만들 음료 1가지 정하기
  ...
  link: 홈카페 입문 가이드 | https://homecafe.space/hands-on-home-cafe-guide-for-beginners/ | reference
- 재료와 도구 준비하기(원두·우유·시럽·잔)
  how: 아이스 커피 기본 레시피: 잔에 얼음 → 연유 → 우유 → 에스프레소 1샷 순서로 붓고 저어 마십니다.
  done: 재료와 도구를 꺼냈다.
- 레시피 순서대로 음료 만들기
...
- 한 줄 평가 남기기(달기·쓴기·다음에 바꿀 점)
  ...
  },
  {
    ...
    flow: {
      ...
      conversion_note: '클래스101 취미 클래스(4000개 이상)와 30일 챌린지 성공 연구(완주율 23%, 체크리스트 그룹 3.2배 성공)를 30일 실행 계획으로 전환했습니다.',
      tags: ['취미', '자기계발', '클래스', 'reference'],
    },
    text: `## D-1 시작 준비
- 배울 취미와 강의 또는 자료 결정하기 D-1
  why: 막연하게 시작하면 첫 주에 방향을 잃기 쉽습니다. 30일 챌린지 완주율은 23%에 불과하며 실패 원인의 78%는 중간 점검 부재입니다.
  how: 클래스101 등에서 커리큘럼과 난이도를 보고 하나를 선택합니다. '최소 실행 기준'(예: 하루 10분 연습)을 먼저 정하고 완벽함 대신 무조건 실행을 원칙으로 삼습니다.
...
## 2~4주차: 반복과 심화(11~30일)
- 주 2~3회 정해진 시간에 연습하기
  how: 체계적 체크리스트를 사용한 그룹은 미사용 그룹보다 완주율이 3.2배 높았습니다. 2주차부터는 시간이나 강도를 20~30% 늘려 강화합니다.
...
## 아침 루틴 (30분)
- 알람 즉시 일어나기(스누즈 없애기)
  why: 스누즈를 누르면 수면 관성이 더 강해져 오히려 일어나기 더 어려워집니다. 30일 챌린지는 매일 한다는 것이 관성을 만들어주며, 작은 행동도 30일 반복하면 자연스럽게 습관으로 정착합니다.
  done: 오늘 알람에 스누즈 없이 일어났다.
  link: 브런치 모닝루틴 가이드 | https://brunch.co.kr/@pletalk/58 | reference
- 물 한 컵 마시기
  done: 기상 후 물을 마셨다.
- 오늘 할 일 TOP 3 메모하기(5분)
  why: 아침에 우선순위를 정해야 하루 중 중요한 일에 집중할 수 있습니다. 스타벅스 CEO처럼 "급하게 업무를 시작하지 않고 좋아하는 것으로 마음을 채운 후 엔진을 가동"하는 방식입니다.
...
## 매일 실천
- 취침 1시간 전 스마트폰 알림 끄기
  why: 자기 전 화면 노출은 수면 질을 낮춥니다. 연구에 따르면 2주간 디지털 디톡스를 실천한 사람은 항우울제에 준하는 행복감 개선 효과를 보였습니다.
...
- SNS 사용 시간 하루 총 X분 이하로 목표 설정하기
  how: "하루 종일 스마트폰 금지"보다 점진적으로 줄이는 것이 효과적입니다. "아침 8~10시 금지", "저녁 9시 이후 SNS 금지" 같은 구체적인 규칙을 정합니다. 스마트폰 스크린타임 기능에서 앱별 하루 사용 제한을 설정합니다.
...
- 강아지 상태(발바닥·다리·기분) 간단히 확인하기
  why: 산책 전 상태를 확인하면 이상 징후를 조기에 발견할 수 있습니다. 적정 산책 온도는 15~25°C이며 폭염·한파 시 산책 시간을 조절합니다.
  done: 강아지 상태를 확인했다.
