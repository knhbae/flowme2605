import { cloneAuthoringValue } from "./runtime-adapter";
import type {
  PersonalWorkspacePocStructureTemplateStorageAdapter,
...
export function createStructureTemplateSidecarRepository(
  storage: PersonalWorkspacePocStructureTemplateStorageAdapter,
) {
  const key = STRUCTURE_TEMPLATE_SIDECAR_STORAGE_KEY;
