...
import { createMemoryStructureTemplateStorage } from "./runtime-adapter";
import {
...
test("an empty raw source restores its exact sidecar draft", () => {
  const storage = createMemoryStructureTemplateStorage();
...
test("latest empty-source recovery excludes materialized drafts", () => {
  ...
  const materializedOnlyStorage = createMemoryStructureTemplateStorage();
...
test("draft IDs isolate equal empty-source fingerprints", () => {
  const repository = createStructureTemplateSidecarRepository(
    createMemoryStructureTemplateStorage(),
  );
...
test("recursive reducer replay is deterministic and duplicate dismiss or absent restore is a no-op", () => {
  const initial = draftFixture("draft-recursive-no-op", "exercise-phased-4w-v1");
  const nestedScopeId = initial.groups[0].children[0].instanceId;
  const actions = [
    {
      type: "set_value" as const,
      scopeInstanceId: nestedScopeId,
      slotId: "item_title",
      value: "재귀 실행 항목",
    },
    {
      type: "dismiss_slot" as const,
      scopeInstanceId: nestedScopeId,
      slotId: "detail",
    },
  ];
  const run = () => actions.reduce(
    (draft, action, index) => reduceStructureDraft(
      draft,
      action,
      `2026-08-29T12:0${index + 1}:00.000Z`,
    ),
    initial,
  );
  const first = run();
  ...
  const duplicateDismiss = reduceStructureDraft(first, actions[1], T0);
  assert.equal(duplicateDismiss, first);
  const absentRestore = reduceStructureDraft(first, {
    type: "restore_slot",
    scopeInstanceId: nestedScopeId,
    slotId: "never-dismissed",
  }, T0);
  assert.equal(absentRestore, first);
  assert.equal(duplicateDismiss.revision, first.revision);
  assert.equal(absentRestore.revision, first.revision);
});

test("sidecar accesses only the fixed PoC prefix and never clears storage", () => {
  const values = new Map<string, string>();
  const calls: Array<Readonly<{ method: string; key?: string }>> = [];
  const storage = {
    getItem(key: string) {
  ...
  const repository = createStructureTemplateSidecarRepository(storage);
  const draft = draftFixture("draft-fixed-poc-prefix");
  ...
  const keyedCalls = calls.filter((call) => call.key !== undefined);
  assert.ok(keyedCalls.length > 0);
  assert.equal(
    keyedCalls.every((call) => call.key === STRUCTURE_TEMPLATE_SIDECAR_STORAGE_KEY),
    true,
  );
  assert.equal(
    keyedCalls.every((call) => call.key!.startsWith("flow:poc:personal-workspace:v1:")),
    true,
  );
  assert.equal(calls.filter((call) => call.method === "clear").length, 0);
});
