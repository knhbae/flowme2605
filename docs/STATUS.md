...
**Status:** v0.1.0 RELEASED / R3B PRODUCTION BASELINE / PUBLIC PLAN SURFACE LOCAL QA PASS / PUBLICATION AUTHORIZED / PR PREPARATION / OBSERVED USERS 0
**Current Version:** v0.1.0  
**Primary Focus:** Prepare an intentional scoped commit and pull request for the locally verified public plan surface candidate. Merge, deployment, and production smoke remain pending until each real external state is verified.
...
| Lane | Current truth |
| --- | --- |
| Active product gate | [Public Plan Surface Unification](./specs/2026-08-12-public-plan-surface-unification/spec.md) is locally implemented, has passed the authorized automated QA, and is authorized for publication. The next checkpoint is a scoped candidate commit and pull request; existing save modes, child identities, storage keys, and transactions remain authoritative. |
| Local candidate identity | Base `origin/main` is `8f72ad6922ffa20a765a45cb9b5312ecfa8ca46f`; candidate commit and PR head remain `PENDING` until created. The final verified local production build reports BUILD_ID `bzdhR-nY_afpTfx_xDZ7e`. |
...
| User action now | Publication authorization is complete. No further Owner decision is required before the scoped commit and PR preparation step. |
| AI action now | Create an intentional scoped commit, push the branch, and open the PR in the authorized publication workflow. Record the real commit, PR, CI, merge, deployment, and smoke states only after each exists; keep unrelated dirty worktrees untouched. |
...
| Public plan surface local candidate | [QA evidence](./specs/2026-08-12-public-plan-surface-unification/qa.md) | `npm test` PASS: pretest `173/173`, main `622/622`, approved `182/182`, public-surface `8/8`; build PASS `18/18`; new E2E `9/9` across `9` executable routes and four widths; Map rollback/copy contracts `24/24`; final current-source Playwright `578/578` across `74` files with workers `2`, retries `0`, and `0` unexpected/skipped/flaky. Publication is authorized; commit, push, PR, CI, merge, deployment, and production smoke remain `PENDING`; observed users remain `0`. |
| Worktree baseline | `git worktree list` | `flow-public-surface-unification-20260812` is on `codex/public-surface-unification-20260812` from base `8f72ad6922ffa20a765a45cb9b5312ecfa8ca46f`; candidate commit and PR head remain `PENDING`. Generated Playwright output remains evidence, not publication. The dirty `flow-mvp` checkout remains user-owned and untouched. |
