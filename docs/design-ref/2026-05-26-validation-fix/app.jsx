...
function App() {
  return (
    <DesignCanvas>
      ...
      <DCSection id="cs-d30-fix" title="1 · Validation Fix · computer-skills-d30-study" subtitle="testable_content.md의 첫 observed-session 후보. A1(모바일 CTA) + A2(anchor 라벨) + A4(D-N 그룹핑) 동시 적용.">
        <DCArtboard id="cs-d30-rationale" label="변경 근거 + 다른 라우트 복제 가이드" width={920} height={1180}>
          <CSRationaleDoc/>
        </DCArtboard>
        <DCArtboard id="cs-d30-desktop-before" label="데스크톱 BEFORE" width={1100} height={1000}>
          <CSDesktopBefore/>
        </DCArtboard>
        <DCArtboard id="cs-d30-desktop-after" label="데스크톱 AFTER" width={1100} height={1280}>
          <CSDesktopAfter/>
        </DCArtboard>
        <DCArtboard id="cs-d30-mobile-before" label="모바일 BEFORE" width={460} height={950}>
          <CSMobileBefore/>
        </DCArtboard>
        <DCArtboard id="cs-d30-mobile-after" label="모바일 AFTER" width={460} height={950}>
          <CSMobileAfter/>
        </DCArtboard>
      </DCSection>

      <DCSection id="diet-fix" title="2 · Validation Fix · diet-habit-2week (health-sensitive)" subtitle="A3 — decision/principle/observation을 한 list에 섞지 않는다. CS-D30 패턴(A1·A2) + stop-condition 분리.">
        <DCArtboard id="diet-rationale" label="원칙 + 같이 적용할 라우트 5개 + PR-4/5/6" width={920} height={1280}>
          <DietRationaleDoc/>
        </DCArtboard>
        <DCArtboard id="diet-desktop-before" label="데스크톱 BEFORE" width={1100} height={1120}>
          <DietDesktopBefore/>
        </DCArtboard>
        <DCArtboard id="diet-desktop-after" label="데스크톱 AFTER" width={1100} height={1380}>
          <DietDesktopAfter/>
        </DCArtboard>
        <DCArtboard id="diet-mobile-before" label="모바일 BEFORE" width={460} height={950}>
          <DietMobileBefore/>
        </DCArtboard>
        <DCArtboard id="diet-mobile-after" label="모바일 AFTER" width={460} height={950}>
          <DietMobileAfter/>
        </DCArtboard>
      </DCSection>

      <DCSection id="newcar-fix" title="3 · Validation Fix · new-car-delivery-check (money-at-risk)" subtitle="A6 — ‘확인 list’ 만으로는 보류를 만들 수 없다. 보류 기준 섹션 + 증거 시트 + 보류 메모 + 보류 카운터 CTA.">
        <DCArtboard id="newcar-rationale" label="원칙 + 같이 적용할 라우트 4개 + PR-7/8/9/10" width={920} height={1320}>
          <NCDRationaleDoc/>
        </DCArtboard>
        <DCArtboard id="newcar-desktop-before" label="데스크톱 BEFORE" width={1100} height={1100}>
          <NCDDesktopBefore/>
        </DCArtboard>
        <DCArtboard id="newcar-desktop-after" label="데스크톱 AFTER" width={1100} height={1480}>
          <NCDDesktopAfter/>
        </DCArtboard>
        <DCArtboard id="newcar-mobile-before" label="모바일 BEFORE" width={460} height={950}>
          <NCDMobileBefore/>
        </DCArtboard>
        <DCArtboard id="newcar-mobile-after" label="모바일 AFTER" width={460} height={950}>
          <NCDMobileAfter/>
        </DCArtboard>
      </DCSection>

      <DCSection id="desktop-timeline" title="4 · 데스크톱 · timeline — 이사 D-30" subtitle="앵커 = 이사일. Primary = 월간 캘린더(.ics), Secondary = 실행 리스트(.xlsx)">
      ...
      <DCSection id="desktop-routine" title="5 · 데스크톱 · routine — 운동 4주 루틴" subtitle="앵커 = 시작일. Primary = 회차 그리드(28회), Secondary = 회차 기록표 (RPE/세트/메모)">
        <DCArtboard id="desktop-routine-board" label="WEEK 2 화요일 — 회차 진행 중" width={1280} height={1560}>
          <ScreenRoutine/>
        </DCArtboard>
      </DCSection>

      <DCSection id="desktop-phase" title="6 · 데스크톱 · phase — 이유식 단계별 (민감 카테고리)" subtitle="앵커 = 아이 생년월일. design.md ‘FIRST FLAG’. 위험 라벨·응급 정지 조건·공식/경험 분리 강하게 적용.">
        <DCArtboard id="desktop-baby" label="초기 D+12 · 알레르기 위험 식재료 표시" width={1280} height={1660}>
          <ScreenBabyFood/>
        </DCArtboard>
      </DCSection>

      <DCSection id="desktop-checklist" title="7 · 데스크톱 · checklist + decision — 신차 인수 점검" subtitle="앵커 = 인수일. Primary = 후보 비교표, Secondary = 현장 체크 + 구매 보류 메모">
        <DCArtboard id="desktop-car" label="비교 + 현장 체크 + 보류 메모" width={1280} height={1280}>
          <ScreenCarDelivery/>
        </DCArtboard>
      </DCSection>

      <DCSection id="mobile" title="8 · 모바일 — 첫 뷰포트가 산출물에 닿게" subtitle="anchor + 1개의 목적지 명시 CTA. 표는 시트로 우회. 민감 카테고리는 banner 우선.">
      ...
      <DCSection id="system" title="9 · 시스템" subtitle="색상 · 배지 · 버튼 · 카피 기준">
