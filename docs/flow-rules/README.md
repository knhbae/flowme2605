...
- [Flow content source selection rules](./flow-content-source-selection.md): choose Korean-first original content with a real execution skeleton, use representative coverage axes to avoid repetitive batches, track `lifeArea x planningPattern x portfolioRole`, and require source-row trace plus score comments before pre-app promotion.
