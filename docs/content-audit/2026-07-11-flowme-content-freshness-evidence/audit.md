...
실행 노출 모델을 결합한 결과 정상 사용자 route는 161개이며, 이 가운데 확인일이 없던 8개만 실제 수정 대상으로 판정했다. 8개를 재확인한 뒤 정상 사용자 route의 원문 확인일과 출처 정밀도 누락은 0개가 됐다.
...
| 중고차 구매 현장 점검 | [중고차 구매 체크리스트](https://www.drive-insight.net/posts/used-car-buying-checklist-ko/) | exact | 조사·현장 확인·계약 전 확인 순서가 현재 Flow와 대응한다. |
...
- 정상 사용자 후보 161개 중 86개는 여전히 `needs_review`다. 확인일이 있다는 사실은 source-fit 승인을 뜻하지 않는다.
- broad source 5개는 정확 원문 교체 후보로 남는다.
