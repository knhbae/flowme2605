...
P19-02 resolves the completion-control split that made the same task-complete action appear as buttons, checkboxes, and lower-level checklist toggles across My Flow and Calendar. Task completion is now measured as one checkbox pattern through `taskCompletionControls`; legacy task-completion buttons must stay at zero, mixed task-completion controls must stay at zero, and lower-level detail/public preview checkboxes are kept in the separate `subChecklistCheckboxCount` bucket.
...
- P19-02: My Flow and Calendar task-complete controls use the same checkbox pattern, while `열기` remains the detail action and lower-level `확인 항목`/public preview checkboxes stay in a separate evidence bucket.
...
- UI baseline commit: `8acb2da`
- Package generated from commit: `8acb2da`
