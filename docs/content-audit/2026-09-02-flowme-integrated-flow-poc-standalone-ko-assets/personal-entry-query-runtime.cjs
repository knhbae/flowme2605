...
const canonicalEntry = path.join(repoRoot, 'lib', 'flow', 'personal-workspace-poc-entry.ts');

// Build-time adapter only. write:false returns the unchanged canonical resolver
// in memory; it neither writes a generated file nor adds any app writer export.
function bundle(format, platform, globalName) {
  ...
  if (!output) throw new Error('personal-entry-query-runtime-build-failed');
  return output.text.replace(/\r\n?/gu, '\n').trimEnd();
}
function buildBrowserText() { return bundle('iife', 'browser', 'FlowPocPersonalEntryCanonical'); }
let cachedCommonJs;
function loadCommonJs() {
  if (cachedCommonJs) return cachedCommonJs;
  const source = bundle('cjs', 'node'), runtimeModule = { exports: {} };
  const compile = vm.runInThisContext(`(function (module, exports) {\n${source}\n})`,
    { filename: `${canonicalEntry}.standalone-query.cjs` });
