...
const canonicalEntry = path.join(
  ...
  'lib',
  'flow',
  'personal-workspace-poc-structure-template',
  'index.ts',
);
...
function bundle(format, platform, globalName) {
  ...
  if (!output) throw new Error('structure-template-runtime-build-failed');
  return output.text.replace(/\r\n?/gu, '\n').trimEnd();
}

function buildBrowserText() {
  return bundle('iife', 'browser', 'FlowMePersonalWorkspaceStructureTemplate');
}
