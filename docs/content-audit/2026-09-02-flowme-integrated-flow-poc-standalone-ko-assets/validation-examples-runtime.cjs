...
const canonicalEntry = path.join(
  ...
  'lib',
  'flow',
  'personal-workspace-poc-validation-examples.ts',
);
...
function bundle(format, platform, globalName) {
  ...
  if (!output) throw new Error('validation-examples-runtime-build-failed');
  return output.text.replace(/\r\n?/gu, '\n').trimEnd();
}

function buildBrowserText() {
  return bundle('iife', 'browser', 'FlowMePersonalWorkspacePocValidationExamples');
}
