...
const canonicalEntry = path.join(
  ...
  'lib',
  'flow',
  'personal-workspace-poc-source-candidates.ts',
);
...
function bundle(format, platform, globalName) {
  ...
  if (!output) throw new Error('source-update-runtime-build-failed');
  return output.text.replace(/\r\n?/gu, '\n').trimEnd();
}

function buildBrowserText() {
  return bundle('iife', 'browser', 'FlowMePersonalWorkspaceSourceUpdate');
}
