...
5. 일반 메모에는 전체 Flow 기준일을 추정하지 않는다. 사용자가 선택한 날짜는 `첫 할 일 날짜`로만 사용한다.
6. 첫 항목만 Calendar에 배치하고 나머지는 날짜 없는 할 일로 저장한다. 저장 후 필요한 항목에만 날짜를 추가할 수 있다.
7. 첫 할 일 날짜는 저장 후 My Flow에서 다시 수정할 수 있게 기존 draft edit path를 재사용한다.
8. 메모 원문은 `source_title: 내 메모`, `source_url` 없음으로 보존해 URL/source-backed 원본과 구분한다.
...
- `기준 D-Day` 대신 `첫 할 일 날짜`를 사용해 일반 메모를 마감형 Flow처럼 오해하지 않게 했다.
...
- 첫 항목만 `day_offset: 0`이고 나머지는 `day_offset` 없는 todo다. Calendar export는 첫 항목만, checklist/memo 계열은 전체 항목을 읽는다.
...
판정 수치는 [런타임 콘텐츠 재고 분리 evidence](../2026-07-12-flowme-runtime-content-inventory-separation-evidence/README.md)를 재사용했다. 공개 승인된 71개 route를 현행 콘텐츠로 보고, review-gated 86개·archive 20개·공개 중단 구형 Flow Map 8개는 현행 UX 평가와 추천 콘텐츠 표본에서 제외했다. 따라서 과거 문서에 화면이 남아 있어도 현재 production route가 아니면 이번 결론을 바꾸는 근거로 사용하지 않는다.

## 남은 리스크

- 현재 메모 분해는 규칙 기반이며 복잡한 표, 긴 문단, 시간 표현을 정교하게 해석하지 않는다.
- 날짜는 자동 추론하지 않고 사용자가 첫 할 일 날짜만 선택한다.
