# URL-to-FLOW SourceRow-only Semantic Proposal Prompt v1.1

Prompt version: `url-to-flow-prompt-v1.1`  
...
CASE_INPUT_JSON
{
  "requestId": "rq-a47e19d260bf",
  "caseId": "case-10",
  "maxItems": 7,
  "sourceOwnership": {
    "primarySourceId": "src-b6720d4e91a5",
    "supportingSourceIds": [
      "src-40c9a71e2f86"
    ]
  },
  "sourceRows": [
    {
      "sourceRowId": "row-58a7d0c31e94",
      "sourceId": "src-b6720d4e91a5",
      "rowType": "date",
      "title": "정기검사 유효기간",
      "detail": null,
      "order": 0
    ...
    {
      "sourceRowId": "row-a20e6b9f4731",
      "sourceId": "src-40c9a71e2f86",
      "rowType": "reference",
      "title": "검사기관 내부 검사 단계",
      "detail": null,
      "order": 0
...
## Round 1 결함 교정: required output contract lock

Round 1의 유일한 prompt 수정 class는 `required_output_contract_compliance`다. 의미 판단 규칙은 바꾸지 않는다. 의미를 쓰기 전에 아래 구조를 먼저 만들고, 마지막 silent check에서 다시 확인한다.

- `status`는 정확히 `generationState`, `outcome`, `readiness`, `errorCode` 네 키다. `proposal` 같은 대체 키를 쓰지 않는다.
- non-null `scheduleCandidate`는 정확히 `sourceRowIds`, `sourceText`, `parsedByRule` 세 키다. `sourceRowIds`는 빈 배열일 수 없다.
- `projectionPlan`의 각 항목은 정확히 `target`, `applicability`, `reason` 세 키다. 가능한 모든 target을 열거하지 말고 실제로 applicable한 최소 후보만 넣는다.
- `humanReviewRequired`는 항상 문자열 배열이다. 필요 없으면 `[]`이며 boolean을 쓰지 않는다.
- `uncertainties`와 `hardFailCodes`도 항상 문자열 배열이다.
- nullable 필드는 생략하지 말고 명시적으로 `null`을 쓴다.
- JSON skeleton의 키 이름을 설명 문구로 바꾸거나 새 키를 추가하지 않는다.
