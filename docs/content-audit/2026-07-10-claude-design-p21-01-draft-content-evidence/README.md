# FlowMe Claude Design P21-01 Draft Content Evidence

- Generated: 2026-07-10T15:06:28.263Z
- Branch: `main`
- UI baseline commit: `fc700a1`
- Package generated from commit: `fc700a1`
...
- URL-first miss/candidate 초안이 한 항목짜리 빈 골격에서 3~7개 결정론적 제안 항목으로 바뀌었다.
- 이 fixture는 제안 항목 3개를 만들고 `day_offset` 0, 1, 2를 기준일에서 실제 날짜로 투영한다.
- 저장 전에는 제안 목록만 확인하며 full editor를 만들지 않았다.
- 저장 후 My Flow에서 포함 여부, 제목, 날짜, 메모를 기존 편집 모델로 조정한다.
- Calendar와 export는 기존 draft projection을 그대로 읽는다.
- 실제 AI API는 사용하지 않으며 live-AI 암시 marker는 `false`다.
- P21-03에서 지적된 정상 route 구조 표시 2건도 사용자 행동 문구로 바꿔 normal/wide 모두 0으로 닫았다.

핵심 marker: `urlFirstMissDraftSuggestedItemCount: 3`, `urlFirstMissDraftStepDatesFromAnchor: true`, `urlFirstMissDraftImpliesLiveAi: false`, `normalRouteStructuralDisplayHitCount: 0`, `wideViewportStructuralDisplayHitCount: 0`.
...
- Normal route native date input raw ISO exemptions: 13
...
- Progress metric contextual label count: 26
...
- My Flow anchor settings open accessible names: ["주말 준비 초안 기준일·이름 바꾸기","이사 준비 이사일·이름 바꾸기"]
...
- My Flow item edit accessible names: ["주말 준비 범위 정하기 제목·날짜·메모 수정","내 일정에 맞춘 첫 단계 제목·날짜·메모 수정","임대차 계약 확정일자 부여 여부 확인하기 제목·날짜·메모 수정"]
- Edit entry visible by viewport: {"390":{"anchor":4,"item":3},"1024":{"anchor":1,"item":1}}
- Draft Flow My Flow landing visible: yes
- Draft Flow edit entry visible: yes
- Draft Flow anchor edit visible by viewport: {"390":2,"1024":1}
...
- Wide viewport guardrail route count: 13
...
- Studio entry visible by viewport: {"390":16,"1024":4}
- Studio entry reachable by viewport: {"390":16,"1024":4}
