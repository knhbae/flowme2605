...
P19-04 is treated as the Calendar wide heading/label duplicate audit. The current package keeps duplicate wide Calendar headings at zero and preserves Flow identity markers without returning to generic `일정`-first labels.

P19-05 is treated as the public `/f` pre-save checkbox preview policy. Pre-save checkboxes are measured separately from saved task-completion controls, and completion-like labels on public pre-save checkboxes remain at zero.

P19-06 keeps the Home URL/memo entry discoverable as the first-time user entry into `/flows`; it does not add another lookup implementation.

P19-07 keeps the post-save edit entry discoverable for both whole-Flow anchor/name changes and individual task title/date/memo edits.

P19-08 is closed by the P18-08 AI draft gate plus this package's evidence. It stays a spec/gate item: no live AI API, no automatic draft generation, and no pre-save full editor are claimed.
...
{
  "totalScreenshots": 47,
  "uiBaselineCommit": "1943a9b",
  "packageGeneratedFromCommit": "1943a9b",
...
- UI baseline commit: `1943a9b`
- Package generated from commit: `1943a9b`
