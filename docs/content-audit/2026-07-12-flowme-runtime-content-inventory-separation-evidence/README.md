...
- 공개 승인: 71개
- 검토 게이트: 86개
- 정상 발견 경로의 검토 게이트 링크: 0개 (Flow 찾기 0, 공개 제작자 프로필 0)
- 공개 제작자 프로필 초기 콘텐츠: 12개, 더 보기 action viewport 합계: 2개, 펼친 미리보기/베타 라벨: 0/0개
- 검토 게이트의 예전 실행 미리보기/목록: 0/0개, 숨김 marker: 1개
- 내부 전체 재고: 617개
...
생성형 샘플과 명시적 archive 20개는 삭제하지 않고 내부 검토 재고에 보존했다. 미저장 archive는 runtime에서 제거하고, 사용자가 저장한 archive는 완료·메모를 유지하는 이전 저장본으로 남긴다. 과거 direct public URL은 한국어 복귀 경로가 있는 서비스용 404다. 검토 게이트 86개는 공개 승인 콘텐츠로 세지 않으며 noindex와 행동 차단을 유지한다. 정상 카탈로그와 공개 제작자 프로필의 링크는 0개이며, direct route에서도 확인 전 실행 문구를 숨긴다. 구형 Flow Map 8개는 현재 대표 Map으로 카탈로그를 교체하고 direct route도 닫았다.
