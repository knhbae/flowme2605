# FlowMe Claude Design P21-04 Draft Lifecycle Evidence

- Generated: 2026-07-10T15:53:55.920Z
- Branch: `main`
- UI baseline commit: `fddf6c8`
- Package generated from commit: `fddf6c8`
...
- lifecycle 5개 그룹을 390px/1024px에서 모두 재현했다: 저장 실패, canonical 중복, 빈 My Flow/Calendar, 전체 완료 후 0, 이미 열린 My Flow의 오프라인 로컬 행동.
- 저장 실패 시 제목과 기준일이 유지되고 재시도 안내가 보인다.
- 같은 canonical URL의 draft를 다시 저장해도 추가 Flow를 만들지 않으며 기존 My Flow draft로 가는 링크가 보인다.
- 빈 My Flow/Calendar는 Flow 찾기 recovery를 제공한다.
- 전체 완료 상태는 남은 개수 0으로 기록되고 완료 체크를 되돌릴 수 있다.
- 오프라인 범위는 이미 열린 My Flow의 로컬 체크로 한정하며 네트워크 이동 성공을 주장하지 않는다.

핵심 marker: `draftLifecycleScenarioCount: 5`, `draftSaveFailureInputPreserved: true`, `draftDuplicateCreatesExtraSavedFlow: false`, `draftEmptyStateCaptured: true`, `draftCompletedRemainingCount: 0`, `draftOfflineLocalActionsAvailable: true`, `draftLifecycleInternalHitCount: 0`.
...
- [P21 AI draft gate spec](https://github.com/knhbae/flowme2605/blob/main/docs/specs/2026-07-11-url-first-ai-draft-gate/spec.md)
...
- Calendar mobile agenda open-label rows: 15
- Calendar same-date distinct Flow groups: 4
- Calendar same-date grid Flow labels: 2
- Calendar agenda grouped by Flow: yes
- Calendar grid same-date Flow count: 4
- Calendar grid visible Flow labels: 2
- Calendar grid distinct visible marker identities: 2
- Calendar grid overflow summary visible: yes
- Calendar grid hidden Flow summary count: 2
- Calendar grid horizontal overflow count: 0
- Calendar selected-day agenda shows all Flows: yes
...
- Home URL-first entry separator present: yes
- Home URL-first concatenated-label hits: 0
- Draft lifecycle scenario count: 5
- Draft save-failure captured: yes
- Draft save-failure recovery visible: yes
- Draft save-failure input preserved: yes
- Draft duplicate captured: yes
- Draft duplicate creates extra saved Flow: no
- Draft duplicate recovery visible: yes
- Draft empty My Flow/Calendar captured: yes
- Draft completed-zero captured: yes
- Draft completed remaining count: 0
- Draft offline captured: yes
- Draft offline local actions available: yes
- Draft lifecycle internal hits: 0
- Draft lifecycle horizontal overflow count: 0
- My Flow anchor edit entry visible: yes
- My Flow anchor settings open labels: ["기준일·이름 바꾸기","이사일·이름 바꾸기"]
- My Flow anchor settings open accessible names: ["주말 준비 초안 기준일·이름 바꾸기","완료 상태를 확인할 주말 준비 기준일·이름 바꾸기","이사 준비 이사일·이름 바꾸기"]
- My Flow anchor edit labels: ["기준일 바꾸기","이사일 바꾸기"]
- Item date override labels: ["이 할 일 날짜"]
- Anchor vs item override copy present: yes
- My Flow item edit entry visible: yes
- My Flow item edit accessible names: ["주말 준비 범위 정하기 제목·날짜·메모 수정","내 일정에 맞춘 첫 단계 제목·날짜·메모 수정","완료 상태를 확인할 주말 준비 범위 정하기 제목·날짜·메모 수정","임대차 계약 확정일자 부여 여부 확인하기 제목·날짜·메모 수정"]
- Edit entry visible by viewport: {"390":{"anchor":6,"item":4},"1024":{"anchor":3,"item":2}}
- Draft Flow My Flow landing visible: yes
- Draft Flow edit entry visible: yes
- Draft Flow anchor edit visible by viewport: {"390":4,"1024":3}
- Draft Flow item edit entry visible: yes
- Draft Flow anchor/override policy visible: yes
- Draft Flow Calendar projection updated: yes
- Draft Flow export projection updated: yes
- Normal route row control accessible name samples: 4
- Normal route row control samples with context: 4
- Wide viewport evidence count: 19
- Wide viewport width: 1024
- Wide viewport horizontal overflow count: 0
- Wide layout route count: 19
- Wide layout primary CTA visible count: 9
- Wide layout My Flow visible Flow finding link max: 2
- Wide home recommendation width ratio min: 1
- Wide viewport guardrail route count: 19
...
- Wide viewport routes captured: [{"id":"39f-url-first-draft-studio-shelf-wide","route":"/u/my-flow-studio","viewportWidth":1024,"noHorizontalOverflow":true},{"id":"39c-url-first-draft-anchor-edit-wide","route":"/my","viewportWidth":1024,"noHorizontalOverflow":true},{"id":"45-draft-save-failure-wide","route":"/flows","viewportWidth":1024,"noHorizontalOverflow":true},{"id":"46-draft-duplicate-wide","route":"/flows","viewportWidth":1024,"noHorizontalOverflow":true},{"id":"47a-draft-empty-my-flow-wide","route":"/my","viewportWidth":1024,"noHorizontalOverflow":true},{"id":"47b-draft-empty-calendar-wide","route":"/calendar","viewportWidth":1024,"noHorizontalOverflow":true},{"id":"48-draft-completed-zero-wide","route":"/my","viewportWidth":1024,"noHorizontalOverflow":true},{"id":"49-draft-offline-local-action-wide","route":"/my","viewportWidth":1024,"noHorizontalOverflow":true},{"id":"32-home-wide","route":"/","viewportWidth":1024,"noHorizontalOverflow":true},{"id":"33-flows-wide","route":"/flows","viewportWidth":1024,"noHorizontalOverflow":true},{"id":"34-flow-map-moving-wide","route":"/flow-maps/moving-d30","viewportWidth":1024,"noHorizontalOverflow":true},{"id":"35-public-vehicle-wide","route":"/f/vehicle-inspection-prep","viewportWidth":1024,"noHorizontalOverflow":true},{"id":"44-calendar-same-date-multi-flow-wide","route":"/calendar","viewportWidth":1024,"noHorizontalOverflow":true},{"id":"44b-calendar-grid-flow-stack-wide","route":"/calendar","viewportWidth":1024,"noHorizontalOverflow":true},{"id":"36-post-save-my-moving-wide","route":"/my?savedMap=moving-d30","viewportWidth":1024,"noHorizontalOverflow":true},{"id":"37-url-first-hit-wide","route":"/flows","viewportWidth":1024,"noHorizontalOverflow":true},{"id":"38-url-first-candidate-detail-wide","route":"/flows","viewportWidth":1024,"noHorizontalOverflow":true},{"id":"40-creator-profile-my-flow-studio-wide","route":"/u/my-flow-studio","viewportWidth":1024,"noHorizontalOverflow":true},{"id":"42-creator-profile-flow-curation-team-wide","route":"/u/flow-curation-team","viewportWidth":1024,"noHorizontalOverflow":true}]
- Studio nav destination: /u/my-flow-studio
- Studio nav destination tier: creator-profile
- Studio entry visible by viewport: {"390":18,"1024":6}
- Studio entry reachable by viewport: {"390":18,"1024":6}
...
- Creator profile route count: 6
- Creator profile viewport widths: [390,1024]
- Creator profile tier: creator-profile
- Creator profile guardrail hits: 0
- Creator profile filled route count: 6
- Creator profile empty route count: 0
- Creator profile content card count: 162
- Creator profile draft tab visible: yes
- Creator profile draft content card count: 4
- Creator profile URL-first draft card count: 2
- Creator profile draft edit path visible: yes
- Creator profile draft edit destinations: [{"label":"내 Flow에서 수정","href":"/my","pathname":"/my"}]
...
- URL-first native date input raw ISO exemptions: 4
...
- URL-first miss draft in-app entry visible: yes
- URL-first miss draft editable item count: 3
- URL-first miss draft suggested item count: 3
- URL-first miss draft dates from anchor: yes
- URL-first miss draft save path visible: yes
- URL-first miss draft internal hits: 0
...
- Public post-save completion control visible: yes
- Public post-save completion control pattern: checkbox
- Public post-save completion control active: yes
- Public post-save completion checkbox count: 1
- Public post-save completion button count: 0
