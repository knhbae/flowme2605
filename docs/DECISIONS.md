...
### 2026-07-18 - Public Flow quality requires source sufficiency and live-baseline value uplift

**Decision:** Keep the v3 deterministic `SourceRow -> Item` controller only as a lower source-accounting and unsupported-claim safety layer. It cannot grant public-content Go. Before compilation, require a source-sufficiency disposition of `compile_candidate`, `draft_only`, `reextract_required`, or `blocked`; after compilation, require first-class `doneWhen`, usable destination payloads with a loss ledger, explicit source/product/user/safety ownership, all eight live Flow-finding capabilities, and independent blinded comparison against a strong matched live reference. Treat the live seven-map `/flows` catalog as the current public-surface baseline, with moving D-30, OPIC, and new-car purchase as the primary quality references. A thin but faithful output is `source-reviewed-but-thin`, not a publish candidate. Preserve `SourceEvidence` as the smallest truth unit and `Item` as the smallest stateful execution unit; ICS, checklist, todo, sheet, and memo remain projections. Every source-to-Flow quality report or review surface must lead with the actual source evidence, exact generated content, rendered destination artifacts, and the count of generated versus intentionally non-generated cases before scores or algorithm explanation. A hold/re-extract disposition must show an explicit zero-output state rather than look like a hidden completed Flow.

**Reason:** Re-audit showed that all fifteen v3 Items were exact SourceRow copies or fixed `확인하기/완료하기/열어보기` suffixes; only one Item had a memo, one had schedule evidence, none had a source locator or observable completion criterion, and none of the ten projections contained a usable destination payload. Nine of ten positive cases were label-only and the remaining filter case was only partially executable. The prior 4.99/5 model-proxy score therefore measured sparse-row fidelity, not product quality. After re-fetching the bounded official Samsung filter source, a v4 candidate preserved the source's 4-week interval, conditional cleaning methods, drying action, and caution while adding only a user-selected first date, execution state, and three loss-aware projections. Two reversed-order independent model-proxy reviews preferred that candidate overall in both contexts and on 13/14 value judgments, with zero unsupported or unsafe findings; this remains one-case internal evidence, not human or public-readiness validation. The first report version explained the gate before exposing the Flow and could be misread as twelve completed v4 outputs even though only one existed. Direct review requires seeing the source-to-Item mapping and actual calendar/checklist/memo payload, including zero counts for intentionally withheld cases.

**Applies to:** URL-to-FLOW extraction, SourceEvidence and Item schemas, `doneWhen`, artifact planning, compiler licenses, source sufficiency, proposal validation, Flow finding promotion, reviewer packets and reports, content-audit decks and galleries, cheap/premium model experiments, cost measurement, Calendar/ICS, checklist/todo, sheet, memo exports, and human save/publish authority.

**Reopen when:** ten fully re-extracted URLs across at least five planning patterns pass the same hard and pairwise gates; human reviewers or observed users consistently prefer a different threshold; a representative source cannot express observable completion without unsafe inference; or measured accepted-Flow cost and edit evidence justify changing the cheap-to-premium escalation policy.

**Related docs:** [v4 spec](./specs/2026-07-18-url-to-flow-value-uplift-v4/spec.md), [v4 quality rubric](./specs/2026-07-18-url-to-flow-value-uplift-v4/quality-rubric-v4.md), [live baseline capture](./content-audit/2026-07-18-url-to-flow-value-uplift-v4/live-baseline-capture.md), [v4 Korean PPT report](./content-audit/2026-07-18-url-to-flow-value-uplift-v4/report.html), [12-case actual-content gallery](./content-audit/2026-07-18-url-to-flow-value-uplift-v4/flow-content-gallery.html), [v3 controller decision](#2026-07-18---sourcerow-to-flow-is-a-deterministic-controller-url-extraction-stays-a-separate-gate)

### 2026-07-18 - SourceRow-to-FLOW is a deterministic controller; URL extraction stays a separate gate

**Decision:** Use `SourceRow` as the smallest provenance unit, `Item` as the smallest independently stateful execution unit, `Flow` as the composition of Items and their relations, and Calendar/ICS, checklist, todo, sheet, and memo as projections rather than storage minima. Keep field ownership explicit: an LLM/parser may propose `URL -> SourceRow` and must be able to return unknown or blocked; a versioned deterministic compiler owns licensed `SourceRow -> Item`, projections, review markers, and omission accounting; a human owns save and publish. Adopt the v3 hybrid controller contract for downstream implementation work, but do not treat it as approval for production URL fetch, a real extractor/model, automatic save/publish, or external-account writes.

**Reason:** The preregistered v2 semantic-generation lane failed the same canonical case in two independent rounds (11/12 each) because a sparse row was changed into an unsupported action/completion interpretation. After freezing the boundary and moving only licensed grammar into a deterministic compiler, v3 passed all 12 cases in two fresh processes, accounted for 16/16 SourceRows, produced all 15 eligible Items, blocked both negative cases, yielded 0 unsupported signals, reproduced 12/12 outputs exactly, and received 15/15 keep verdicts with a 4.99/5 seven-axis average from three isolated model-proxy review batches. Provider, model, token, latency, and cost fields remained null, so this evidence establishes the downstream controller boundary only.

**Applies to:** URL-to-FLOW backend architecture, SourceRow and Item contracts, compiler versioning, proposal validation, omission and uncertainty markers, FLOW storage, export adapters, review UI, save/publish authority, regression fixtures, and the next provider/cost experiment.

**Reopen when:** real URL snapshots can be evaluated against human-checked SourceRows, at least two extractor tiers are compared on the same frozen cases, accepted-Flow cost/latency/edit-rate evidence exists, or representative content requires semantics that cannot be expressed through the licensed SourceRow contract without source loss.

**Related docs:** [v2 strict No-Go spec](./specs/2026-07-18-url-to-flow-prompt-lab-v2-strict/spec.md), [v3 hybrid controller spec](./specs/2026-07-18-url-to-flow-prompt-lab-v3-hybrid/spec.md), [v3 comparison](./content-audit/2026-07-18-url-to-flow-prompt-lab-v3-hybrid/comparison.md), [Korean PPT report](./content-audit/2026-07-18-url-to-flow-prompt-lab-v3-hybrid/report.html)

### 2026-07-18 - Sparse SourceRows may stop or stay unknown instead of forcing semantic classification

**Decision:** Keep deterministic intake/preflight, SourceRow extraction, bounded semantic generation, validation, and explicit human save as separate owners. Do not use the current SourceRow-only Prompt Lab v1 contract as a production URL-to-FLOW backend contract: when `rowType/title/detail/order` does not support a required semantic classification such as `lifeArea`, planning context, action wording, or source hierarchy, the pipeline must be able to return `unknown`/`null`, request better SourceRows, or hold the draft rather than force an enum or invent a useful-sounding action. The 2026-07-14 rich-packet lane remains preflight evidence only because it exposed hidden canonical semantics; only the corrected SourceRow-only lane may count toward future generation-quality gates.

**Reason:** In the corrected 12-case experiment, deterministic negatives stayed exact, but Round 2 failed both Item keep and unsupported-content gates, and the Round 3 output comparison still produced unsupported facts plus one SourceRow-accounting failure. Independent audit also found that canonical case IDs remained generator-visible, Round 1 used 4+4+2+2 rather than the preregistered 4+4+4 envelope shape, Round 3 ran despite its unmet prerequisite, and run logs did not bind outputs to packet hashes or prove fresh contexts. The lane is therefore experimental evidence, not completion authority. Sparse noun/resource rows made required semantic fields look more certain than the supplied evidence allowed. More prompt iterations would hide data-contract and experiment-contract problems rather than establish a production-ready conversion boundary.

**Applies to:** URL-to-FLOW extraction and generation contracts, SourceRow sufficiency checks, semantic proposal schemas, provider/model evaluation packets, validators, review UX, backend Go/No-Go, and future cost comparisons.

**Reopen when:** a separately preregistered schema supports uncertainty and insufficient-source outcomes, SourceRow extraction provides an explicit action/context contract, and the unchanged canonical 12-case suite passes schema, accounting, unsupported-content, negative, and blind-review gates with a directly identified provider/model and measured cost evidence.

**Related docs:** [corrected Prompt Lab spec](./specs/2026-07-15-url-to-flow-prompt-lab-source-row-v1/spec.md), [comparison](./content-audit/2026-07-15-url-to-flow-prompt-lab-source-row-v1/comparison.md), [Korean PPT report](./content-audit/2026-07-15-url-to-flow-prompt-lab-source-row-v1/report.html)
