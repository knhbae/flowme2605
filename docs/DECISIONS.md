...
**Applies to:** `/flows/new`, `components/flow/text-authoring/*`, `lib/flow/text-authoring/*`, standalone HTML, grammar catalog, local draft/source-sync, Calendar·Todo·Sheet·TXT preview/export와 Draft PR [#175](https://github.com/knhbae/flowme2605/pull/175). P35 adapter, 외부 서비스 쓰기, AI 생성, merge와 production deployment는 포함하지 않는다. PR에서 자동 생성되는 Vercel Preview는 검토용 CI 결과이며 production release가 아니다.
