...
### 2026-08-04 - Text Authoring의 항목 구조는 선택형 검토로만 노출한다

**Decision:** `/flows/new`의 기본 저작 여정은 `입력 → 결과` 두 단계다. `>=900px`에서는 입력과 결과를 동시에 두 pane으로 보이고, `<900px`에서는 입력/결과 한 pane씩 staged navigation으로 전환한다. 내부 Step/Item 구조와 stable ID/source lineage는 유지하되 상시 세 번째 pane이나 필수 승인 단계는 두지 않는다. 항목 검토는 결과 상단 launcher에서 여는 desktop drawer/mobile bottom sheet이며, 정상 입력은 included Item 수를 중립적으로 알리고 검토를 열지 않아도 결과 확인과 로컬 저장이 가능하다. 경고 count는 outstanding outline issue만 소유한다. Source update conflict, 상대 날짜 기준일, 권리·안전 gate는 각각 기존 source-update, 결과 기준일, review surface에서 해결하며 항목 검토 count와 합산하지 않는다. 결과 행의 `수정`은 해당 Item inspector로 직접 연결하고, 이전 `activeStage=structure` draft는 result로 정규화한 뒤 항목 검토를 연다.

**Reason:** Canonical 구조는 계산·보정·export에 필요하지만 정상 입력에서 이를 필수 중간 단계로 노출하면 결과 확인을 늦추고 모바일 단계 수를 늘린다. 문제 유형별 해결 surface와 count 소유권을 분리해야 경고 수와 다음 행동이 예측 가능하다.

**Applies to:** `/flows/new`, `components/flow/text-authoring/*`, standalone HTML, Text Authoring E2E/visual evidence, authoring 상태 문서. Parser, projection, storage, P35 adapter 계약은 변경하지 않는다.

**Reopen when:** 관찰된 사용 행동에서 구조를 먼저 확인해야만 막을 수 있는 반복 오류가 확인되거나, canonical hierarchy/export 계약이 변경되거나, 900px 경계에서 두 pane이 실제 작업을 방해한다. 내부 QA만으로 재개하지 않는다.

**Related docs:** [선택형 항목 검토 목표](./specs/2026-08-04-flowme-text-authoring-optional-structure-review/00-development-goal-ko.md), [구현·QA 결과](./content-audit/2026-08-04-flowme-text-authoring-optional-structure-review-results/README.md), [standalone HTML](./content-audit/2026-08-04-flowme-text-authoring-grammar-ux-improvement-results/flowme-text-authoring-v2-test.html)

### 2026-08-04 - Text Authoring v2 makes source ownership and result choices explicit

**Decision:** Supersede the visible v1 authoring grammar and result-selection behavior with the approved v2 contract. A new Item is created only by a root `- [ ]` line. Its official properties use an indented bullet such as `  - 설명:`, while the parser keeps the prior indented `설명:` form as read-only compatibility and the writer emits v2 only. Unmarked prose remains source text unless a separately approved import-assist action converts it. Unknown indented colon bullets and nested checkboxes remain source-linked `unknown_property` and `unsupported_nested_item` issues rather than Items. Relative dates become Calendar dates only when the raw source contains `- 기준일: YYYY-MM-DD`; a date selected in the UI must write that line. Calendar preview and ICS use resolved-date order with source order as the tie-break, while canonical, 체크/할 일, 표/엑셀, and 텍스트 keep source order. Source blocks move only after `입력도 이 순서로 맞추기`, within their current Step, as one undoable transaction that retains Item IDs and source lineage. The result picker always keeps four stable slots—캘린더, 체크/할 일, 표/엑셀, 텍스트—and explains disabled choices instead of removing them. 표/엑셀 requires an original table or at least two Items sharing at least two meaningful fields. 텍스트 distinguishes byte-equivalent `원문 그대로` from transformed TXT/Markdown. `항목 구조`는 기본 pane/stage가 아니라 결과의 선택형 `항목 검토`에서 읽고, move·merge·split·role 변경은 그 안의 `순서·묶음 수정`으로 연다.

**Reason:** The v1 property shape could be mistaken for an Item, hidden UI-only anchors made Calendar results hard to explain, source and Calendar order were conflated, and disappearing result choices changed the interface while typing. The approved 35-scenario v2 matrix turns those ambiguities into deterministic parser, projection, and interaction boundaries. Internal tests, simulations, and screenshots can verify implementation but do not constitute observed-user validation.

**Applies to:** `/flows/new`, Text Authoring parser and Markdown round-trip, source-order operations, Calendar/ICS and Sheet/Text projections, visible syntax help, result and structure panes, representative examples, standalone HTML, and the 27-case compatibility plus 35-case v2 evidence suites.

**Reopen when:** A stable nested-Item data model supports parent identity and lossless round-trip; an explicitly approved import-assist review step owns prose conversion; observed authoring behavior shows that the v2 property bullet blocks ordinary entry; or a destination contract changes the four result families or Sheet eligibility. Any reopen must preserve raw source, explicit user control over source rewrites, and honest separation between internal QA and observed-user evidence.

**Related docs:** [v2 review brief](./content-audit/2026-08-04-flowme-text-authoring-grammar-ux-improvement-handoff/review-brief-ko.md), [v2 implementation prompt](./content-audit/2026-08-04-flowme-text-authoring-grammar-ux-improvement-handoff/codex-implementation-prompt-ko.txt), [Text Authoring spec](./specs/2026-07-28-flowme-text-authoring-ux-v1/spec.md)

### 2026-08-04 - Text Authoring and P35 stay separate behind a versioned adapter contract

**Decision:** Keep the Text Authoring canonical/source model and the P35 effective source/personal/execution model in separate ownership lanes. Do not merge their dirty worktrees or copy route, component, or storage implementations across them. A future connection must be a deterministic, source-preserving adapter that emits a versioned `FlowBundle`, projection options, source revision, and machine-readable loss manifest; P35 then validates that output through its own effective snapshot and projection manifest. Preserve `flowId`, `itemId`, valid `stepId`, ordered IDs/counts, and every held/unavailable reason. Never map Text Authoring `sourceChecked` to P35 execution completion, invent dates for undated Items, or hide repeat/time/timezone/resource/source loss. The current runtime integration state is `HOLD_NOT_READY` until an approved adapter v2 and six shared golden fixtures pass on a clean integration base.

**Reason:** The two checkouts currently share enough DTO shape to make an adapter feasible but not enough effective behavior to justify direct integration. P35 has newer completion, effective-layer, manifest, and projection contracts, while the Text Authoring adapter still flattens or omits repeat, time, timezone, duration, Step identity, and some source/resource distinctions. A versioned one-way boundary protects raw creator source and stable identity without overwriting personal or execution state.

**Applies to:** Text Authoring `FlowBundle` generation, P35 source import, Item/Flow identity, projection counts, loss manifests, source update handling, completion state, future adapter schemas and integration branches. It does not authorize P35 code changes, a new integration branch, commit, merge, or deployment.

**Reopen when:** Text Authoring and P35 have independent green closeouts, the owner separately approves integration, clean base commits are fixed, adapter contract v2 is versioned, and the dated, undated, mixed, routine, memo, and invalid-date fixtures preserve IDs, counts, loss, source, personal, and execution ownership end to end.

**Related docs:** [Round 2 plan](./specs/2026-08-04-flowme-text-authoring-v2-round2-correction/00-next-work-plan-ko.md), [P35 integration gate](./specs/2026-08-04-flowme-text-authoring-v2-round2-correction/p35-integration-gate.md), [Round 2 results](./content-audit/2026-08-04-flowme-text-authoring-v2-round2-results/README.md)

### 2026-07-31 - Text Authoring grammar stays familiar, explicit, and fail-closed

**Decision:** Use familiar Markdown structure (`#`, `##`, `- [ ]`, and named Markdown links), explicit ISO-style date/time (`YYYY-MM-DD`, `HH:mm`), readable Korean `key: value` properties, and the bounded FlowMe relative-date extension `D-3` / `D-Day` / `D+2` as the Text Authoring v1 grammar. A canonical Item always starts at the root of its Step with `- [ ]`; its properties sit immediately below it with exactly two leading spaces. Item groups use another `##` Step, not an indented child Item. The writer emits this one canonical form and one Korean label per meaning; the parser may read plain bullets, numbered lists, indentation, and documented legacy aliases as compatibility input without presenting them as new syntax. A relative-date label such as `기준일: 행사일` names the anchor but never supplies the actual date: Calendar projection requires a separately supplied ISO anchor. `반복:` and `조건:` preserve source meaning only in this non-AI slice; they do not expand occurrences, generate ICS `RRULE`, hide Items, or create conditional branches. Ambiguous dates, unsupported properties, URL-only input, and explanatory prose remain attached to the source as explicit issues instead of being guessed into dates, Items, or fields.

**Reason:** A 27-scenario simulation across eight existing FLOW contents and nineteen single-change compatibility/error cases preserved source counts and natural Calendar/Todo/Sheet/Memo destinations without inventing missing execution semantics. The same boundary passed route/standalone UI checks, including live editing, invalid-date recovery, result switching, and short-screen scrolling. Keeping the grammar small and fail-closed is more predictable than adding shorthand whose meaning depends on AI or hidden inference.

**Applies to:** `/flows/new`, the standalone Text Authoring HTML, `lib/flow/text-authoring/parser.ts`, canonical Markdown export/re-import, artifact projection and preflight, ICS export, visible syntax help and examples, grammar simulation fixtures, and future non-AI creator authoring changes.

**Reopen when:** An approved Item-parent model adds stable parent identity, hierarchy-aware move/include/export behavior, and round-trip support; an approved AI/source-import layer owns ambiguity resolution with reviewable evidence; product scope explicitly adds a recurrence or condition engine; a supported locale/date policy replaces ISO input; or separately authorized behavior evidence shows that the explicit syntax blocks ordinary authoring. Before changing the grammar, rerun the existing-content and single-change simulation matrix. Anchor-sensitive preflight identity and the outward display of unresolved source issues remain implementation decisions, not reasons to infer more syntax.

**Related docs:** [Text Authoring spec](./specs/2026-07-28-flowme-text-authoring-ux-v1/spec.md), [grammar comparison](./specs/2026-07-28-flowme-text-authoring-ux-v1/authoring-grammar-comparison.md), [grammar simulation report](./content-audit/2026-07-31-flowme-text-authoring-grammar-simulation/flowme-text-authoring-grammar-simulation-ko.html), [simulation JSON](./content-audit/2026-07-31-flowme-text-authoring-grammar-simulation/grammar-simulation-results.json)

### 2026-07-30 - Text Authoring keeps live preview and representative conversion examples visible

**Decision:** On `/flows/new`, title changes update the current Flow immediately and unprotected source/text changes run the deterministic parser after a short debounce without moving the user away from the current stage. A saved draft, a document with correction revisions, or an already staged source update is never replaced by that live path: the new source is preserved as an incoming candidate and requires the existing explicit compare/apply or reject decision. Keep one compact, horizontally scrollable example switcher above the work area as a production onboarding aid. It starts with one synthetic example covering every supported authoring label and links to exact representative input shapes for a five-action plain memo, the qualified 27-item moving timeline, the 14-row K-MOOC table, and the seven-video Allblanc sequence. Each link names its natural artifact. Merely browsing examples is seed state and creates no draft or recovery; switching away from edited work requires a discard confirmation.

**Reason:** The creator needs to see the outline and artifact change while typing and must be able to infer date, relative-date, repeat, source, resource, caution, and completion syntax from the screen. The earlier collapsed example selector hid the input-to-artifact relationship, while unconditional reparsing could erase corrections. Separating safe live parsing, protected source comparison, and non-persisted example browsing gives immediate feedback without weakening source fidelity or local recovery.

**Applies to:** `/flows/new`, `components/flow/text-authoring/*`, the deterministic parser and source-update boundary in `lib/flow/text-authoring/*`, the standalone local HTML, Text Authoring E2E, and the authoring spec/state/QA documents.

**Reopen when:** The parser becomes expensive enough that the bounded debounce no longer keeps typing responsive, a canonical source-update contract can preserve corrections through a different mechanism, representative fixtures change version or count, or separately authorized behavior evidence shows that the persistent example row or current-stage live update obstructs the task. Automated screenshots and simulations alone are QA, not observed-user validation.

**Related docs:** [Text Authoring spec](./specs/2026-07-28-flowme-text-authoring-ux-v1/spec.md), [state model](./specs/2026-07-28-flowme-text-authoring-ux-v1/state-model.md), [local implementation evidence](./content-audit/2026-07-29-flowme-text-authoring-ta-implementation/README.md)

### 2026-07-30 - Text Authoring uses Claude structure while retaining the FlowMe visual system

**Decision:** Keep the existing deterministic parser, `SourceRow -> Item -> Step -> Flow` authoring document, artifact projection, review boundary, and namespaced local-draft storage as the functional truth. Use the accepted Claude `0729_1412` desktop/mobile design only for structure and interaction: hide the redundant stage navigation when all three desktop panes are visible, retain staged navigation at tablet/mobile widths, and open the Item inspector directly when a mobile structure row is activated. Retain the existing FlowMe visual system for color, surface, state, border, radius, typography, and primary actions; do not import the Claude black/gray/red, zero-radius Modernist theme. User-facing surfaces use Korean content terms and omit fixture IDs, raw revision IDs, `Step`/`Item` implementation labels, screen-size controls, observed-user counters, and other test chrome. Do not add AI, remote content collection, external-service writes, publishing, or user validation to this slice.

**Reason:** The current implementation already preserves source, dates, repeats, corrections, projection counts, and local recovery more safely than the isolated reference. Claude provides a useful workbench composition and mobile editing shortcut, but the owner explicitly prefers the prior FlowMe palette and surface treatment. Separating structural reference from visual identity closes the hierarchy and interaction gaps without making this module look detached from FlowMe or reopening the settled data and safety contracts.

**Applies to:** `/flows/new` on the dedicated Text Authoring branch, `components/flow/text-authoring/*`, the layout-only authoring scope in `app/globals.css`, artifact preview labeling, the standalone local HTML, responsive browser evidence at 390/1024/1440px, and the explicit `?legacy=1` rollback.

**Reopen when:** The canonical authoring or storage contract changes, an explicitly approved FlowMe visual redesign replaces the current tokens, a new structural reference replaces `0729_1412`, or separately authorized behavior evidence shows that the staged/direct-edit interaction fails its intended task. Automated screenshots and simulations alone remain internal QA.

**Related docs:** [Text Authoring spec](./specs/2026-07-28-flowme-text-authoring-ux-v1/spec.md), [local implementation evidence](./content-audit/2026-07-29-flowme-text-authoring-ta-implementation/README.md), [service structure](./SERVICE_STRUCTURE.md)

### 2026-07-30 - Text Authoring MVP PoC uses an owner, Claude Code, and Codex internal gate

**Decision:** Exclude real-user observation from the current Text Authoring MVP PoC gate. Use three independent review lanes instead: the owner performs the fixed local journey, Claude Code reviews the bounded source/UX/correctness contract independently, and Codex verifies the same scope with current repository, command, and browser evidence. An internal success candidate requires all three review records, owner completion of input -> source-preserved structure -> minimal correction -> local save/reload and representative plain-text export, and no unresolved in-scope Blocking or High finding. Record exactly one `continue / fix / stop` decision. Never describe this gate as observed-user validation, release evidence, or production readiness.

**Reason:** The owner explicitly chose direct owner plus Claude Code plus Codex review for this MVP PoC and removed participant observation from its success condition. This keeps the next decision executable without inventing user evidence, while preserving an honest boundary between internal feasibility review and later real-world usability evidence.

**Applies to:** The active Text Authoring spec, status, roadmap, QA and task gate, local implementation evidence, reviewer prompts and records, and the `continue / fix / stop` decision. It does not change P35 production, publish authorization, or the excluded My Flow/canonical/backend/cloud/full-export work.

**Reopen when:** The owner explicitly requests target-user validation, a release or investment decision requires behavior evidence beyond internal review, or a real-user study is separately approved. Internal agent review must not be relabeled as usability validation when reopened.

**Related docs:** [Text Authoring spec](./specs/2026-07-28-flowme-text-authoring-ux-v1/spec.md), [three-party review kit](./content-audit/2026-07-29-flowme-text-authoring-ta-implementation/mvp-poc-three-party-review-ko.md), [local implementation evidence](./content-audit/2026-07-29-flowme-text-authoring-ta-implementation/README.md)
