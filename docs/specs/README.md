...
- [2026-08-13 Text Authoring P1-C 장문·표 보존](./2026-08-13-flowme-text-authoring-p1-c-longform/00-development-goal-ko.md) completed local implementation and fresh QA as `LOCAL_INTERNAL_QA_PASS / LOCAL_COMMIT_INCLUDED` in commit `0e02a83664b5c8fb22b6d8619ab7fb2366784d0b`. Push, PR, merge, deployment, P35/external side effects, and observed-user sessions remain `0`.

- [2026-08-11 Text Authoring service P0](./2026-08-11-flowme-text-authoring-service-p0/00-development-goal-ko.md) reached `LOCAL_INTERNAL_QA_PASS / OWNER_REVIEW_READY` on the preserved source branch `codex/text-authoring-service-p0-20260811`. Approval `TA-P0-PROMOTE-20260813-01` applied its exact `51`-file row set (SHA-256 `687E943319C86D9A60F947753453295AACCC7C68594DD480DE03BB5138281D45`) to target `codex/text-authoring-v5-integration-20260811` at common baseline `a5f2127eff75f8fdf91bbedd9e60526f47209049`, then fixed the allowed QA/accessibility edge in the same promotion scope. Fresh target checks pass for Text Authoring `259/259`, full test exit `0`, build exit `0` with `19` routes, and final E2E `58/58` with 4 workers in `3.9m`, including eight widths, 200% reflow, reduced motion, 44px, source-error relation, keyboard/focus, and overflow. The result is included in one local promotion commit. The [source result](../content-audit/2026-08-11-flowme-text-authoring-service-p0-results/README.md) and [promotion result](../content-audit/2026-08-13-flowme-text-authoring-p0-promotion-results/README.md) keep those evidence scopes separate. Push, PR, merge, deployment, P35/external side effects, and observed-user sessions are `0`.

### Unmerged Foundation

- [2026-08-11 Text Authoring exception coverage v5](./2026-08-11-flowme-text-authoring-exception-coverage-v5/00-development-goal-ko.md) was preserved in closed Draft PR [#175](https://github.com/knhbae/flowme2605/pull/175) as the foundation used by the local P0 slice. Its GitHub run and Vercel Preview are historical evidence only; the current latest-main stacked branches require fresh local and PR checks.
...
- [2026-07-28 FlowMe Text Authoring UX v1](./2026-07-28-flowme-text-authoring-ux-v1/spec.md) - the design record is retained while the promoted non-AI creator slice is implemented in Draft PR [#175](https://github.com/knhbae/flowme2605/pull/175). The current human-readable grammar is [authoring-grammar-logic.md](./2026-07-28-flowme-text-authoring-ux-v1/authoring-grammar-logic.md); merge, production deployment, P35 integration, and observed-user validation remain open.
