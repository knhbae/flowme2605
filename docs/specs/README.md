...
- [2026-07-18 URL-to-FLOW Source-Grounded Value Uplift v4](./2026-07-18-url-to-flow-value-uplift-v4/spec.md) - corrects the product-quality baseline to the seven live Flow-finding maps, reclassifies the same sparse twelve cases as 1 compile candidate / 9 re-extractions / 2 blocks, adds source sufficiency, `doneWhen`, projection payload, and live-reference pairwise gates, and retains v3 only as a lower source-accounting layer.
- [2026-07-18 URL-to-FLOW Prompt Lab v3 hybrid controller](./2026-07-18-url-to-flow-prompt-lab-v3-hybrid/spec.md) - freezes the same 12 cases and field ownership, compiles licensed SourceRows deterministically into Items/projections/markers, passes two fresh-process replays plus isolated model-proxy review, and grants Go only to the downstream controller contract rather than URL extraction, provider choice, cost, or production fetch.
- [2026-07-18 URL-to-FLOW Prompt Lab v2 strict semantic lane](./2026-07-18-url-to-flow-prompt-lab-v2-strict/spec.md) - records the preregistered two-round No-Go where the same sparse `예약과 문진표 준비` case failed 11/12 twice, and preserves the stop gate instead of running an unauthorized third round.
- [2026-07-15 URL-to-FLOW Prompt Lab v1 SourceRow-only corrected experiment](./2026-07-15-url-to-flow-prompt-lab-source-row-v1/spec.md) - reruns the same 12 cases with SourceRows as the intended generator-visible semantics, preserves deterministic negatives and three-round evidence, and records both failed quality gates and audited protocol deviations, including the canonical case-ID leak and missing run-to-packet hash binding, without model/cost overclaiming.
- [2026-07-14 URL-to-FLOW Prompt Lab v1 preflight](./2026-07-14-url-to-flow-prompt-lab/spec.md) - preserves the rich-packet schema/validator preflight; its canonical `userJob` and source metadata made it unsuitable as SourceRow-only generation-quality evidence, so it is excluded from corrected completion.
