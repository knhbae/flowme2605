...
- [2026-08-04 FlowMe Text Authoring optional Item review](./2026-08-04-flowme-text-authoring-optional-structure-review/00-development-goal-ko.md) is the current completed local UX correction on `codex/text-authoring-ta-implementation-20260729`. The default authoring journey is `입력 → 결과`; `>=900px` shows two simultaneous panes and `<900px` shows one staged pane. Internal Step/Item structure remains canonical but opens only from the result's optional `항목 검토` drawer/bottom sheet. Normal input can preview and save without opening it; unresolved source-linked outline issues alone promote its warning count, while source update, relative anchor, and rights/safety retain separate surfaces. Legacy `activeStage=structure` drafts normalize to result. Current evidence is authoring `161 / 161`, full unit `694 / 694`, focused Text Authoring E2E/visual scenarios `34`, build `18 / 18`, nine screenshots, and actual 27-Item mobile review scrolling. The [results](../content-audit/2026-08-04-flowme-text-authoring-optional-structure-review-results/README.md) are internal QA with observed-user sessions `0`; publication and P35 integration remain unperformed.
- [2026-07-28 FlowMe Text Authoring UX v1, superseded by the v2 authoring contract](./2026-07-28-flowme-text-authoring-ux-v1/spec.md) is the completed local Text Authoring Round 2 gate on `codex/text-authoring-ta-implementation-20260729`. Grammar, source snapshot ownership, parser/writer round-trip, source checkbox preservation, fail-closed merge/split, date projection, explicit source reorder/undo, fixed result slots, Sheet eligibility, raw/structured Text, detailed previews and links, read-only `항목 구조`, representative/QA example separation, and responsive route/standalone parity are locally implemented. The canonical writer is [text-authoring-contract-v2.json](./2026-07-28-flowme-text-authoring-ux-v1/text-authoring-contract-v2.json); v1 remains read compatibility. Current evidence is authoring `161 / 161`, full unit `694 / 694`, focused E2E `31 / 31`, v2 matrix `35 / 35`, browser QA `8 / 8`, Round 2 cross-surface `20 / 20`, production build `18 / 18`, and dependency audit `0`. The standalone HTML is `2,105,820` bytes. Text Authoring and P35 remain separate behind a versioned adapter contract; runtime integration is `HOLD_NOT_READY`. Commit, push, PR, merge, new Preview, and production deploy remain unperformed; the recorded Preview is an older snapshot and P35 production is unchanged. Publication and observed-user research are separate owner decisions. Exact evidence lives in the [Round 2 completion report](../content-audit/2026-08-04-flowme-text-authoring-v2-round2-results/README.md). Internal QA is not observed-user validation.
- P35 remains the released production baseline. The completed [P35 literal-route release-evidence follow-up](../content-audit/2026-07-29-p35-release-hardening/README.md) still records two non-fixture cases: literal `/my` opens the saved-Flow cross-Flow `할 일` view with the adjacent `Flow` view, and in a fresh one-Flow public-save representative state literal `/my?experiment=off` opens the legacy Flow hub without changing any `flow:*` localStorage key/value bytes. That evidence changes no runtime, storage, or schema behavior and does not imply that the local Text Authoring TA is released.

### Design And Research Shelf

These packages inform the active implementation or later decisions but are not parallel product queues.
