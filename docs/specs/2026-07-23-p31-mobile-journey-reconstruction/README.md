...
상태: `implementation_complete_release_verification_active`

앱 코드 변경: 있음, persistence schema migration 없음
...
사용자가 P31 전체 완료를 승인한 뒤 P31-01~05를 bounded implementation으로 연결했다.

- effective date precedence를 My Flow, Calendar, ICS에서 통일
- Home 활용 예시/이어가기와 Flow 찾기 catalog/search 역할 분리
- source 링크와 실제 대표 항목 중심 discovery card
- 결혼 artifact 선택, 운동 routine summary-first save-before
- compact mobile My Flow library와 dedicated workspace
- Calendar Item bottom sheet와 날짜 없는 항목 placement tray
- 모바일/wide 보관·복구 parity와 보관 후 영구 삭제 contract
- 24-cell 자동·휴리스틱 재실행: explanation-free `21/24`, 평균 depth `4.79`
- 실제 관찰 사용자 `0`

정본 evidence: [P31 implementation evidence](../../content-audit/2026-07-23-p31-mobile-journey-reconstruction-evidence/README.md)
