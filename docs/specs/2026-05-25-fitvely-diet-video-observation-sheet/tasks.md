...
- [x] Add RED seed tests for sheet-first FITVELY diet exact-video routes.
- [x] Add RED seed tests for `기준 후보` and `관찰표` detail copy.
- [x] Change FITVELY diet exact-video generated metadata to `sheet`.
- [x] Rewrite diet exact-video detail copy around one source rule and one observation row.
- [x] Add RED backlog test for sheet-first health-observation progress.
