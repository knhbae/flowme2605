...
Status: Living Production baseline with Flow Map Item date parity released through PR #182; the Text Authoring stack is in Draft PR integration and observed users remain `0`.
...
| `/flows/authoring` | Text Authoring 콘텐츠 목록. 명시 저장된 초안과 준비 완료 상태를 다시 열고, 이름 변경·복제·보관·복구한다. 자동 recovery를 저장 초안으로 표시하지 않으며 search/filter/hard delete/version history는 P0 제품 목록에서 제외한다. | `TextAuthoringServiceRoute`, `TextAuthoringWorkspace`, `DraftLibrary` in `components/flow/text-authoring/` | `lib/flow/text-authoring/storage`, `service-state`, namespaced local draft/recovery storage |
| `/flows/new` | 새 Text Authoring 콘텐츠 진입. 기본 제품 모드는 빈 작업 원문에 focus하고 Calendar·Todo·Sheet·TXT 네 결과를 고정 위치에서 계산한다. 평문은 Item을 발명하지 않고 TXT 원문 메모로 보존한다. `?authoringQa=0`은 대표 5개 내부 QA, `?authoringQa=1`은 전체 31-case 내부 catalog이며 둘 다 제품 데이터와 분리한다. `?legacy=1` 또는 `FLOWME_TEXT_AUTHORING_ENABLED=0`은 기존 수동 제작 화면으로 돌아간다. | `TextAuthoringServiceRoute`, `TextAuthoringWorkspace` in `components/flow/text-authoring/`; fallback `NewFlow` in `components/flow/AppClient.tsx` | `lib/flow/text-authoring/*`, `flow-experience-projection`, namespaced local draft/recovery storage |
| `/flows/authoring/[draftId]` | 명시 저장된 Text Authoring 초안 재진입. 마지막 explicit saved revision을 기본으로 열고, 더 최신인 동일 draft recovery가 있으면 자동 병합하지 않고 선택시킨다. 존재하지 않는 ID는 목록으로 정규화한다. | `TextAuthoringServiceRoute`, `TextAuthoringWorkspace` in `components/flow/text-authoring/` | coherent revision pair, SourceSnapshot/WorkingSource, recovery and ready receipts |
...
- `/flows/authoring` owns the local creator-draft list; `/flows/new` starts a new working source; `/flows/authoring/[draftId]` restores one explicit saved identity. Browser history separates these route transitions while input/result changes stay surface state.
- The left editor owns the mutable WorkingSource; the first SourceSnapshot is immutable. Parser, canonical draft, and projections may be saved only when their current revision pair is coherent.
- Recovery autosave is crash protection and never an explicit-save receipt. `초안 저장` is the durable save; `준비 완료` is a status-only receipt for that saved revision and has no publish, network, or P35 side effect.
- Calendar, Todo, Sheet, and TXT are deterministic projections of the same canonical Items. Repeated occurrences share a bounded occurrence window and stable ordered occurrence IDs; they are derived rows, not duplicated source Items.
- Plain prose remains one source-only TXT memo without invented Item, date, source, or action. Fact-only CSV/TSV/Markdown table rows stay in Sheet/TXT unless the source explicitly declares executable actions.
...
- The default product routes expose neither QA examples nor fixture/scenario IDs, parser/revision stage labels, or example taxonomy. `?authoringQa=0` and `?authoringQa=1` are separate internal QA surfaces.
- These routes store only namespaced local creator drafts and receipts. They do not write to P35 saved plans, execution state, external Calendar/Todo/Excel services, or a server-backed publish queue.
