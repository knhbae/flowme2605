...
The [M7-2 Render checkpoint](./specs/2026-09-12-flowme-integrated-product-poc-program/alpha-m7-2-personal-trial.md#926-현재--render-다기기-시험-경로-선택배포-전-코드-준비) keeps the local `/alpha` and `/auth/callback` paths while allowing one configured `https://<service>.onrender.com` origin only with `FLOWME_ALPHA_HOSTING=render-trial-v1`, `preview` stage, the existing development Supabase project, and `FLOWME_ALPHA_M3_CAPACITY=checkpoint-v1`. The [proxy follow-up](./specs/2026-09-12-flowme-integrated-product-poc-program/alpha-m7-2-personal-trial.md#926-후속--render-https-전달-경계-재현과-발행-범위) handles Next's internal request URL by requiring the exact configured external Host, HTTPS forwarded protocol, matching forwarded host when present, and matching browser Origin when present. All alpha BFF endpoints reuse that check before upstream work; server-internal media reads carry the same fixed public origin. `GET /api/alpha/health` checks configuration and signing-key shape only; it does not test Supabase availability or account writes. No Render service, Auth redirect, code publication, or deployment has been created for this trial. Production `/my` and its storage remain separate.

## Isolated alpha checked-backup candidate — 2026-09-24

The [M7-2 connection record](./specs/2026-09-12-flowme-integrated-product-poc-program/alpha-m7-2-compact-inverse.md#924-후속--저장-api와-검사한-백업-파일의-연결) owns the conditional M3 capacity/checkpoint contract. Only exact `FLOWME_ALPHA_M3_CAPACITY=checkpoint-v1` selects signed dry-run/snapshot/checkpoint commit after existing authorization/domain validation; absence retains the existing writer, invalid configuration fails closed, and failed candidate RPCs never fall back. The matching SQL remains under `supabase/candidates/`, not remotely applied or a migration. Its owner-latest full-file checkpoint is outside the journal/backup source. Retention, copied-media access and write/storage costs require a separate decision before real-data activation; the flag has not been set.

`/api/alpha/preservation` accepts optional backup `format:checked-file-v1`, returns an exact checked-file envelope, and retains the old sealed response for old requests. The panel validates file/owner/integrity and uses the supplied file without recompression. The server authenticates checkpoint seals; browser integrity validation does not authenticate the seal. In default mode, the new format packages a fresh backup without checkpoint SQL or new DB writes. No route or operating `/my` ownership changes.

## Isolated alpha catalog content intake — 2026-09-23

The [2026-09-24 OPIC connection](./specs/2026-09-12-flowme-integrated-product-poc-program/alpha-m7-2-catalog-wave3.md#w3-34-후속-실행--2026-09-24) activates exactly two frozen v3 single-day sources, bringing editable intake to 13 Flows / 103 items. v1/v2 identities and the full catalog pack remain unchanged. M3 per-tab recovery now uses an opt-in lossless versioned JSON reference representation with legacy plain reads, physical-byte CAS and unchanged expanded-domain validation; M1 storage is unchanged. The creator save/Undo RPC has a bounded 30s timeout (BFF 35s, browser 60s); the signed preservation-read RPC has a separate 20s timeout below its unchanged BFF 30s deadline. Permissions, validation, 30 MB byte limits and global/role/login settings are unchanged. The private JSON validator uses bounded strict JSONPath rather than recursive working tables while preserving its original depth/size/unsafe-key rules. The private account-shape validator and preservation reader each use a function-local generic query plan to avoid repeatedly planning large JSON arguments; their bodies, table checks, grants and byte limits are unchanged and the caller setting is restored after return. These are DEV/local changes, not a production release, a latency guarantee or an all-catalog editing claim.

While the preservation dialog is open, `AlphaWorkspace` defers only automatic interval/focus/online/visibility refreshes. Closing it requests one catch-up read when visible and idle; hidden/busy cases resume on the next eligible automatic event. Session/token binding, pending-result resolution, explicit refresh and required `onSaved` refresh remain active; existing in-flight requests are not cancelled. Unmount/owner changes and lost dialog snapshots release the hold without restoring hidden data. The server still reads the complete backup snapshot twice and checks revision/owner/session/signature before any restore. This avoids unnecessary concurrent polling, not a claim that all large-payload latency is resolved.

The [full catalog intake](./specs/2026-09-12-flowme-integrated-product-poc-program/alpha-m7-2-full-catalog.md) adds a separate immutable private `space.catalogLibrary`: the previous PoC's 177 original Bundles and 26 Maps, 21 archived policies, canonical references, and 2 distinct current-worktree variants. It is not a public repository or an execution workspace. `catalog-library-pack.v1.json` is frozen and full payload equality is validated independently of mutable seeds. The locator-only `catalog-library-import` creator command is server-reconstituted, owner-signed and CAS/ledger-bound; only this intent can write this field. M3 and other creator commands cannot change it. The existing two editable drafts below remain separate. `AlphaCatalogLibrary` provides private search/list/detail/Map links, not full-catalog editing or execution. The DEV account-shape/creator migration is additive; old readers may fail closed on this new field and must not be used as an untested rollback.

The 2026-09-26 trial access correction keeps the frozen pack in server-only source modules. `GET /api/alpha/catalog` checks the exact configured alpha origin and a non-anonymous DEV Auth identity before returning the immutable source with `private, no-store`; it makes no account write. During this trial every signed-in account may read that source and import its own private copy, including newly registered accounts. This is not a permanent public-content or ownership policy. The browser has only fixed identity/hash seals and fetches source after login; catalog creator intents bypass client reconstruction and are rebuilt and checked on the server. A signed-out request fails closed. The source module reads the untracked local pack only at runtime; Render trial hosting requires a separate `FLOWME_ALPHA_CATALOG_PACK_FILE` secret file, accepting a sealed gzip-Base64 text wrapper under the source-size bound. Missing, malformed or altered source fails closed. The existing `/my` route and operating storage remain untouched; the actual Render secret upload, clean checkout/CI source supply and publication remain gated.

Account backup files can use `flowme-alpha-backup-file/1` gzip envelopes. Decoding restores exact sealed JSON before existing integrity/owner/server-proof checks. Legacy files remain readable, compressed and expanded byte limits stay bounded, and no ledger pruning occurs. File compression does not remove existing DB history duplication or increase restore request limits.

The [M7-2 content intake](./specs/2026-09-12-flowme-integrated-product-poc-program/alpha-m7-2-content-intake.md) adds `/alpha` creator-only import of the two existing PoC catalog Flows. `catalog-content.ts` defines an immutable versioned content allowlist and an editable native projection; `catalog-content-import.ts` adds a private saved draft without changing current working input, personal execution, source attribution or shared publications. The distinct `catalog-content` native provenance is not an old D2 saved version. The UI previews original content before sending a locator-only `catalog-content-import` semantic command. Server reconstruction and exact boundary comparison restrict this intent to `creatorWorkspace`; existing signed M4 CAS/ledger/Undo applies. The additive DEV-only validator migration retains all grants and direct-write denial. Same-source import is no-op; archived/different versions and unsupported meaning fail closed. v1 parser semantics must remain readable across updates. `/my`, operating storage, production and public publication are unchanged by this feature.

## Isolated alpha persistence — 2026-09-21

The [M1 contract](./specs/2026-09-12-flowme-integrated-product-poc-program/alpha-m1-persistence.md) adds a headless module under `lib/flow/integrated-poc/alpha-persistence/`. It reuses existing pure Program transitions and validators, exposes an account-bound private repository, and tests CAS, operation receipts, session recovery and offline backup with a fake server. The local recovery adapter writes only `flow:poc:personal-workspace:v1:alpha-m1:*`. Public reference context and the operation journal are offline/server backup inputs, not account-read responses. ProgramApp's live local writer and operating storage remain unchanged.

The [M2 boundary](./specs/2026-09-12-flowme-integrated-product-poc-program/alpha-m2-auth.md) adds opt-in `/alpha` and `/auth/callback` routes using `alpha-auth/` and the M1 account validator. Only allowlisted development URL, local callback origin and a modern publishable key are accepted; production or missing configuration fails closed. Authentication uses PKCE and PoC-prefixed SDK storage. The account gateway freezes each request's token, verifies Auth identity, and discards stale generations. Development migration `20260920234519` adds RLS-protected empty-account initialization/read and a private validation bucket. Authenticated users cannot update/delete account payloads in M2; M3 must explicitly add server command semantics. Auth callback/provider readiness and real A/B evidence are recorded separately; this is not a deployment or a replacement for `/my`.

## Isolated alpha private synchronization — 2026-09-21

[M3](./specs/2026-09-12-flowme-integrated-product-poc-program/alpha-m3-sync.md) connects the authenticated `/alpha` workspace to `AlphaWorkspace`, `alpha-sync/`, and the same-origin Node endpoint `POST /api/alpha/account`. The server verifies the fixed JWT/owner, full M1 domain and immutable source boundary, then signs an owner/command-specific HMAC for the development database's atomic command RPC. Additive migrations `20260921090702` and `20260921090850` retain direct REST UPDATE/DELETE denial and introduce owner-locked CAS, operation receipts and inverse-field Undo. Private signing keys and ledgers have no browser table grants. No build or CI command applies migrations.

The UI reuses personal document, folder, period, execution, legacy detail and output components. M3 initially hid unconnected creator/public actions; the M4 and M5 sections below describe the later connections. Polling (20 seconds), focus/online refresh and owner/tab-specific sessionStorage recovery distinguish confirmed data, ambiguous requests and parked unsent text. Every browser storage write remains inside `flow:poc:personal-workspace:v1:*`. Current Undo compensates the last server transaction, not grouped local typing history. Actual source import, full backup, real devices and deployment remain separate M6–M7 gates. A static GitHub Pages UI would need a separately hosted trusted API; the current Node write route is not a static asset. `/my` and the local PoC writer remain unchanged.

## Isolated alpha creator persistence — 2026-09-21

[M4](./specs/2026-09-12-flowme-integrated-product-poc-program/alpha-m4-authoring.md) enables `Flow 만들기` inside the authenticated `/alpha` workspace. The existing creator editor distinguishes working autosave, explicit saved history, pending native input, retained source journals and private handoff. Simple empty scaffolds insert once on selection; examples and StructureDraft retain their explicit preview/apply paths. Discovery, publication and community are connected by the subsequent M5 work below.

`POST /api/alpha/creator` accepts versioned semantic creator intents or a latest-operation Undo. The server replays the pure domain transition and signs the original intent plus its generated field diff/result. Additive DEV migration `20260921105352` commits only creatorWorkspace/text/archivedDocumentIds/retentionDocuments under owner/live-session checks, CAS and the existing idempotency/inverse ledger. The older M3 endpoint still rejects creator aggregate patches. Receipt replay preserves the first generated IDs. The account remains version 1; complete old-client/backup compatibility is a separate M6 gate.

Full working recovery and unapplied comparison/property inputs use owner/tab-specific `flow:poc:personal-workspace:v1:alpha-creator-recovery:v1:*` session storage. Exact full working confirmation may acknowledge an ambiguous response without remounting the editor or deleting native Undo; newer input remains protected. Retained comparison JSON requires manual re-comparison against the latest source, not automatic choice replay. Existing native source import is exercised with synthetic domain fixtures; actual original-device migration remains M6. Administrator exact-CAS test teardown is not a product restore feature. Operating `/my`, storage and production DB are unchanged.

## Isolated alpha social persistence — 2026-09-21

[M5](./specs/2026-09-12-flowme-integrated-product-poc-program/alpha-m5-social.md) connects `둘러보기` and `내 활동`, reusing Discovery, Publisher, CopyInspector, proposal review and Community. `POST /api/alpha/social` replays 26 semantic intents using a server-only signed reference context. The development transaction locks the account and shared revisions, enforces owner/live-session checks and returns the original receipt for an identical request. Public versions are immutable; private-copy Undo cannot roll back shared changes. M3/M4 also validate accounts against the signed context after public references exist.

Public identities are opaque aliases, not Auth UUIDs or emails. Browser projections contain only the caller's private space; proposals are visible only to their author and the Flow owner. Alpha social projections are explicitly versioned and rejected by the ordinary local PoC decoder. Public output remains a no-write operation and supports an exact `/alpha` URL with a fixed version and item selection. The existing `/my?personalWorkspacePoc=v1` gate is unchanged.

`/api/alpha/media` stages actual decoded/re-encoded WebP bytes in a private development bucket. The server strips EXIF, verifies size/pixels/hash, authorizes each read and rechecks authorization after download. Registry states distinguish uploading, staged, published, detached and cancelled; only an explicit publication links a file. The authenticated object-info prerequisite and bytes GET share live-session/access checks; list/sign/copy/upsert and caller-supplied nonempty metadata are denied. Deleting a post blocks subsequent file reads; it cannot revoke bytes already downloaded. TTL is an access expiry, not an automatic physical cleanup job.

Owner/tab-specific social recovery preserves draft input. An ambiguous private-draft save is acknowledged only after an exact expected revision and full private-space match; public publication is never automatically replayed from a recovery draft. Recovery actions remain operable inside native dialogs, and closing them returns focus to a visible opener or its closed menu summary. M5 limits are replaceable DEV contracts, not final moderation, retention or public-release policies. Actual source import, file-complete backup/restore, real-device/observed-user evidence and deployment remain M6–M7/D05 gates.

## Alpha preservation and pre-use safety — 2026-09-23

[M6](./specs/2026-09-12-flowme-integrated-product-poc-program/alpha-m6-preservation.md) subsequently connects `AlphaPreservationPanel` and `POST /api/alpha/preservation`: explicit source preview/import, owner-sealed account backup and current-revision private restoration. Original source bytes, identities, records and referenced photo bytes are preserved; public state and historical server journals are not rewound by account restoration. The M3/M5 paragraphs above describe their earlier stage boundaries, not an absence of the later M6 implementation.

[M7-1](./specs/2026-09-12-flowme-integrated-product-poc-program/alpha-m7-1-readiness.md) adds a download-time restore-envelope byte check: the JSON-escaped standard restore request, including bounded command metadata, must fit the unchanged 30,000,000-byte protocol. Otherwise no successful backup package is returned. Actual DEV QA covers corrupt Storage, both cancel/restore lock orders, high-level publication/Undo rejection, and a 10,224,235-byte sealed backup round trip. The old M5 media API cannot serve restored private bytes and is not an eligible rollback build for that state. No migration, public route, production setting, `/my` writer or operating storage schema changed in M7-1. Policy decisions, real-device evidence, service-wide DR and deployment remain separate gates.
...
| Approved execution presentation | `components/flow/DateGroupedTodoList.tsx`, `components/flow/PublicFlowItemPreview.tsx`, `components/flow/FlowArtifactDataPreview.tsx`, `components/flow/FlowCapabilityResultPreview.tsx`, `components/flow/my-flow/MyFlowSortMenu.tsx`, `components/flow/FlowContextDisclosure.tsx`, `components/flow/FlowExportPanel.tsx`, `lib/flow/date-grouped-todo-list.ts`, `lib/flow/public-flow-text-syntax.ts`, `lib/flow/my-flow-local-ia.ts`, `lib/flow/approved-item-raw-memo.ts`, `lib/flow/responsive-execution-workspace.ts`, `lib/flow/export-scope.ts` | Shared pure mappings own date grouping, Plan ordering, raw memo/checklist preservation, responsive mode, export scope, and display-only copy disambiguation. **Unmerged Preview delta:** each newly opened public Flow starts on complete authoring-syntax Text with unsupported personal schedule values separated as metadata. Approved Todo and chronological Calendar previews render every eligible row, and Calendar setup precedes rows; legacy/default previews retain compact disclosure. Focused components render these mappings across public preview, My Plan, and Calendar without creating content-specific persistence owners. Public controls remain non-mutating; personal completion and artifact effects still use existing runtime commands. |
...
| Artifact UI | `FlowSaveBeforeFrame.tsx`, `FlowArtifactDataPreview.tsx`, `FlowCapabilityResultPreview.tsx`, `FlowTransferConfirmation.tsx`, `FlowExportPanel.tsx`, `FlowExportReceipt.tsx`, `RoutineScheduleEditor.tsx`, `ArtifactWorkbench.tsx`, `ArtifactPreview.tsx` | Render the compact whole-Flow outline, source-faithful read-only Text result, actual-data Todo/Calendar result, contextual adjustment, shared routine definition, immutable transfer confirmation, and saved persistent receipt. **Unmerged Preview delta:** ordinary Flows and executable Maps show all approved rows; chronological Calendar results receive setup before rows; legacy/default modes retain compact disclosure. Keep source evidence separate from personal editing. |
