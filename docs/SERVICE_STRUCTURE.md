...
| `/my` | User's saved-plan recovery and execution hub. In the unpublished Round 2 local program, literal `/my` defaults to a compact Today summary only when derived open work exists, followed by the canonical saved-plan library and an explicitly selected plan detail. Today, recent, active, completed, and archived views are lenses over the same saved identity rather than separate stores. The library keeps one discovery action at 0 plans, no search at 1 or 5 plans, and query plus one status filter at 20 plans. Save deep-links open the saved detail with one count-accurate banner; plan and Item Back restore query, filter, document/rail scroll, owning plan, and focus. Saved detail owns the authoritative transfer: format, destination, scope, IDs/count, loss, one-way, and duplicate risk are confirmed before the local clipboard/file effect; only a successful effect writes a persistent receipt. Archive keeps receipts, personal backup includes the exact registry, and permanent deletion removes only the owning plan's receipts with a session-journal reload recovery path for cleanup failure. Completion and archive remain separate. Exact `savedPlanLibrary=off`, `savedTransfer=off`, and `quickLocalResult=off` independently restore their bounded prior behavior without key rename or migration; the older `experiment=off` remains an independent cross-Flow Todo rollback inside the legacy surface. These Round 2 behaviors are local evidence, not the released production contract. | `MyFlows` in `components/flow/AppClient.tsx` | `storage`, `source-backed-my-flow`, `my-flow-local-ia`, `p35-round2-flags`, `result-transfer`, `export-receipt-storage`, `export-receipt-cleanup-journal`, `my-flow-cross-flow-todo`, `personal-flow-lifecycle`, `projection-identity`, `personal-item-detail-overlay`, `whole-flow-reading`, `my-flow-step-export`, `export`, `artifact-fields` |
...
### Current P35 Production Baseline And Round 2 Local Evidence
...
- Released P35 literal `/my` owns cross-Flow `할 일` with an adjacent saved Flow view. The unpublished Round 2 local default replaces that split with compact derived Today, one canonical saved-plan library, and selected detail; no Today heading or placeholder is rendered when Today is empty.
