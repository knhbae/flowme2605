...
Status: Living production baseline. Public plan surface release `47c54803c6bb7544aad757ce62c4ce58decbfe53` is deployed and smoked; no product gate is active.
...
The public plan surface below is the current Production behavior. [PR #176](https://github.com/knhbae/flowme2605/pull/176)
final head `3555cd1db9f426dcbc30c81652be01dd38b1ce5e` merged as
`47c54803c6bb7544aad757ce62c4ce58decbfe53`; exact-head and post-merge CI,
exact-source Production deployment, and canonical smoke `11/11` passed. The
shared presentation changes no Flow or Map persistence identity, storage key,
schema, or transaction owner. Automated production QA is not observed-user
...
| `/my` | User's saved-plan recovery and execution hub. The current Production baseline retains R3B URL-backed Plan ordering, one date-grouped Todo mapping, responsive mobile/stacked/desktop composition, and a typed selected-Plan execution boundary without adding a saved identity or completion store; `r3a-lab` remains exact-query only. Save deep-links and Plan/Item Back preserve query, selected objects, document/rail scroll, owning plan, and focus. Saved detail remains the authoritative transfer owner: format, destination, scope, IDs/count, loss, one-way, and duplicate risk are confirmed before the local clipboard/file effect, and only a successful effect writes a persistent receipt. Archive keeps receipts, backup preserves the exact registry, and permanent deletion removes only the owning plan's receipts with cleanup recovery. Exact rollback switches restore their bounded prior behavior without key rename or migration. | `MyFlows` in `components/flow/AppClient.tsx` | `storage`, `source-backed-my-flow`, `my-flow-local-ia`, `p35-round2-flags`, `result-transfer`, `export-receipt-storage`, `export-receipt-cleanup-journal`, `my-flow-cross-flow-todo`, `personal-flow-lifecycle`, `projection-identity`, `personal-item-detail-overlay`, `whole-flow-reading`, `my-flow-step-export`, `export`, `artifact-fields` |
| `/flow-maps/[map]` | Source-backed direct route. Canonical aliases redirect before rendering; AJD `moving-d30` and `curated-ajd-moving-d30` resolve to `/f/moving-d30-basic`. By default every executable Map uses the same `flow-public-shell`, artifact-first frame, and approved Text/Todo/Calendar result grammar as `/f`. `save_all` renders canonical child projections as one display result but persists through the existing atomic Map transaction; `choose_child` previews one child and navigates to its `/f/[slug]` without creating a Map save key. Exact `visualSubtraction=off` restores the exact legacy Map shell, presentation, action, and anchor behavior for both `save_all` and `choose_child`. Quality-held Maps remain noindex source-review pages with no result, edit, save, or export action. | `SourceBackedFlowMapPublicPage`, `SourceBackedFlowMapSaveExperience`, `SourceBackedFlowMapChooseChildExperience`, `SourceBackedFlowMapSaveButton`, `PublicPlanShareShell`, `PublicPlanResultPreview` in `components/flow/` | `canonical-flow-registry`, `source-backed-my-flow`, `effective-flow-map-result`, `effective-flow-map-snapshot`, `flow-map-action-contract`, existing `storage` and Map transaction owners |
...
### Locally Verified Public Plan Surface Candidate
...
- This is the current Production structure from merge
  `47c54803c6bb7544aad757ce62c4ce58decbfe53`. Exact-head and post-merge CI,
  exact-source Production deployment, and canonical smoke `11/11` passed;
  observed users remain `0`.

### Current Production Baseline And Inherited R3B/P35/Round 2 Contracts

- Public plan surface merge `47c54803c6bb7544aad757ce62c4ce58decbfe53`
...
- The current `/my` baseline uses R3B Plan ordering, date-grouped Todo mapping, responsive composition, and typed selected-Plan rendering. It adds no second saved identity, completion store, or artifact-effect owner; R3A remains only the exact-query internal snapshot surface.
...
| Source-backed map UI | `components/flow/SourceBackedFlowMapPage.tsx`, `components/flow/SourceBackedFlowMapSaveExperience.tsx`, `components/flow/SourceBackedFlowMapChooseChildExperience.tsx`, `components/flow/SourceBackedFlowMapSaveButton.tsx`, `components/flow/SourceBackedFlowMapCreatorEditor.tsx` | Route executable `save_all` and `choose_child` Maps through the shared public presentation while keeping review-hold, creator editing, child selection, controlled anchor, and existing save-to-My-Flow controller boundaries separate. Exact `visualSubtraction=off` restores the exact legacy Map shell, presentation, action, and anchor behavior for both `save_all` and `choose_child`. |
| Flow Map result adapter | `lib/flow/effective-flow-map-result.ts`, `lib/flow/effective-flow-map-snapshot.ts`, `lib/flow/source-backed-my-flow.ts` | Derive a read-only combined result from canonical child projections and the effective Map snapshot. Preserve Map owner metadata, child Flow order/boundaries, canonical row IDs, selection, destination, and anchor; never persist the synthetic display bundle as one Flow. |
...
- R3A remains an exact-query internal snapshot boundary. The current Production release retains the R3B selected-Plan rendering and saved-transfer request boundaries while adding the shared public Flow/Map presentation; Item detail/editor remains runtime-owned until a separate narrow read-only boundary is proven.
...
- `PublicPlanShareShell` and `PublicPlanResultPreview` own public presentation only. Single-Flow controllers retain single-Flow saves and receipts; Flow Map snapshot/transaction owners retain `save_all`; `choose_child` must not create a Map save record; `effective-flow-map-result` must remain read-only.
