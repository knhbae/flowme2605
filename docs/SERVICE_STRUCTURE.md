...
| `/f/[slug]` | Public Flow execution/detail route for a single slug. Canonical aliases redirect before rendering; for AJD moving, legacy public slugs resolve to the 24-item `moving-d30-basic` detail. It is a share-entry shell before save, not the persistent 4-tab app shell. The P28 Hybrid workspace leads with a compact whole-Flow outline, an actual-data content-native result, contextual include/date/title/memo/order adjustment, and exactly one visible primary save action. The legacy full artifact workbench remains available only through a secondary disclosure. Flow-level export stays secondary, source resources stay read-only, and the route continues into the same whole-Flow outline in `/my` after save. | `PublicFlow`, `FlowSaveBeforeFrame`, `FlowArtifactDataPreview` in `components/flow/` | `canonical-flow-registry`, `flow-experience-projection`, `whole-flow-reading`, `seed-flows`, `parser`, `export`, `storage`, `artifact-plan`, personal overlay helpers |
...
| `/calendar` | Global date-first execution entry. It applies one persisted multi-Flow scope predicate to the month grid, selected-day agenda, undated placement, and counts; archived/excluded content is absent. The scope control is hidden for one or fewer Flows, compact through five, and becomes a searchable picker at six or more. Routine definitions stay outside execution, occurrence wrappers are named non-tabbable groups, and each executable occurrence owns one completion control. | `MyFlows` with `surface="calendar"`, `CalendarFlowScopePicker` | `storage`, `source-backed-my-flow`, `calendar-flow-scope`, `my-flow-step-export`, `export`, recurrence projection |
| `/my` | User's saved execution hub. Mobile uses a library-to-detail drill-in with a clear return action; wide layouts use one bounded library rail and one detail pane. Small libraries remain directly browsable and five-plus libraries expose search/filter. A search result opens the selected whole-Flow workspace without a duplicate selector. Flow archive is reversible and preserves source, overlay, run, and export history. Item detail owns quick editing plus separate subcheck/resource disclosures; source records remain immutable while personal overlays own exclusion, nested changes, and order. The post-save receipt is a compact band over the same returning outline, and export opens as a scope/count/destination preflight. | `MyFlows` in `components/flow/AppClient.tsx` | `storage`, `source-backed-my-flow`, `personal-flow-lifecycle`, `personal-item-detail-overlay`, `my-flow-local-ia`, `whole-flow-reading`, `my-flow-step-export`, `export`, `artifact-fields` |
