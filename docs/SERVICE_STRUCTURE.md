...
| `/f/[slug]` | Public Flow execution/detail route for a single slug. It is a share-entry shell before save, not the persistent 4-tab app shell. It leads with the complete effective Flow preview and exactly one visible primary save action. `조정` stays on the same route and activates only one personal operation at a time: include, date, title/memo, or order. Flow-level export stays secondary, source resources stay read-only, and the route continues into the same whole-Flow outline in `/my` after save. | `PublicFlow` in `components/flow/AppClient.tsx` | `seed-flows`, `parser`, `export`, `storage`, `artifact-plan`, personal overlay helpers |
...
| `/calendar` | Global date-first execution entry. It applies one Flow scope to grid, selected-day agenda, and undated placement; archived/excluded content is absent. Routine definitions stay outside execution, occurrence wrappers are named non-tabbable groups, and each executable occurrence owns one completion control. | `MyFlows` with `surface="calendar"` | `storage`, `source-backed-my-flow`, `calendar-flow-scope`, `my-flow-step-export`, `export`, recurrence projection |
| `/my` | User's saved execution hub. `지금` groups cross-Flow execution by date; `Flow 목록` manages whole saved Flows; `완료` keeps reopenable history. Small libraries remain directly browsable and five-plus libraries expose search/filter. A search result opens the selected whole-Flow workspace. Flow archive is reversible and preserves source, overlay, run, and export history. Item detail owns quick editing plus separate subcheck/resource disclosures; source records remain immutable while personal overlays own exclusion, nested changes, and order. The post-save receipt is a compact band over the same returning outline, and export opens as a scope/count/destination preflight. | `MyFlows` in `components/flow/AppClient.tsx` | `storage`, `source-backed-my-flow`, `personal-flow-lifecycle`, `personal-item-detail-overlay`, `my-flow-local-ia`, `my-flow-step-export`, `export`, `artifact-fields` |
