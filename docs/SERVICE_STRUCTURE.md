...
- R3B merge `2b937ce811b518950f495341d05736ebd102887a` is the current production
  source. Exact-head and post-merge CI passed, the exact source reached a
  successful Production deployment, and canonical approved-spec smoke passed
  `23/23`. Exact `myFlowExperience=r3a-lab` remains an internal query-only
  surface. Automated production QA is not observed-user validation.
...
- R3A remains an exact-query internal snapshot boundary. R3B is the current production presentation and separates selected-Plan rendering and saved-transfer request planning behind typed boundaries; Item detail/editor remains runtime-owned until a separate narrow read-only boundary is proven.
