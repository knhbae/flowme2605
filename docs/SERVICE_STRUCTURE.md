...
The [M7-2 connection record](./specs/2026-09-12-flowme-integrated-product-poc-program/alpha-m7-2-compact-inverse.md#924-후속--저장-api와-검사한-백업-파일의-연결) owns the conditional local M3 capacity/checkpoint experiment. Only exact `FLOWME_ALPHA_M3_CAPACITY=checkpoint-v1` selects signed dry-run/snapshot/checkpoint commit after existing authorization/domain validation; absence retains local compatibility, explicit `on-demand-v1` selects the deployed writer/request-time backup, invalid configuration fails closed, and failed candidate RPCs never fall back. The hosted trial rejects checkpoint mode. The matching SQL remains a local candidate, not remotely applied or a migration. Its owner-latest full-file checkpoint is outside the journal/backup source. Retention, copied-media access and write/storage costs remain unresolved. The first Render deployment mistakenly required this unapplied mode; the linked correction records that failure rather than claiming the candidate was activated successfully.
